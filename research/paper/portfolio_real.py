...
HOLDS = [30, 60, 120, 240]
# 台帳 execution_ledger.db の ORDER_FILL 495件から、約定時点の fullPrice で
# 直接測ったペア別スプレッド（中央値）。コーパスの提示値は USD_JPY で 1.70 で、
# 実際の 2.12 倍。全バックテストが2倍以上のコストを課していた。
REAL_SPREAD = {"USD_JPY": 0.80, "EUR_USD": 0.80, "GBP_USD": 1.30, "AUD_USD": 1.40}
EFFECTIVE_ROUND_TRIP = 0.80
...
for pair in PAIRS:
    ...
    for k, dd in run(pair, HOLDS, REAL_SPREAD.get(pair, cost) / 2).items():
        for d, v in dd.items():
            merged[k][d] += v

print(f"\n=== 実約定スプレッド（ペア別・台帳実測）で採点（4ペア合算・保有 {HOLDS} 分）===")
