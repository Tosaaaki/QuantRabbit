...
def run(input_root: Path, source_ledger: Path, output_root: Path) -> dict:
    ...
    ledger = output_root / "proposal_ledger_currency_network_exhaustion_v19.jsonl"
    ledger.write_text("".join(json.dumps(row, sort_keys=True, allow_nan=False) + "\n" for row in rows), encoding="utf-8")
    payload = {
        "experiment": "FX_CURRENCY_NETWORK_EXHAUSTION_V19",
        "family": "V18_V19_MUTUALLY_EXCLUSIVE_CURRENCY_NETWORK",
        "family_hypotheses": 2,
        "evidence_class": "opened_development_not_future_holdout",
        "single_changed_variable": "excluding_self_currency_network_alignment_positive_gate",
        ...
        "admission_blockers": [
            ...
            "family correction for V18/V19 is not yet complete",
    ...
    result = output_root / "result_currency_network_exhaustion_v19.json"
...
def main() -> int:
    ...
    print(json.dumps({
        "source_major_signals": result["source_major_signals"], "admitted_raw_signals": result["admitted_raw_signals"],
