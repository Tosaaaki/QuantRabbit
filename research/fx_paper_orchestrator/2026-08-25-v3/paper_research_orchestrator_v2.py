...
COMPONENT_REGISTRY_V1_PATH = "evidence/component_worker_registry_v1/component_worker_registry_v1.json"
COMPONENT_REGISTRY_PATH = "evidence/component_worker_registry_v2/component_worker_registry_v2.json"
...
def load_registry(root: Path, registry_path: Path) -> dict[str, Any]:
    ...
    independence_ref = require_keys(registry.get("component_worker_independence_policy"), {
        "path", "sha256", "builder_path", "builder_sha256", "classification",
    }, "component_worker_independence_policy")
    if independence_ref["classification"] != "NON_STRATEGY_ORCHESTRATOR_EVIDENCE_POLICY":
        raise ContractError("sign-aware independence policy classification changed")
    for path_key, hash_key in (("path", "sha256"), ("builder_path", "builder_sha256")):
...
def validate_cycle_contract(root: Path, cycle: dict[str, Any]) -> None:
    ...
    if raw_source == "GENERATED_IN_CYCLE":
        ...
        if cycle["cycle_id"] == "V35":
            prereg = json.loads(within(root, cycle["preregistration"]).read_text(encoding="utf-8"))
            failure = prereg.get("failed_predecessor", {})
            selection = prereg.get("training_only_rule_selection", {})
            rule = prereg.get("admission_rule", {})
            if failure.get("cycle_id") != "V34" \
                    or failure.get("status") != "FAILED_RESULT_VALIDATION_NO_RERUN" \
                    or failure.get("metrics_admissible") is not False \
                    or failure.get("rerun_permitted") is not False \
                    or prereg.get("work_order", {}).get("sha256") \
                    != "e2120996e628b0a72ecc6956b713b211f6bce6e01a31c5611f19c78f491b6e17":
                raise ContractError("V35 did not preserve V34 as an inadmissible terminal failure")
            if selection.get("candidate_rules_preregistered") != 1 \
                    or selection.get("candidate_rules_compared_by_outcome") != 0 \
                    or selection.get("v34_result_metrics_used") is not False \
                    or selection.get("cost_consulted") is not False \
                    or selection.get("post_entry_return_outcome_consulted") is not False \
                    or selection.get("evaluation_month_used") is not False \
                    or selection.get("holdout_used") is not False:
                raise ContractError("V35 admission rule was not one outcome-free preregistered candidate")
            if rule.get("name") != "GLOBAL_HARD_AGE_NO_OVERLAP_ADMISSION" \
                    or rule.get("hard_spacing_seconds") \
                    != cycle["inventory_contract"]["finite_max_age_seconds"] \
                    or rule.get("same_v33_raw_signal_ids_directions_timestamps") is not True \
                    or rule.get("same_mask_all_cost_arms") is not True \
                    or rule.get("cost_or_outcome_inputs") is not False:
                raise ContractError("V35 global no-overlap rule differs from preregistration")
            selectors = [node for node in tree.body if isinstance(node, ast.FunctionDef)
                         and node.name == "apply_rule"]
            rankers = [node for node in tree.body if isinstance(node, ast.FunctionDef)
                       and node.name == "daily_representatives"]
            if len(selectors) != 1 or [arg.arg for arg in selectors[0].args.args] != ["rows"] \
                    or len(rankers) != 1 or [arg.arg for arg in rankers[0].args.args] != ["rows"]:
                raise ContractError("V35 deterministic admission selector contract mismatch")
        if cycle["cycle_id"] == "V36":
            prereg = json.loads(within(root, cycle["preregistration"]).read_text(encoding="utf-8"))
            predecessor = prereg.get("predecessor_disposition", {})
            selection = prereg.get("training_only_family_selection", {})
            rule = prereg.get("signal_family_rule", {})
            if predecessor.get("cycle_id") != "V35" \
                    or predecessor.get("status") != "FROZEN_REJECTED_EVIDENCE_NO_REWRITE_NO_RERUN" \
                    or predecessor.get("reason_code") != "GLOBAL_NO_OVERLAP_ADVERSE_COST_FRAGILE" \
                    or predecessor.get("work_order_sha256") \
                    != "9b3926f810070e31b88ce25e3565c1c18f5c6f571f65cfda90eabca9a2ba44f1":
                raise ContractError("V36 did not preserve V35 and its signal-family work order")
            if selection.get("candidate_signal_families_preregistered") != 1 \
                    or selection.get("candidate_signal_families_compared_by_outcome") != 0 \
                    or selection.get("post_entry_return_outcome_consulted") is not False \
                    or selection.get("cost_consulted") is not False \
                    ...
                    or selection.get("walk_forward_used_for_selection") is not False \
                    or selection.get("holdout_used") is not False:
                raise ContractError("V36 signal family was not one outcome-free preregistered candidate")
            if rule.get("name") != "LONDON_OPEN_COMPLETED_ASIAN_RANGE_BREAKOUT" \
        ...
        if cycle["cycle_id"] == "V38":
            prereg = json.loads(within(root, cycle["preregistration"]).read_text(encoding="utf-8"))
            predecessor = prereg.get("predecessor_disposition", {})
            selection = prereg.get("training_only_family_selection", {})
            rule = prereg.get("signal_family_rule", {})
            if predecessor.get("cycle_id") != "V37" \
                    or predecessor.get("status") != "FROZEN_REJECTED_EVIDENCE_NO_REWRITE_NO_RERUN" \
                    or predecessor.get("reason_code") != "LONDON_ASIAN_RANGE_BREAKOUT_RAW_EDGE_ABSENT" \
                    or predecessor.get("work_order_sha256") \
                    != "7a1ab3526ceb10408b236c2369a33179c8a54fa7bd32c85fc7e530554c7f6f1c":
                raise ContractError("V38 did not preserve V37 and its signal-family work order")
            if selection.get("candidate_signal_families_preregistered") != 1 \
                    or selection.get("candidate_signal_families_compared_by_outcome") != 0 \
                    or selection.get("post_entry_return_outcome_consulted") is not False \
                    or selection.get("cost_consulted") is not False \
                    ...
                    or selection.get("walk_forward_used_for_selection") is not False \
                    or selection.get("holdout_used") is not False:
                raise ContractError("V38 signal family was not one outcome-free preregistered candidate")
            if rule.get("name") != "LONDON_OVEREXTENSION_BEYOND_COMPLETED_ASIAN_RANGE_FADE" \
                    or rule.get("eligibility_formula") \
                    != "abs(log(C1155/O0800)) > log(H_asian/L_asian)" \
                    or rule.get("direction_formula") != "-sign(log(C1155/O0800))" \
                    or rule.get("maximum_signals_per_pair_utc_day") != 1 \
                    or rule.get("same_signal_action_state_all_cost_arms") is not True \
                    or rule.get("cost_or_post_entry_outcome_inputs") is not False:
        ...
        if cycle["cycle_id"] == "V40":
            prereg = json.loads(within(root, cycle["preregistration"]).read_text(encoding="utf-8"))
            predecessor = prereg.get("predecessor_disposition", {})
            selection = prereg.get("training_only_family_selection", {})
            rule = prereg.get("signal_family_rule", {})
            fixed = prereg.get("fixed_contracts", {})
            if predecessor.get("cycle_id") != "V39" \
                    or predecessor.get("status") != "FROZEN_REJECTED_EVIDENCE_NO_REWRITE_NO_RERUN" \
                    or predecessor.get("reason_code") \
                    != "LONDON_OVEREXTENSION_CARRY_MONTHLY_GATE_SHORTFALL" \
                    or predecessor.get("work_order_sha256") \
                    != "f4018e3239ab1588a6f147b8b97efb7bb2350d0ba6d3e4998f0e58ab6232cda0":
                raise ContractError("V40 did not preserve V39 and its signal-family work order")
            if selection.get("candidate_signal_families_preregistered") != 1 \
                    or selection.get("candidate_signal_families_compared_by_outcome") != 0 \
                    or selection.get("post_entry_return_outcome_consulted") is not False \
                    or selection.get("cost_consulted") is not False \
                    ...
                    or selection.get("walk_forward_used_for_selection") is not False \
                    or selection.get("holdout_used") is not False:
                raise ContractError("V40 signal family was not one outcome-free preregistered candidate")
            if rule.get("name") != "LONDON_FIX_PREWINDOW_OVEREXTENSION_FADE" \
                    or rule.get("eligibility_formula") \
                    != "abs(log(C1555/O1200)) > log(H_0800_1155/L_0800_1155)" \
                    or rule.get("direction_formula") != "-sign(log(C1555/O1200))" \
                    or rule.get("maximum_signals_per_pair_utc_day") != 1 \
                    or rule.get("same_signal_action_state_all_cost_arms") is not True \
                    or rule.get("cost_or_post_entry_outcome_inputs") is not False \
...
def validate_result(root: Path, cycle: dict[str, Any]) -> dict[str, Any]:
    ...
    if cycle["cycle_id"] in {"V36", "V37"}:
        ...
        if indicator.get("asian_range_utc") != "00:00-05:55_COMPLETED_MID_HIGH_LOW" \
                ...
                or indicator.get("fill_utc") != "08:00_EXECUTABLE_OPEN" \
                or indicator.get("threshold") != "STRICT_RANGE_BOUNDARY_NO_TUNED_BUFFER" \
                or indicator.get("cost_used_for_signal") is not False \
                or indicator.get("post_entry_outcome_used_for_signal") is not False \
                or indicator.get("evaluation_month_used_for_threshold") is not False:
    ...
    if cycle["cycle_id"] == "V38":
        if payload.get("cycle_id") != "V38" \
                or payload.get("experiment") != "FX_LONDON_OVEREXTENSION_FADE_V38" \
                or payload.get("family") != "FX_SESSION_RANGE_NORMALIZED_MEAN_REVERSION" \
                or payload.get("single_changed_variable") \
                != "fx_specific_london_overextension_fade_signal_family" \
                or payload.get("same_execution_actions_all_cost_arms") is not True \
                or payload.get("same_execution_state_transitions_all_cost_arms") is not True:
            raise ContractError("V38 result identity or cost-arm parity mismatch")
        indicator = payload.get("indicator", {})
        if indicator.get("eligibility_formula") \
                != "abs(log(C1155/O0800)) > log(H_asian/L_asian)" \
                or indicator.get("direction_formula") != "-sign(log(C1155/O0800))" \
                or indicator.get("decision_utc") != "11:55_COMPLETED" \
                or indicator.get("fill_utc") != "12:00_EXECUTABLE_OPEN" \
                or indicator.get("cost_used_for_signal") is not False \
                or indicator.get("post_entry_outcome_used_for_signal") is not False \
                or indicator.get("evaluation_month_used_for_threshold") is not False:
            raise ContractError("V38 result signal formula differs from preregistration")
        if any(row.get("execution_selected") is not True for row in rows) \
                or any(row["utc_day"] >= "2026-07-01" for row in rows):
            raise ContractError("V38 ledger selection or evaluation scope mismatch")
        if any(not isinstance(row.get("arm_actions"), dict)
               or set(row["arm_actions"]) != set(ARMS)
               or len(set(row["arm_actions"].values())) != 1 for row in rows):
    ...
    if cycle["cycle_id"] == "V40":
        if payload.get("cycle_id") != "V40" \
                or payload.get("experiment") != "FX_LONDON_FIX_OVEREXTENSION_FADE_V40" \
                or payload.get("family") != "FX_LONDON_FIX_MEAN_REVERSION" \
                or payload.get("single_changed_variable") \
                != "fx_specific_london_fix_overextension_fade_signal_family" \
                or payload.get("same_execution_actions_all_cost_arms") is not True \
                or payload.get("same_execution_state_transitions_all_cost_arms") is not True:
            raise ContractError("V40 result identity or arm parity mismatch")
        indicator = payload.get("indicator", {})
        execution_rule = payload.get("execution_rule", {})
        if indicator.get("eligibility_formula") \
                != "abs(log(C1555/O1200)) > log(H_0800_1155/L_0800_1155)" \
                or indicator.get("direction_formula") != "-sign(log(C1555/O1200))" \
                or indicator.get("decision_utc") != "15:55_COMPLETED" \
                or indicator.get("fill_utc") != "16:00_EXECUTABLE_OPEN" \
                or indicator.get("cost_used_for_signal") is not False \
                or indicator.get("post_entry_outcome_used_for_signal") is not False \
                or execution_rule.get("target_hold_seconds") != 14100:
            raise ContractError("V40 result signal formula or fixed carry mismatch")
        if any(row.get("execution_selected") is not True for row in rows) \
                or any(row["utc_day"] >= "2026-07-01" for row in rows):
            raise ContractError("V40 ledger selection or evaluation scope mismatch")
        if any(not isinstance(row.get("arm_actions"), dict)
               or set(row["arm_actions"]) != set(ARMS)
               or len(set(row["arm_actions"].values())) != 1 for row in rows):
    ...
    if cycle["cycle_id"] in {"V34", "V35"}:
        cycle_id = cycle["cycle_id"]
        expected_experiment = {
            "V34": "FX_CAUSAL_TAIL_EXCESS_REPRESENTATIVE_V34",
            "V35": "FX_GLOBAL_NO_OVERLAP_ADMISSION_V35",
        }[cycle_id]
        if payload.get("cycle_id") != cycle_id \
                or payload.get("experiment") != expected_experiment \
                or payload.get("same_execution_mask_all_cost_arms") is not True \
                or payload.get("same_execution_state_transitions_all_cost_arms") is not True:
            raise ContractError(f"{cycle_id} result identity or arm parity mismatch")
        ...
        if [[row[field] for field in identity] for row in rows] != [
            ...
            raise ContractError(f"{cycle_id} changed sealed V33 RAW signal identity")
        if payload.get("parent_ledger_sha256") != cycle["signal_contract"]["parent_ledger_sha256"] \
                or payload.get("parent_result_sha256") != cycle["signal_contract"]["parent_result_sha256"] \
                or payload.get("same_parent_signal_id_set") is not True \
                or payload.get("same_parent_decision_timestamps") is not True \
                or payload.get("same_parent_directions") is not True:
            raise ContractError(f"{cycle_id} parent identity provenance mismatch")
        mask = [[row["signal_id"], row.get("execution_selected") is True] for row in rows]
        if hashlib.sha256(canonical_bytes(mask)).hexdigest() != payload.get("execution_mask_sha256"):
            raise ContractError(f"{cycle_id} execution mask hash mismatch")
        if any(not isinstance(row.get("arm_actions"), dict)
               or set(row["arm_actions"]) != set(ARMS)
               or len(set(row["arm_actions"].values())) != 1 for row in rows):
            raise ContractError(f"{cycle_id} actions differ across cost arms")
        ...
        if cycle_id == "V34" and (not selected_by_day or any(count != 1 for count in selected_by_day.values())):
        ...
        if cycle_id == "V34":
            ...
            selected_rows = [row for row in rows if row.get("execution_selected") is True]
            if not selected_rows:
                raise ContractError("V35 selected no global representative")
            selected_fill_ns = [parse_time(row["fill_time"]).value for row in selected_rows]
            spacing_ns = cycle["inventory_contract"]["finite_max_age_seconds"] * 1_000_000_000
            if any(right - left < spacing_ns for left, right in zip(
                    selected_fill_ns, selected_fill_ns[1:])):
                raise ContractError("V35 admitted overlapping hard-age windows")
            if payload.get("failed_predecessor_cycle") != "V34" \
                    or payload.get("v34_result_metrics_used") is not False \
                    or rule.get("name") != "GLOBAL_HARD_AGE_NO_OVERLAP_ADMISSION" \
                    or rule.get("hard_spacing_seconds") \
                    != cycle["inventory_contract"]["finite_max_age_seconds"] \
                    or rule.get("cost_inputs") is not False \
                    or rule.get("post_entry_outcome_inputs") is not False \
                    or rule.get("v34_result_metrics_used") is not False:
                raise ContractError("V35 result no-overlap rule differs from preregistration")
        ...
        for arm in ARMS:
            ...
            if cycle["cycle_id"] in {"V32", "V33", "V34", "V35", "V36", "V37", "V38", "V39", "V40", "V41"}:
                ...
                if set(metrics) < required_metrics:
                    raise ContractError(f"{cycle['cycle_id']} required metrics missing in {period_name}/{arm}")
                if metrics.get("processed_raw_signals") != raw_count \
                        or (cycle["cycle_id"] not in {"V34", "V35"} and metrics.get("cash_signals") != 0):
                    raise ContractError(f"{cycle['cycle_id']} did not process the complete RAW ledger in {period_name}/{arm}")
                if metrics["max_inventory_age_seconds"] > max_age:
                    raise ContractError(f"{cycle['cycle_id']} max-age exceeded in {period_name}/{arm}")
                if cycle["cycle_id"] in {"V34", "V35"}:
                    if metrics.get("executed_signals", 0) + metrics.get("cash_signals", 0) != raw_count:
                        raise ContractError(f"{cycle['cycle_id']} execution/cash partition mismatch in {period_name}/{arm}")
                    if metrics.get("max_gross_exposure_nav", 0) \
                            > cycle["inventory_contract"]["rule_max_gross_leverage"] + 1e-12:
                        raise ContractError(f"{cycle['cycle_id']} representative exposure exceeded in {period_name}/{arm}")
...
def route_next_work_order(
    ...
    if result_reason == "EXECUTION_SUBSET_RAW_EDGE_ABSENT":
        ...
        variable = SIGNAL_FAMILY_PIVOT_VARIABLE
            ...
            "LONDON_ASIAN_RANGE_BREAKOUT_RAW_EDGE_ABSENT",
            "LONDON_ASIAN_RANGE_BREAKOUT_COST_DOMINANT",
            "LONDON_ASIAN_RANGE_BREAKOUT_ADVERSE_COST_FRAGILE",
    }:
        reason = result_reason
        variable = SIGNAL_FAMILY_PIVOT_VARIABLE
    elif result_reason in {
            "LONDON_OVEREXTENSION_FADE_RAW_EDGE_ABSENT",
            "LONDON_OVEREXTENSION_FADE_COST_DOMINANT",
            "LONDON_OVEREXTENSION_FADE_ADVERSE_COST_FRAGILE",
    }:
        reason = result_reason
        variable = SIGNAL_FAMILY_PIVOT_VARIABLE
    elif result_reason in {
        ...
        variable = SIGNAL_FAMILY_PIVOT_VARIABLE
            ...
            "LONDON_FIX_FADE_RAW_EDGE_ABSENT",
...
def component_portfolio_proposal_eligible(root: Path) -> bool:
    ...
    if not path.exists():
        return False
    try:
        import component_worker_evidence_registry_v2 as component_registry
...
def audit(root: Path, registry: dict[str, Any]) -> dict[str, Any]:
    ...
    component_summary = {"status": "V2_NOT_BUILT", "portfolio_composition_proposal_allowed": False}
...
def build_component_registry_v2_checkpoint(root: Path) -> dict[str, Any]:
    """Build the sign-aware non-strategy independence evidence under the shared lock."""
    import component_worker_evidence_registry_v2 as component_registry
    paths = state_paths(root)
    with exclusive_lock(paths["lock"], paths["journal"]):
        registry_path = within(root, component_registry.REGISTRY_PATH)
        if registry_path.exists():
            ...
            append_journal(paths["journal"], "COMPONENT_REGISTRY_V2_BUILD_STARTED",
                           classification="NON_STRATEGY_SIGN_AWARE_INDEPENDENCE_EVIDENCE")
            try:
                built = component_registry.build(root)
                recovered = False
            except (OSError, ValueError, RuntimeError) as error:
                append_journal(
                    paths["journal"], "COMPONENT_REGISTRY_V2_BUILD_FAILED",
                    error_sha256=hashlib.sha256(str(error).encode()).hexdigest(),
                    error_excerpt=sanitized_subprocess_excerpt(str(error), limit=500),
                    strategy_rerun=False,
                )
        ...
        validated = component_registry.validate(root)
        if built["registry_sha256"] != validated["registry_sha256"]:
            raise ContractError("component registry V2 build/readback mismatch")
        journal_events = [json.loads(line) for line in paths["journal"].read_text(encoding="utf-8").splitlines()
                          if line.strip()] if paths["journal"].is_file() else []
        current_file_hash = sha256_file(registry_path)
        if not any(event.get("event") == "COMPONENT_REGISTRY_V2_SEALED"
                   and event.get("registry_file_sha256") == current_file_hash
                   for event in journal_events):
            append_journal(
                paths["journal"], "COMPONENT_REGISTRY_V2_SEALED",
                registry_file_sha256=current_file_hash,
                embedded_registry_sha256=validated["registry_sha256"],
                portfolio_composition_proposal_allowed=validated["portfolio_composition_proposal_allowed"],
                strategy_adoption_authorized=False, recovered_without_strategy_rerun=recovered,
            )
...
def main() -> int:
    ...
    parser.add_argument("command", choices=(
        "audit", "build-component-registry", "build-component-registry-v2",
        "execute-next", "execute-v26-recovery", "status",
    ))
    ...
    try:
        ...
        elif args.command == "build-component-registry-v2":
            import component_worker_evidence_registry_v2 as component_registry
            built = build_component_registry_v2_checkpoint(root)
            result = {
                "registry_path": component_registry.REGISTRY_PATH,
                "registry_file_sha256": sha256_file(within(root, component_registry.REGISTRY_PATH)),
                "portfolio_composition_proposal_allowed": built["portfolio_composition_proposal_allowed"],
                "pair_classifications": [item["classification"] for item in built["sign_aware_pair_reviews"]],
                "strategy_adoption_authorized": False,
                "authority": AUTHORITY,
            }
