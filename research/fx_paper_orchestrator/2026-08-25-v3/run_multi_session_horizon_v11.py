...
def run(input_root: Path, source_ledger: Path, output_root: Path) -> dict:
    ...
    for source in source_rows:
        ...
        if any(value is None for value in scores.values()):
            continue
        rows.append({
            "signal_id": source["signal_id"], "pair": pair, "fill_time": source["fill_time"],
            "exit_time": corpus[pair][fill_index + HORIZON].time,
            "direction": int(source["direction"]), "horizon_m5_bars": HORIZON, "scores": scores,
        })
    ...
    ledger = output_root / "proposal_ledger_multi_session_h384_v11.jsonl"
    ledger.write_text("".join(json.dumps(row, sort_keys=True, allow_nan=False) + "\n" for row in rows), encoding="utf-8")
    payload = {
        "experiment": "FX_AUCTION_TRAP_LIQUID_MAJORS_H384_V11",
        "evidence_class": "opened_development_not_future_holdout",
        "single_changed_variable": "fixed_horizon_192_to_384_m5_bars",
    ...
    result = output_root / "result_multi_session_horizon_v11.json"
...
def main() -> int:
    ...
    print(json.dumps({
        "rescored_signals": result["rescored_signals"], "walk_forward": result["periods"]["WALK_FORWARD"],
