#!/usr/bin/env python3
"""Credential-free, file-only forward-shadow plumbing.
...
class OfflineBBOFile:
    ...
    def snapshot(self) -> SourceSnapshot:
        ...
        if not self.path.is_file():
            raise ShadowCoreError("LOCAL_FILE_REQUIRED", self.path.name)
        ...
        if (
            ...
            raise ShadowCoreError("SOURCE_CHANGED_DURING_READ", self.path.name)
    ...
    def ingest(self, source: LocalFileEventSource) -> dict[str, Any]:
        ...
        if existing_manifest is not None:
            if (
                ...
                or existing_manifest["source_mtime_ns"] != snapshot.source_mtime_ns
    ...
    for worker_arm in WORKER_ARMS:
        ...
        for scenario_name, scenario_spec in EXECUTION_SCENARIOS.items():
            ...
            if execution_key not in missing_keys:
                continue
            target = proposal.decision_ts_ns + int(scenario_spec["latency_ns"])
            fill_row = next(
                (row for row in pair_rows if row["event"]["source_ts_ns"] >= target),
                None,
            )
            if fill_row is None:
...
def finalize_period(store: ShadowStore, period_end_ts_ns: int) -> dict[str, Any]:
    ...
    prior = [
        ...
        and row.get("period_end_ts_ns") == period_end_ts_ns
    ]
    if prior:
        return prior[-1]["summary"]
    opens = _open_virtual_positions(store)
    try:
        conversion_book = _conversion_book(store)
    except accounting.AccountingError as error:
