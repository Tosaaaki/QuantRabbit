...
import build_forward_shadow_core_checkpoint_v2 as checkpoint
...
def copy_builder_inputs(destination: Path) -> None:
    for relative in (
        ...
        checkpoint.ADVERSARIAL_TEST_PATH,
        checkpoint.CHECKPOINT_TEST_PATH,
        "shadow_jpy_accounting_v1.py",
        checkpoint.SUPERSEDED_AUDIT_PATH,
...
def assert_checkpoint_boundary(payload: dict) -> None:
    assert payload["checkpoint_id"] == "CREDENTIAL_FREE_FORWARD_SHADOW_CORE_CORRECTIVE_V2"
    ...
    assert payload["supersession"]["prior_evidence_status"] == "SUPERSEDED_NOT_ADMISSIBLE"
    assert payload["supersession"]["prior_evidence_preserved"] is True
    ...
    assert payload["restart_safety"][
        "manifest_ledger_checkpoint_source_blob_bidirectional_binding"
    ] is True
    assert all(payload["adversarial_corrections"].values())
