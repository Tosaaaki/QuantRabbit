...
POLICY_PATH = "FORWARD_SHADOW_CORE_CONTRACT_V1.json"
RUNTIME_PATH = "forward_shadow_core_v1.py"
CORE_TEST_PATH = "test_forward_shadow_core_v1.py"
CHECKPOINT_TEST_PATH = "test_forward_shadow_core_checkpoint_v1.py"
BUILDER_PATH = "build_forward_shadow_core_checkpoint_v1.py"
EVIDENCE_ROOT = "evidence/forward_shadow_core_v1"
AUDIT_PATH = f"{EVIDENCE_ROOT}/forward_shadow_core_checkpoint_v1.json"
VALID_MANIFEST_PATH = f"{EVIDENCE_ROOT}/valid_batch_manifest_v1.json"
BARS_PATH = f"{EVIDENCE_ROOT}/completed_bars_v1.jsonl"
QUALITY_PATH = f"{EVIDENCE_ROOT}/quality_failure_matrix_v1.jsonl"
EXECUTION_PATH = f"{EVIDENCE_ROOT}/shared_proposal_execution_v1.jsonl"
...
def _runtime_import_roots(root: Path) -> list[str]:
    tree = ast.parse((root / RUNTIME_PATH).read_text(encoding="utf-8"))
    result = set()
...
def _source_hashes(root: Path) -> dict[str, str]:
    paths = (
        ...
        CHECKPOINT_TEST_PATH, "jpy_accounting_v2.py",
    )
...
def _compute(root: Path) -> tuple[dict[str, str], dict[str, Any]]:
    ...
    with tempfile.TemporaryDirectory() as temporary:
        ...
        final = shadow.finalize_period(
            store, _start_ns() + 250 * 60 * 1_000_000_000
        )
        ...
        execution_rows = [
            *store.proposal_ledger.rows,
            *store.virtual_ledger.rows,
        ]
        ...
        payload = {
            "schema_version": 1,
            "checkpoint_id": "CREDENTIAL_FREE_FORWARD_SHADOW_CORE_V1",
            ...
            "restart_safety": {
                "state_sha256_before_restart": status_before_restart["state_sha256"],
                "state_sha256_after_restart": status_after_restart["state_sha256"],
                "checkpoint_sha256_before_restart": checkpoint_before[
                    "checkpoint_sha256"
                ],
                "checkpoint_sha256_after_restart": checkpoint_after[
                    "checkpoint_sha256"
                ],
