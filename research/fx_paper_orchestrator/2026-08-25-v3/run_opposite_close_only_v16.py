...
def simulate_pair(pair, bars, source_rows, arm, start, end):
    eligible = [
    ...
    marks = {}
    opens = closes = ignored_same = opposite_close_only = terminal_closes = 0
    ...
    return marks, {
        ...
        "ignored_same_direction": ignored_same, "opposite_close_only": opposite_close_only,
        "terminal_closes": terminal_closes, "terminal_open_inventory": int(position is not None),
        "sleeve_equity_multiple": wealth,
    }


def simulate_portfolio(corpus, rows, arm, start, end):
...
def run(input_root: Path, source_ledger: Path, output_root: Path) -> dict:
    ...
    payload = {
        "experiment": "FX_AUCTION_TRAP_H384_OPPOSITE_CLOSE_ONLY_V16",
        "evidence_class": "opened_development_not_future_holdout",
        "single_changed_variable": "opposite_signal_close_only_no_same_signal_reopen",
    ...
    result = output_root / "result_opposite_close_only_v16.json"
...
def main() -> int:
    ...
    print(json.dumps({"periods": result["periods"], "development_admitted": result["development_admitted"],
