...
import build_forward_shadow_core_checkpoint_v1 as evidence
...
def test_checkpoint_and_manifest_tamper_fail_closed(tmp_path: Path) -> None:
    ...
    with pytest.raises(shadow.ShadowCoreError, match="CHECKPOINT_MISMATCH"):
...
def test_shared_proposal_stream_cost_arms_latency_idempotence_and_scope(tmp_path: Path) -> None:
    ...
    for row in fills:
        minimum = item.decision_ts_ns + shadow.EXECUTION_SCENARIOS[
            row["cost_arm"]
        ]["latency_ns"]
        assert row["entry_ts_ns"] >= minimum > item.decision_ts_ns
...
def test_runtime_has_file_only_dependency_and_zero_external_capability() -> None:
    tree = ast.parse((ROOT / "forward_shadow_core_v1.py").read_text(encoding="utf-8"))
    imports = set()
    ...
    assert imports <= {
        "__future__", "argparse", "calendar", "csv", "dataclasses", "datetime",
        "decimal", "hashlib", "io", "json", "jpy_accounting_v2", "math", "os",
...
def test_cli_validate_ingest_resume_status_and_finalize(tmp_path: Path) -> None:
    ...
    environment["PYTHONPATH"] = f"{ROOT / 'paper_replay_compat'}:{ROOT}"
    for command in commands:
        completed = subprocess.run(
            [str(PYTHON), str(ROOT / "forward_shadow_core_v1.py"), *command],
            cwd=ROOT,
            env=environment,
            check=True,
            capture_output=True,
