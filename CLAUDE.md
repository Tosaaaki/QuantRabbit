...
**コンテキスト（この会話の全内容）は有限。長くなるほどコスト増・判断力低下・溢れリスク。**

### 共同トレード中のセッション運用
- **1-2時間、または大きな判断の区切りでユーザーに新セッションへの切り替えを提案する**（セッションを開くのはユーザーの操作）
- 提案する前に `collab_trade/state.md` を最新にしておく
- 新セッションで「共同トレード」→ CLAUDE.md + state.md読んで即復帰
- **前のコンテキストの要約は読まなくていい。state.mdの方が正確で速い**

### コンテキスト切れ・新セッションでの復帰手順
1. **`collab_trade/state.md` を即座に読め** — 現在のポジション・テーゼ・確定益が書いてある
2. **`collab_trade/CLAUDE.md` を読め** — 行動規範・手法・ルール
3. **OANDA APIで現在の口座状態を確認**
4. **「待機中」を連呼するな。即座に市場を見に行け**
