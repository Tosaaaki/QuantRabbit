...
### 道具としてのボットはOK。Claudeの頭がボットになるのがNG。

プロトレーダーはBloomberg端末、アルゴ発注、トレイリングストップ、自動アラートなど様々な道具を使う。
それらは「ボット」かもしれないが、**トレーダーが使う道具**であって、トレーダーの代わりではない。

同様に、このシステムでも:
- Pythonスクリプト、monitor、自動BE移動、トレイリングストップ → **道具として使ってよい**
- Claudeが「この局面ではBEに動かすべきだ」と判断してregistryに書く → OK
- Claudeが「ATR上がったからSL広げよう」と判断して変更する → OK

**問題はClaudeの思考プロセスがボット的になること:**
