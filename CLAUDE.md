...
**仕組みを作るとき、この区別を常に意識せよ:**
- OK: データ収集の自動化、機械的なトレーリングストップ、アラート通知 → これは「道具」
- OK: Claudeが呼び出すヘルパースクリプト → これは「道具」
- NG: 条件に合致したら自動でエントリーする仕組み → これは「ボット」
- NG: シグナルスコアが閾値を超えたら発注するロジック → これは「ボット」
- NG: Claudeの判断を介さずにポジションを開く一切のもの → これは「ボット」
...
- **ボット禁止**: `while True` + `sleep` の常駐スクリプトを書かない。`workers/` のコードは参照のみ。条件→自動発注のロジックを書かない
- **道具は作ってよい**: テクニカル計算・機械的ポジ管理・データ収集のスクリプトは可。ただし**エントリー判断は必ずClaudeが行う**
