...
def _diff_roots(
    before: dict, after: dict, roots: tuple[str, ...] = ("exit", "strategies", "alloc")
) -> list[dict]:
    b: dict[str, Any] = {}
    a: dict[str, Any] = {}
    for r in roots:
...
class OnlineTuner:
    ...
    def __post_init__(self) -> None:
        ...
        self.min_micro_trades = _env_int(
            "TUNER_MIN_MICRO_TRADES", self.min_micro_trades
        )
        self.rollback_after_bad_runs = _env_int(
            "TUNER_ROLLBACK_AFTER_BAD_RUNS", self.rollback_after_bad_runs
        )
        self.rollback_cooldown_min = _env_int(
            "TUNER_ROLLBACK_COOLDOWN_MIN", self.rollback_cooldown_min
        )

        self.min_reason_nonempty_pct = _env_float(
            "TUNER_MIN_REASON_NONEMPTY_PCT", self.min_reason_nonempty_pct
        )
        self.min_target_regime_pct = _env_float(
            "TUNER_MIN_TARGET_REGIME_PCT", self.min_target_regime_pct
        )
        ...
        self.good_win_rate_min = _env_float(
            "TUNER_GOOD_WIN_RATE_MIN", self.good_win_rate_min
        )
        self.bad_ev_max = _env_float("TUNER_BAD_EV_MAX", self.bad_ev_max)
        self.bad_pf_max = _env_float("TUNER_BAD_PF_MAX", self.bad_pf_max)
        self.bad_win_rate_max = _env_float(
            "TUNER_BAD_WIN_RATE_MAX", self.bad_win_rate_max
        )
    ...
    def _normalize_columns(self, df: "pd.DataFrame", source: str) -> "pd.DataFrame":
        ...
        if "timestamp" not in df.columns:
            logging.warning(
                "[tuner] missing timestamp column in %s, filling now()", source
            )
    ...
    def compute_metrics(self, df) -> Dict[str, Any]:
        ...
        if "pips" in df.columns:
            ...
            if len(p_nonnull) > 0:
                ...
                m["pips_std"] = (
                    float(p_nonnull.std(ddof=0)) if len(p_nonnull) > 1 else 0.0
                )
        ...
        if {"strategy", "reason"}.issubset(df.columns):
            tp = (
                df[df["reason"].isin(["tp", "tp_hit", "soft_tp_timeout"])]
                .groupby("strategy")
                .size()
            )
    ...
    def _quality_gate(
        self, df: Optional["pd.DataFrame"], metrics: Dict[str, Any]
    ...
    def _should_rollback(
        self, state: dict, metrics: Dict[str, Any]
    ) -> tuple[bool, list[str]]:
    ...
    def _update_lkg_if_good(
        self, state: dict, metrics: Dict[str, Any], current_overrides: dict
    ) -> bool:
    ...
    def propose_patch(
        self, metrics: Dict[str, Any], preset: Dict[str, Any]
    ) -> Dict[str, Any]:
        patch = {
            "meta": {
        ...
        if hard_timeout > 0.25:
            ub = (
                preset.get("exit", {}).get("lowvol", {}).get("upper_bound_max_sec", 3.6)
            )
            ub = max(self.bounds.upper_bound_max_sec[0], ub - 0.2)
            patch.setdefault("exit", {}).setdefault("lowvol", {})[
                "upper_bound_max_sec"
            ] = round(min(ub, self.bounds.upper_bound_max_sec[1]), 2)
            ...
            patch.setdefault("exit", {}).setdefault("lowvol", {})[
                "upper_bound_max_sec"
            ] = round(max(ub, self.bounds.upper_bound_max_sec[0]), 2)

        hazard_exit = reasons_pct.get("hazard_exit", 0.0)
        if hazard_exit == 0.0 and hazard_ticks_sum > 20:
            # make hazard more sensitive a bit
            hb = (
                preset.get("exit", {}).get("lowvol", {}).get("hazard_debounce_ticks", 2)
            )
            hb = max(self.bounds.hazard_debounce_ticks[0], hb - 1)
            patch.setdefault("exit", {}).setdefault("lowvol", {})[
                "hazard_debounce_ticks"
            ] = int(min(hb, self.bounds.hazard_debounce_ticks[1]))
            csb = (
                preset.get("exit", {})
                .get("lowvol", {})
                .get("hazard_cost_spread_base", 0.25)
                - 0.02
            )
            clb = (
                preset.get("exit", {})
                .get("lowvol", {})
                .get("hazard_cost_latency_base_ms", 240)
                - 20
            )
            patch["exit"]["lowvol"]["hazard_cost_spread_base"] = round(
                _clamp(csb, *self.bounds.hazard_cost_spread_base), 3
            )
            patch["exit"]["lowvol"]["hazard_cost_latency_base_ms"] = int(
                _clamp(clb, *self.bounds.hazard_cost_latency_base_ms)
            )
            ...
            patch.setdefault("exit", {}).setdefault("lowvol", {})[
                "hazard_debounce_ticks"
            ] = int(max(hb, self.bounds.hazard_debounce_ticks[0]))
            csb = (
                preset.get("exit", {})
                .get("lowvol", {})
                .get("hazard_cost_spread_base", 0.25)
        ...
        if scratch > 0.55:
            mg = (
                preset.get("exit", {})
                .get("lowvol", {})
                .get("min_grace_before_scratch_ms", 120)
                + 30
            )
            sr = (
                preset.get("exit", {})
                .get("lowvol", {})
                .get("scratch_requires_events", 1)
            )
            patch.setdefault("exit", {}).setdefault("lowvol", {})[
                "min_grace_before_scratch_ms"
            ] = int(_clamp(mg, *self.bounds.min_grace_before_scratch_ms))
            patch["exit"]["lowvol"]["scratch_requires_events"] = int(
                _clamp(sr + 1, *self.bounds.scratch_requires_events)
            )
        elif scratch < 0.15:
            mg = (
                preset.get("exit", {})
            ...
            patch["exit"]["lowvol"]["scratch_requires_events"] = int(
                _clamp(
                    max(sr - 1, self.bounds.scratch_requires_events[0]),
                    *self.bounds.scratch_requires_events,
                )
            )
        ...
        if ev < 0.05:
            ...
            rb = (
                preset.get("strategies", {})
                .get("BB_RSI_Fast", {})
                .get("reentry_block_s", 15)
                + 2
            )
            patch.setdefault("strategies", {}).setdefault("MomentumPulse", {})[
                "min_confidence"
            ] = round(_clamp(mp, *self.bounds.momentum_conf_min), 2)
            patch["strategies"].setdefault("MicroVWAPRevert", {})["vwap_z_min"] = round(
                _clamp(zv, *self.bounds.microvwap_z_min), 2
            )
            patch["strategies"].setdefault("VolCompressionBreak", {})[
                "accel_pctile"
            ] = int(_clamp(ap, *self.bounds.volcomp_accel_pctile))
            patch["strategies"].setdefault("BB_RSI_Fast", {})["reentry_block_s"] = int(
                _clamp(rb, *self.bounds.bb_rsi_reentry_block_s)
            )
        elif ev > 0.12:
        ...
        base_share = (
            ...
            .get("regime", {})
            .get("quiet_low_vol", {})
            .get("micro_share", 0.35)
        )
        if micro_tp_ok > 0.65:
            micro_share = base_share + 0.05
        else:
            micro_share = base_share - 0.05
        patch.setdefault("alloc", {}).setdefault("regime", {}).setdefault(
            "quiet_low_vol", {}
        )["micro_share"] = round(_clamp(micro_share, *self.bounds.micro_share), 2)
    ...
    def run_once(self, logs_glob: str, shadow: bool = False) -> str:
        ...
        summary = {
            ...
            "reason_nonempty_pct": float(
                quality.get("reason_nonempty_pct", 1.0) or 0.0
            ),
        ...
        if shadow:
            ...
            if not ok:
                patch = {
                    "meta": {"generated_at": _utc_iso(), "window_min": self.minutes}
                }
                ...
                lkg_updated = self._update_lkg_if_good(
                    state, metrics, current_overrides
                )
                ...
                if do_rb:
                    ...
                    patch["meta"]["decision"] = (
                        "rollback" if rolled else "rollback_failed"
                    )
                    ...
                    if is_bad:
                        ...
                        if (
                            int(quality.get("micro_trades", 0) or 0)
                            < self.min_micro_trades
                        ):
                            if "alloc" in patch:
                                try:
                                    del patch["alloc"]
                        ...
                        decision = {
                            "decision": "apply",
                            "lkg_updated": lkg_updated,
                            "alloc_skipped": patch.get("meta", {}).get(
                                "alloc_skipped", False
                            ),
