...
class MicroTrendRetestExitWorker:
    """
    MicroTrendRetest 専用 EXIT。
    - hard_stop: entry_thesis.hard_stop_pips (fallback: sl_pips) を越えたら損切り
    - trend_failure: MA10/MA20 反転 or MA20割れ（buffer）で早期撤退
    - time_stop: 最大保有を超えて含み益化しなければ撤退
    - win side: TP / トレール / lock-floor / RSI利確
    """
    ...
    async def _review_trade(self, trade: dict, now: datetime, mid: float) -> None:
        ...
        # 2) Trend/retest failure: MA flip or price crossing MA20 with buffer
        ...
        # 3) Time stop: if it doesn't turn profitable in time, exit.
        if hold_sec >= self.max_hold_sec and pnl <= 0.0:
            # Use EXIT_ALLOW_NEGATIVE_REASONS-compatible token (EXIT_ALLOW_NEGATIVE_BY_WORKER may be disabled).
            await self._close(trade_id, -units, "trend_exhaust", pnl, client_id, allow_negative=True)
            self._states.pop(trade_id, None)
            return

        # --- Win-side management (pnl > 0) ---
