"""Micro Pullback-Fib worker (trend pullback)."""
...
def _trend_pullback_signal(fac_m5: Dict[str, object], price: float) -> Optional[Tuple[str, float, Dict[str, float]]]:
    ma10 = _float(fac_m5.get("ma10"))
    ma20 = _float(fac_m5.get("ma20"))
    macd_hist = _float(fac_m5.get("macd_hist")) or 0.0
    adx = _float(fac_m5.get("adx")) or 0.0
    rsi = _float(fac_m5.get("rsi")) or 50.0

    if ma10 is None or ma20 is None or ma10 == ma20:
        return None
    if adx < config.TREND_ADX_MIN:
        return None

    if ma10 > ma20 and macd_hist >= 0:
        side = "long"
    elif ma10 < ma20 and macd_hist <= 0:
        side = "short"
    else:
        return None

    buffer = config.PULLBACK_MA_BUFFER_PIPS * PIP
    if side == "long":
        if price > ma10 - buffer:
            return None
        if not (config.PULLBACK_RSI_MIN <= rsi <= config.PULLBACK_RSI_MAX):
            return None
    else:
        if price < ma10 + buffer:
            return None
        rsi_low = 100.0 - config.PULLBACK_RSI_MAX
        rsi_high = 100.0 - config.PULLBACK_RSI_MIN
        if not (rsi_low <= rsi <= rsi_high):
            return None

    score = 0.55
    if side == "long" and rsi <= config.PULLBACK_RSI_MIN + 2:
        score += 0.1
    if side == "short" and rsi >= 100.0 - (config.PULLBACK_RSI_MIN + 2):
        score += 0.1
    if adx >= config.TREND_ADX_MIN + 4:
        score += 0.1
    if score < config.TREND_SCORE_MIN:
        return None

    base_conf = 44.0 + score * 35.0
...
async def micro_pullback_fib_worker() -> None:
    ...
    while True:
        ...
        range_ctx = detect_range_mode(fac_m1, factors.get("H4") or {})
        price = _float(fac_m1.get("close")) or _float(fac_m5.get("close")) or 0.0
        price = _latest_mid(price)
        if price <= 0:
            continue

        signal = _trend_pullback_signal(fac_m5, price)
        if not signal:
            continue
        ...
        signal_tag = f"{config.STRATEGY_TAG}-pullback"
        tech_thesis = {
            ...
            "tech_policy": {
                ...
                "nwave_min_leg_pips": 3.5,
                "fib_trigger": 0.382,
                "mid_distance_pips": 2.8,
            },
            "env_tf": "H1",
            "struct_tf": "M5",
            "entry_tf": "M5",
...
if __name__ == "__main__":
    asyncio.run(micro_pullback_fib_worker())
