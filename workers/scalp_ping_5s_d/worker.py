"""Experimental 5-second ping scalp worker (D版).

この実装は既存 ``workers.scalp_ping_5s`` を再利用しつつ、環境変数プレフィックスを
``SCALP_PING_5S_D_*`` に切り替えて独立した戦略タグで運用するための薄いラッパです。
"""
...
def _apply_alt_env(prefix: str, *, fallback_tag: str, fallback_log_prefix: str) -> None:
    ...
    # D is a clone of SCALP_PING_5S. Keep only the D-prefixed overrides and
    # remove non-D SCALP_PING_5S_* variables so stale A-layer values are not mixed.
    # Keep D variables intact during cleanup so they can be copied down below.
    ...
    # Capture all variables after cleanup before mutating so we don't lose D keys
    ...
    # Keep env-prefix identity aligned with D strategy so downstream guards use D
    # namespace settings (e.g., SCALP_PING_5S_D_PERF_GUARD_* / D-specific
    # tunables).
    ...
    # Safety baseline for D variant: keep entry protection on by default.
    ...
    if not allow_unprotected:
        if not _env_truthy(mapped_use_sl_key, False):
            logger.warning(
                "[SCALP5S_D] forcing %s=1 (set %s_ALLOW_UNPROTECTED_ENTRY=1 to bypass)",
                mapped_use_sl_key,
                prefix,
            )
        ...
        if _env_truthy(mapped_disable_hard_stop_key, False):
            logger.warning(
                "[SCALP5S_D] forcing %s=0 (set %s_ALLOW_UNPROTECTED_ENTRY=1 to bypass)",
    ...
    # used by entry logic so missing/shifted D-side toggles are visible in startup
    ...
    logger.info(
        "[SCALP5S_D] env mapped: source=%s mapped_prefix=%s enabled=%s revert_enabled=%s env_prefix=%s strategy=%s log_prefix=%s",
        prefix,
        base_prefix,
        base_enabled,
    ...
    if str(base_env_prefix).strip().upper() != prefix:
        logger.warning(
            "[SCALP5S_D] SCALP_PING_5S_ENV_PREFIX=%s but expected=%s",
            base_env_prefix or "(unset)",
            prefix,
        )
    if str(mapped_revert_enabled).strip().lower() in {"0", "false", "no", "off"}:
        logger.warning(
            "[SCALP5S_D] SCALP_PING_5S_REVERT_ENABLED is OFF. "
