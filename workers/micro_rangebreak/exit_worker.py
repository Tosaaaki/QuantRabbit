"""Exit loop for MicroRangeBreak (micro pocket) – プラス決済専用。"""
...
ALLOWED_TAGS: Set[str] = {"MicroRangeBreak"}
POCKET = "micro"
...
class MicroRangeBreakExitWorker:
    """
    MicroRangeBreak 専用 EXIT。
    - 最低保有 20s までクローズ禁止
    - PnL>0 のみクローズ（TP/トレール/RSI利確/レンジ長時間微益）
    """

    def __init__(self) -> None:
        self.loop_interval = max(0.5, _float_env("MICRO_RB_EXIT_LOOP_INTERVAL_SEC", 1.0))
        self._pos_manager = PositionManager()
        self._states: Dict[str, _TradeState] = {}

        self.profit_take = max(1.3, _float_env("MICRO_RB_EXIT_PROFIT_PIPS", 2.0))
        self.trail_start = max(1.5, _float_env("MICRO_RB_EXIT_TRAIL_START_PIPS", 2.6))
        self.trail_backoff = max(0.25, _float_env("MICRO_RB_EXIT_TRAIL_BACKOFF_PIPS", 0.8))
        self.lock_buffer = max(0.2, _float_env("MICRO_RB_EXIT_LOCK_BUFFER_PIPS", 0.5))
        self.lock_trigger = max(0.6, _float_env("MICRO_RB_EXIT_LOCK_TRIGGER_PIPS", self.profit_take * 0.35))
        self.min_hold_sec = max(5.0, _float_env("MICRO_RB_EXIT_MIN_HOLD_SEC", 20.0))

        self.range_profit_take = max(1.0, _float_env("MICRO_RB_EXIT_RANGE_PROFIT_PIPS", 1.6))
        self.range_trail_start = max(1.2, _float_env("MICRO_RB_EXIT_RANGE_TRAIL_START_PIPS", 2.1))
        self.range_trail_backoff = max(0.2, _float_env("MICRO_RB_EXIT_RANGE_TRAIL_BACKOFF_PIPS", 0.65))
        self.range_lock_buffer = max(0.15, _float_env("MICRO_RB_EXIT_RANGE_LOCK_BUFFER_PIPS", 0.35))
        self.range_lock_trigger = max(
            0.5, _float_env("MICRO_RB_EXIT_RANGE_LOCK_TRIGGER_PIPS", self.range_profit_take * 0.35)
        )
        self.range_max_hold_sec = max(90.0, _float_env("MICRO_RB_EXIT_RANGE_MAX_HOLD_SEC", 30 * 60))
        ...
        self.tech_take_enabled = _bool_env("MICRO_RB_EXIT_TECH_TAKE_ENABLED", True)
        self.tech_take_min_pips = max(
            0.2, _float_env("MICRO_RB_EXIT_TECH_TAKE_MIN_PIPS", self.profit_take * 0.5)
        )
    ...
    async def _close(
        ...
        if ok:
            LOG.info("[EXIT-micro_rangebreak] trade=%s units=%s reason=%s pnl=%.2fp", trade_id, units, reason, pnl)
        else:
            LOG.error("[EXIT-micro_rangebreak] close failed trade=%s units=%s reason=%s", trade_id, units, reason)
    ...
    async def _review_trade(self, trade: dict, now: datetime, mid: float, rsi: Optional[float], range_active: bool) -> None:
        ...
        if not client_id:
            LOG.warning("[EXIT-micro_rangebreak] missing client_id trade=%s skip close", trade_id)
            return
        ...
        if section_decision.should_exit and section_decision.reason:
            LOG.info(
                "[EXIT-micro_rangebreak] section_exit trade=%s reason=%s detail=%s",
                trade_id,
    ...
    async def run(self) -> None:
        LOG.info(
            "[EXIT-micro_rangebreak] worker starting (interval=%.2fs tags=%s)",
            self.loop_interval,
            ...
            while True:
                ...
                for tr in trades:
                    try:
                        ...
                        LOG.exception("[EXIT-micro_rangebreak] review failed trade=%s", tr.get("trade_id"))
                        continue
        except asyncio.CancelledError:
            LOG.info("[EXIT-micro_rangebreak] worker cancelled")
            raise
        finally:
            try:
                self._pos_manager.close()
            except Exception:
                LOG.exception("[EXIT-micro_rangebreak] failed to close PositionManager")


async def micro_rangebreak_exit_worker() -> None:
    worker = MicroRangeBreakExitWorker()
    await worker.run()


if __name__ == "__main__":  # pragma: no cover
    logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s", force=True)
    asyncio.run(micro_rangebreak_exit_worker())
