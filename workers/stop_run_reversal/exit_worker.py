...
ALLOWED_TAGS: Set[str] = {'stop_run_reversal'}
...
POCKET = _pocket_env("STOP_RUN_REVERSAL_POCKET", "micro")
ALLOWED_TAGS = _tags_env("STOP_RUN_REVERSAL_EXIT_TAGS", ALLOWED_TAGS)
...
class StopRunReversalExitWorker:
    def __init__(self) -> None:
        self.loop_interval = max(
            0.5,
            _float_env("STOP_RUN_REVERSAL_EXIT_LOOP_INTERVAL_SEC", 1.0),
        )
        self.min_hold_sec = max(
            5.0,
            _float_env("STOP_RUN_REVERSAL_EXIT_MIN_HOLD_SEC", 25.0),
        )
        self.profit_take = max(
            0.5,
            _float_env("STOP_RUN_REVERSAL_EXIT_PROFIT_PIPS", 2.0),
        )
        self.trail_start = max(
            0.5,
            _float_env("STOP_RUN_REVERSAL_EXIT_TRAIL_START_PIPS", 2.6),
        )
        self.trail_backoff = max(
            0.1,
            _float_env("STOP_RUN_REVERSAL_EXIT_TRAIL_BACKOFF_PIPS", 0.8),
        )
        self.lock_buffer = max(
            0.05,
            _float_env("STOP_RUN_REVERSAL_EXIT_LOCK_BUFFER_PIPS", 0.5),
        )
        self.range_profit_take = max(
            0.4,
            _float_env("STOP_RUN_REVERSAL_EXIT_RANGE_PROFIT_PIPS", 1.6),
        )
        self.range_trail_start = max(
            0.4,
            _float_env("STOP_RUN_REVERSAL_EXIT_RANGE_TRAIL_START_PIPS", 2.0),
        )
        self.range_trail_backoff = max(
            0.1,
            _float_env("STOP_RUN_REVERSAL_EXIT_RANGE_TRAIL_BACKOFF_PIPS", 0.6),
        )
        self.range_lock_buffer = max(
            0.05,
            _float_env("STOP_RUN_REVERSAL_EXIT_RANGE_LOCK_BUFFER_PIPS", 0.35),
        )
        self.range_max_hold_sec = max(
            60.0,
            _float_env("STOP_RUN_REVERSAL_EXIT_RANGE_MAX_HOLD_SEC", 2400.0),
        )
    ...
    async def _close(self, trade_id: str, units: int, reason: str, pnl: float, client_order_id: Optional[str]) -> None:
        ...
        if ok:
            LOG.info("[exit-stop-run] trade=%s units=%s reason=%s pnl=%.2fp", trade_id, units, reason, pnl)
        else:
            LOG.error("[exit-stop-run] close failed trade=%s units=%s reason=%s", trade_id, units, reason)
    ...
    async def _review_trade(self, trade: dict, now: datetime, mid: float, range_active: bool) -> None:
        ...
        if not client_id:
            LOG.warning("[exit-stop-run] missing client_id trade=%s skip close", trade_id)
            return
    ...
    async def run(self) -> None:
        LOG.info(
            "[exit-stop-run] exit worker start interval=%.2fs tags=%s pocket=%s",
            self.loop_interval,
            ",".join(sorted(ALLOWED_TAGS)) if ALLOWED_TAGS else "none",
            POCKET,
        )
        if not ALLOWED_TAGS:
            LOG.info("[exit-stop-run] no allowed tags configured; idle")
            try:
                while True:
            ...
            while True:
                ...
                for tr in trades:
                    try:
                        ...
                        LOG.exception("[exit-stop-run] review failed trade=%s", tr.get("trade_id"))
        except asyncio.CancelledError:
            LOG.info("[exit-stop-run] worker cancelled")
            raise
        finally:
            try:
                self._pos_manager.close()
            except Exception:
                LOG.exception("[exit-stop-run] failed to close PositionManager")


async def stop_run_reversal_exit_worker() -> None:
    worker = StopRunReversalExitWorker()
    await worker.run()
...
if __name__ == "__main__":
    asyncio.run(stop_run_reversal_exit_worker())
