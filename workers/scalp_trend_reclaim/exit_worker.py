"""Exit worker for TrendReclaimLong (scalp pocket)."""
...
class TrendReclaimLongExitWorker:
    def __init__(self) -> None:
        self.loop_interval = max(0.4, float(config.EXIT_LOOP_INTERVAL_SEC))
        self.min_hold_sec = max(5.0, float(config.EXIT_MIN_HOLD_SEC))
        self.max_hold_sec = max(self.min_hold_sec, float(config.EXIT_MAX_HOLD_SEC))
        self.profit_take = max(0.8, float(config.EXIT_PROFIT_PIPS))
        self.trail_start = max(1.0, float(config.EXIT_TRAIL_START_PIPS))
        self.trail_backoff = max(0.2, float(config.EXIT_TRAIL_BACKOFF_PIPS))
        self.lock_buffer = max(0.2, float(config.EXIT_LOCK_BUFFER_PIPS))
        self.lock_trigger = max(0.5, float(config.EXIT_LOCK_TRIGGER_PIPS))
        self.rev_window_sec = max(2.0, float(config.EXIT_REVERSAL_WINDOW_SEC))
        self.rev_pips = max(0.5, float(config.EXIT_REVERSAL_PIPS))
        self.atr_spike_pips = max(1.0, float(config.EXIT_ATR_SPIKE_PIPS))
        self.hard_stop_pips = max(1.0, float(config.EXIT_HARD_STOP_PIPS))
        self.tp_hint_ratio = max(0.2, float(config.EXIT_TP_HINT_RATIO))
    ...
    async def _close(
        ...
        allow_negative = pnl <= 0 or reason in {"atr_spike", "hazard_exit", "reentry_reset"}
        ...
        if ok:
            LOG.info(
                "[EXIT-trend_reclaim] trade=%s units=%s reason=%s pnl=%.2fp",
                trade_id,
                units,
                reason,
                pnl,
            ...
            LOG.error("[EXIT-trend_reclaim] close failed trade=%s units=%s reason=%s", trade_id, units, reason)
    ...
    async def run(self) -> None:
        LOG.info("[EXIT-trend_reclaim] worker starting (interval=%.2fs)", self.loop_interval)
        while True:
            ...
            for tr in trades:
                ...
                try:
                    ...
                    LOG.exception("[EXIT-trend_reclaim] review failed")


async def scalp_trend_reclaim_exit_worker() -> None:
    worker = TrendReclaimLongExitWorker()
    try:
        await worker.run()
    except asyncio.CancelledError:
        LOG.info("[EXIT-trend_reclaim] worker cancelled")
        raise
    finally:
        try:
            worker._pos_manager.close()
        except Exception:
            LOG.exception("[EXIT-trend_reclaim] failed to close PositionManager")


if __name__ == "__main__":
    asyncio.run(scalp_trend_reclaim_exit_worker())
