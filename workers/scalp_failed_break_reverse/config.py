...
BB_ENTRY_MIN_CONF_FOR_SOFT_FAIL = int(
    os.getenv("M1SCALP_BB_ENTRY_MIN_CONF_FOR_SOFT_FAIL", "66")
)
...
BLOCK_HOURS_UTC = frozenset(_parse_hours(os.getenv("M1SCALP_BLOCK_HOURS_UTC", "")))
BLOCK_HOURS_ENABLED = os.getenv(
...
ALLOW_REVERSION = _env_bool("M1SCALP_ALLOW_REVERSION", True)
ALLOW_TREND = _env_bool("M1SCALP_ALLOW_TREND", False)
...
SIGNAL_TAG_CONTAINS = frozenset(
    _parse_csv_lower(
        ...
        or _load_env_file().get("M1SCALP_SIGNAL_TAG_CONTAINS", "vshape-rebound")
    )
)
...
USDJPY_QUICKSHOT_EST_COST_JPY = _env_float(
    "M1SCALP_USDJPY_QUICKSHOT_EST_COST_JPY", 12.0
)
USDJPY_QUICKSHOT_MAX_SPREAD_PIPS = _env_float(
    "M1SCALP_USDJPY_QUICKSHOT_MAX_SPREAD_PIPS", 0.30
)
USDJPY_QUICKSHOT_BREAK_LOOKBACK_M5 = int(
    _env_float("M1SCALP_USDJPY_QUICKSHOT_BREAK_LOOKBACK_M5", 12)
)
USDJPY_QUICKSHOT_BREAK_MARGIN_PIPS = _env_float(
    "M1SCALP_USDJPY_QUICKSHOT_BREAK_MARGIN_PIPS", 0.25
)
USDJPY_QUICKSHOT_PULLBACK_BAND_PIPS = _env_float(
    "M1SCALP_USDJPY_QUICKSHOT_PULLBACK_BAND_PIPS", 2.2
)
USDJPY_QUICKSHOT_RETRACE_MIN_PIPS = _env_float(
    "M1SCALP_USDJPY_QUICKSHOT_RETRACE_MIN_PIPS", 0.35
)
...
USDJPY_QUICKSHOT_MIN_ENTRY_PROBABILITY = _env_float(
    "M1SCALP_USDJPY_QUICKSHOT_MIN_ENTRY_PROBABILITY", 0.55
)
...
SIDE_FILTER = _normalize_side(
    os.getenv("M1SCALP_SIDE_FILTER") or _load_env_file().get("M1SCALP_SIDE_FILTER", "")
)
# Optional strategy tag override for downstream tracking.
STRATEGY_TAG_OVERRIDE = (
    os.getenv("M1SCALP_STRATEGY_TAG_OVERRIDE")
    or _load_env_file().get("M1SCALP_STRATEGY_TAG_OVERRIDE", "FailedBreakReverse")
).strip()

# Reversion safety gate: even if ALLOW_REVERSION=1, require robust range context by default.
REVERSION_REQUIRE_STRONG_RANGE = _env_bool(
    "M1SCALP_REVERSION_REQUIRE_STRONG_RANGE", True
)
REVERSION_MIN_RANGE_SCORE = float(
    os.getenv("M1SCALP_REVERSION_MIN_RANGE_SCORE", "0.68")
)
