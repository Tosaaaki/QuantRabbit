"""Experimental 5-second ping scalp worker (C版).

この実装は既存 ``workers.scalp_ping_5s`` を再利用しつつ、環境変数プレフィックスを
``SCALP_PING_5S_C_*`` に切り替えて独立した戦略タグで運用するための薄いラッパです。
"""
...
def _apply_alt_env(prefix: str, *, fallback_tag: str, fallback_log_prefix: str) -> None:
    ...
    # C is a clone of SCALP_PING_5S. Keep only the C-prefixed overrides and
    ...
    # Keep env-prefix identity aligned with C strategy so downstream guards use C
    # namespace settings (e.g., SCALP_PING_5S_C_PERF_GUARD_* / C-specific
    # tunables).
    ...
    # Fail closed for C variant direction guard:
    ...
    if normalized_side_filter is None:
        ...
        if normalized_side_filter == "":
            logger.warning(
                "[SCALP5S_C] allowing %s to be empty (%s_ALLOW_NO_SIDE_FILTER=1)",
    ...
    if not allow_unprotected:
        if not _env_truthy(mapped_use_sl_key, False):
            logger.warning(
                "[SCALP5S_C] forcing %s=1 (set %s_ALLOW_UNPROTECTED_ENTRY=1 to bypass)",
                mapped_use_sl_key,
                prefix,
            )
        ...
        if _env_truthy(mapped_disable_hard_stop_key, False):
            logger.warning(
                "[SCALP5S_C] forcing %s=0 (set %s_ALLOW_UNPROTECTED_ENTRY=1 to bypass)",
    ...
    # used by entry logic so missing/shifted C-side toggles are visible in startup
    ...
    logger.info(
        "[SCALP5S_C] env mapped: source=%s mapped_prefix=%s enabled=%s revert_enabled=%s env_prefix=%s strategy=%s log_prefix=%s side_filter=%s",
        prefix,
        base_prefix,
        base_enabled,
    ...
    if str(base_env_prefix).strip().upper() != prefix:
        logger.warning(
            "[SCALP5S_C] SCALP_PING_5S_ENV_PREFIX=%s but expected=%s",
            base_env_prefix or "(unset)",
            prefix,
        )
    if str(mapped_revert_enabled).strip().lower() in {"0", "false", "no", "off"}:
        logger.warning(
            "[SCALP5S_C] SCALP_PING_5S_REVERT_ENABLED is OFF. "
