...
POCKET = _pocket_env("SESSION_OPEN_POCKET", "micro")
ALLOWED_TAGS = _tags_env("SESSION_OPEN_EXIT_TAGS", ALLOWED_TAGS)
...
class SessionOpenExitWorker:
    ...
    async def run(self) -> None:
        LOG.info(
            "[exit-session-open] exit worker start interval=%.2fs tags=%s pocket=%s",
            self.loop_interval,
            ",".join(sorted(ALLOWED_TAGS)) if ALLOWED_TAGS else "none",
            POCKET,
        )
        if not ALLOWED_TAGS:
            LOG.info("[exit-session-open] no allowed tags configured; idle")
            try:
                while True:
        ...
        try:
            ...
            try:
                ...
                LOG.exception("[exit-session-open] failed to close PositionManager")


async def session_open_exit_worker() -> None:
    worker = SessionOpenExitWorker()
    await worker.run()
