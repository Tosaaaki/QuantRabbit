"""Dedicated exit loop for the micro pocket.

既存の ExitManager を流用しつつ、micro のテクニカルバイアスと policy_bus を元に
逆方向シグナルを合成し、Exit 判定を独立スレッドで実施する。
"""
...
def _make_signals(fac_m1: dict, pocket_policy: dict | None) -> List[dict]:
    """合成シグナルを作って ExitManager の逆方向判定を動かす。"""
    signals: List[dict] = []
    ...
    try:
        ma10 = float(fac_m1.get("ma10") or fac_m1.get("ema10") or 0.0)
        ma20 = float(fac_m1.get("ma20") or fac_m1.get("ema20") or 0.0)
        rsi = float(fac_m1.get("rsi") or 50.0)
    except Exception:
        ma10 = ma20 = rsi = 0.0
...
async def _execute_exit(
    ...
    if remaining > 0:
        client_id = build_client_order_id("micro_exit", decision.tag)
    ...
    if remaining <= 0:
        ...
        if decision.reason == "reverse_signal":
            ...
            stage_tracker.set_cooldown(
                ...
                opposite,
                reason="flip_guard",
                seconds=min(240, max(60, cooldown_seconds // 2)),
                now=now,
            )


async def micro_exit_worker() -> None:
    ...
    try:
        while True:
            ...
            trades = micro_info.get("open_trades") or []
            if not trades:
                continue
            ...
            range_mode = bool((policy_snapshot.range_mode if policy_snapshot else False) or fac_m1.get("range_active"))

            signals = _make_signals(fac_m1, pocket_policy if isinstance(pocket_policy, dict) else None)
