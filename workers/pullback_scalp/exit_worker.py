"""Exit loop for pullback_scalp worker (scalp pocket) with technical filters."""
...
class PullbackScalpExitWorker:
    """PnL + RSI/ATR/VWAP/レンジ判定を組み合わせた pullback_scalp EXIT."""

    def __init__(self) -> None:
        self.tp_scale = TPScaleConfig()
        self.loop_interval = max(0.3, _float_env("PULLBACK_SCALP_EXIT_LOOP_INTERVAL_SEC", 0.8))
        self._pos_manager = PositionManager()
        self._states: Dict[str, _TradeState] = {}

        self.profit_take = max(1.0, _float_env("PULLBACK_SCALP_EXIT_PROFIT_PIPS", 1.8))
        self.trail_start = max(1.0, _float_env("PULLBACK_SCALP_EXIT_TRAIL_START_PIPS", 2.4))
        self.trail_backoff = max(0.2, _float_env("PULLBACK_SCALP_EXIT_TRAIL_BACKOFF_PIPS", 0.8))
        self.stop_loss = max(0.6, _float_env("PULLBACK_SCALP_EXIT_STOP_LOSS_PIPS", 1.2))
        self.max_hold_sec = max(30.0, _float_env("PULLBACK_SCALP_EXIT_MAX_HOLD_SEC", 18 * 60))
        self.lock_trigger = max(0.4, _float_env("PULLBACK_SCALP_EXIT_LOCK_TRIGGER_PIPS", 0.8))
        self.lock_buffer = max(0.1, _float_env("PULLBACK_SCALP_EXIT_LOCK_BUFFER_PIPS", 0.4))

        self.range_profit_take = max(0.6, _float_env("PULLBACK_SCALP_EXIT_RANGE_PROFIT_PIPS", 1.5))
        self.range_trail_start = max(0.9, _float_env("PULLBACK_SCALP_EXIT_RANGE_TRAIL_START_PIPS", 1.9))
        self.range_trail_backoff = max(0.2, _float_env("PULLBACK_SCALP_EXIT_RANGE_TRAIL_BACKOFF_PIPS", 0.6))
        self.range_stop_loss = max(0.5, _float_env("PULLBACK_SCALP_EXIT_RANGE_STOP_LOSS_PIPS", 1.0))
        self.range_max_hold_sec = max(30.0, _float_env("PULLBACK_SCALP_EXIT_RANGE_MAX_HOLD_SEC", 14 * 60))
        self.range_lock_trigger = max(0.3, _float_env("PULLBACK_SCALP_EXIT_RANGE_LOCK_TRIGGER_PIPS", 0.7))
        self.range_lock_buffer = max(0.1, _float_env("PULLBACK_SCALP_EXIT_RANGE_LOCK_BUFFER_PIPS", 0.3))

        self.range_adx = max(5.0, _float_env("PULLBACK_SCALP_EXIT_RANGE_ADX", 22.0))
        self.range_bbw = max(0.02, _float_env("PULLBACK_SCALP_EXIT_RANGE_BBW", 0.20))
        self.range_atr = max(0.4, _float_env("PULLBACK_SCALP_EXIT_RANGE_ATR", 6.0))

        self.rsi_fade_long = _float_env("PULLBACK_SCALP_EXIT_RSI_FADE_LONG", 42.0)
        self.rsi_fade_short = _float_env("PULLBACK_SCALP_EXIT_RSI_FADE_SHORT", 58.0)
        self.rsi_take_long = _float_env("PULLBACK_SCALP_EXIT_RSI_TAKE_LONG", 69.0)
        self.rsi_take_short = _float_env("PULLBACK_SCALP_EXIT_RSI_TAKE_SHORT", 31.0)
        self.negative_hold_sec = max(15.0, _float_env("PULLBACK_SCALP_EXIT_NEG_HOLD_SEC", 120.0))
        self.allow_negative_exit = _bool_env("PULLBACK_SCALP_EXIT_ALLOW_NEGATIVE", True)

        self.vwap_grab_gap = max(0.15, _float_env("PULLBACK_SCALP_EXIT_VWAP_GAP_PIPS", 0.8))

        self.atr_hot = max(0.5, _float_env("PULLBACK_SCALP_EXIT_ATR_HOT_PIPS", 5.0))
        self.atr_cold = max(0.3, _float_env("PULLBACK_SCALP_EXIT_ATR_COLD_PIPS", 1.1))

        self.touch_enabled = _bool_env("PULLBACK_SCALP_EXIT_TOUCH_ENABLED", True)
        self.touch_window_sec = max(20.0, _float_env("PULLBACK_SCALP_EXIT_TOUCH_WINDOW_SEC", 150.0))
        self.touch_min_ticks = max(20, int(_float_env("PULLBACK_SCALP_EXIT_TOUCH_MIN_TICKS", 40)))
        self.touch_pullback_atr_mult = max(
            0.1, _float_env("PULLBACK_SCALP_EXIT_TOUCH_PULLBACK_ATR_MULT", 0.75)
        )
        self.touch_pullback_min_pips = max(
            0.1, _float_env("PULLBACK_SCALP_EXIT_TOUCH_PULLBACK_MIN_PIPS", 0.5)
        )
        self.touch_pullback_max_pips = max(
            self.touch_pullback_min_pips,
            _float_env("PULLBACK_SCALP_EXIT_TOUCH_PULLBACK_MAX_PIPS", 2.0),
        )
        self.touch_trend_atr_mult = max(
            0.1, _float_env("PULLBACK_SCALP_EXIT_TOUCH_TREND_ATR_MULT", 1.4)
        )
        self.touch_trend_min_pips = max(
            0.2, _float_env("PULLBACK_SCALP_EXIT_TOUCH_TREND_MIN_PIPS", 1.0)
        )
        self.touch_trend_max_pips = max(
            self.touch_trend_min_pips,
            _float_env("PULLBACK_SCALP_EXIT_TOUCH_TREND_MAX_PIPS", 4.2),
        )
        self.touch_reset_ratio = max(
            0.1, min(0.9, _float_env("PULLBACK_SCALP_EXIT_TOUCH_RESET_RATIO", 0.45)))
        self.touch_tighten_count = max(
            1, int(_float_env("PULLBACK_SCALP_EXIT_TOUCH_TIGHTEN_COUNT", 3))
        )
        self.touch_tighten_ratio = max(
            0.5, min(1.0, _float_env("PULLBACK_SCALP_EXIT_TOUCH_TIGHTEN_RATIO", 0.85))
        )
        self.touch_force_count = max(
            self.touch_tighten_count + 1,
            int(_float_env("PULLBACK_SCALP_EXIT_TOUCH_FORCE_COUNT", 5)),
        )
        self.touch_force_min_pips = max(
            0.1, _float_env("PULLBACK_SCALP_EXIT_TOUCH_FORCE_MIN_PIPS", 0.5)
        )
    ...
    async def _close(
        ...
        if ok:
            LOG.info(
                "[EXIT-pullback_scalp] trade=%s units=%s reason=%s pnl=%.2fp range=%s touch=%s",
                trade_id,
                units,
                reason,
                pnl,
            ...
            LOG.error("[EXIT-pullback_scalp] close failed trade=%s units=%s reason=%s", trade_id, units, reason)
        return ok
    ...
    def _evaluate(self, trade: dict, ctx: _Context, now: datetime) -> Optional[str]:
        ...
        if section_decision.should_exit and section_decision.reason:
            LOG.info(
                "[EXIT-pullback_scalp] section_exit trade=%s reason=%s detail=%s",
                trade_id,
        ...
        if touch_stats and touch_stats.count >= self.touch_tighten_count:
            profit_take = max(0.5, profit_take * self.touch_tighten_ratio)
            trail_start = max(0.7, trail_start * self.touch_tighten_ratio)
    ...
    async def run(self) -> None:
        LOG.info(
            "[EXIT-pullback_scalp] worker starting (interval=%.2fs tags=%s)",
            self.loop_interval,
        ...
        try:
            while True:
                ...
                for tr in trades:
                    try:
                        ...
                        LOG.exception("[EXIT-pullback_scalp] evaluate failed trade=%s", tr.get("trade_id"))
                        continue
                    if not reason:
                        continue
                    ...
                    if not client_id:
                        LOG.warning("[EXIT-pullback_scalp] missing client_id trade=%s skip close", trade_id)
                        continue
                    state = self._states.get(trade_id)
            ...
            LOG.info("[EXIT-pullback_scalp] worker cancelled")
            raise
        finally:
            try:
                self._pos_manager.close()
            except Exception:
                LOG.exception("[EXIT-pullback_scalp] failed to close PositionManager")


async def pullback_scalp_exit_worker() -> None:
    worker = PullbackScalpExitWorker()
    await worker.run()


if __name__ == "__main__":  # pragma: no cover
    logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s", force=True)
    asyncio.run(pullback_scalp_exit_worker())
