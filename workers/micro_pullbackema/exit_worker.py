"""Exit loop for MicroPullbackEMA (micro pocket) – プラス決済専用。"""
...
ALLOWED_TAGS: Set[str] = {"MicroPullbackEMA"}
POCKET = "micro"
...
class MicroPullbackEMAExitWorker:
    """
    MicroPullbackEMA 専用 EXIT。
    - 最低保有 20s までクローズ禁止
    - PnL>0 のみクローズ（TP/トレール/RSI利確/レンジ長時間微益）
    """

    def __init__(self) -> None:
        self.loop_interval = max(0.5, _float_env("MICRO_PBEMA_EXIT_LOOP_INTERVAL_SEC", 1.0))
        self._pos_manager = PositionManager()
        self._states: Dict[str, _TradeState] = {}

        self.profit_take = max(1.4, _float_env("MICRO_PBEMA_EXIT_PROFIT_PIPS", 2.2))
        self.trail_start = max(1.6, _float_env("MICRO_PBEMA_EXIT_TRAIL_START_PIPS", 2.6))
        self.trail_backoff = max(0.25, _float_env("MICRO_PBEMA_EXIT_TRAIL_BACKOFF_PIPS", 0.8))
        self.lock_buffer = max(0.2, _float_env("MICRO_PBEMA_EXIT_LOCK_BUFFER_PIPS", 0.5))
        self.min_hold_sec = max(5.0, _float_env("MICRO_PBEMA_EXIT_MIN_HOLD_SEC", 20.0))

        self.range_profit_take = max(1.0, _float_env("MICRO_PBEMA_EXIT_RANGE_PROFIT_PIPS", 1.6))
        self.range_trail_start = max(1.2, _float_env("MICRO_PBEMA_EXIT_RANGE_TRAIL_START_PIPS", 2.1))
        self.range_trail_backoff = max(0.2, _float_env("MICRO_PBEMA_EXIT_RANGE_TRAIL_BACKOFF_PIPS", 0.65))
        self.range_lock_buffer = max(0.15, _float_env("MICRO_PBEMA_EXIT_RANGE_LOCK_BUFFER_PIPS", 0.35))
        self.range_max_hold_sec = max(90.0, _float_env("MICRO_PBEMA_EXIT_RANGE_MAX_HOLD_SEC", 30 * 60))

        self.rsi_take_long = _float_env("MICRO_PBEMA_EXIT_RSI_TAKE_LONG", 70.0)
        self.rsi_take_short = _float_env("MICRO_PBEMA_EXIT_RSI_TAKE_SHORT", 30.0)
        self.tech_exit_enabled = _bool_env("MICRO_PBEMA_EXIT_TECH_ENABLED", True)
        self.rsi_fade_long = _float_env("MICRO_PBEMA_EXIT_RSI_FADE_LONG", 44.0)
        self.rsi_fade_short = _float_env("MICRO_PBEMA_EXIT_RSI_FADE_SHORT", 56.0)
        self.vwap_gap_pips = _float_env("MICRO_PBEMA_EXIT_VWAP_GAP_PIPS", 0.8)
        self.structure_adx = _float_env("MICRO_PBEMA_EXIT_STRUCTURE_ADX", 20.0)
        self.structure_gap_pips = _float_env("MICRO_PBEMA_EXIT_STRUCTURE_GAP_PIPS", 1.8)
        self.atr_spike_pips = _float_env("MICRO_PBEMA_EXIT_ATR_SPIKE_PIPS", 5.0)
        self.tech_neg_min_pips = max(0.2, _float_env("MICRO_PBEMA_EXIT_TECH_NEG_MIN_PIPS", self.profit_take * 0.6))
        hard_default = max(self.tech_neg_min_pips + 0.8, self.profit_take * 1.6)
        self.tech_neg_hard_pips = max(
            self.tech_neg_min_pips,
            _float_env("MICRO_PBEMA_EXIT_TECH_NEG_HARD_PIPS", hard_default),
        )
        self.tech_neg_hold_sec = max(self.min_hold_sec, _float_env("MICRO_PBEMA_EXIT_TECH_NEG_HOLD_SEC", 45.0))
        self.tech_score_min = max(1, int(_float_env("MICRO_PBEMA_EXIT_TECH_SCORE_MIN", 2.0)))
    ...
    async def _close(
        ...
        if ok:
            LOG.info("[EXIT-micro_pbema] trade=%s units=%s reason=%s pnl=%.2fp", trade_id, units, reason, pnl)
        else:
            LOG.error("[EXIT-micro_pbema] close failed trade=%s units=%s reason=%s", trade_id, units, reason)
    ...
    async def _review_trade(self, trade: dict, now: datetime, mid: float, rsi: Optional[float], range_active: bool) -> None:
        ...
        if not client_id:
            LOG.warning("[EXIT-micro_pbema] missing client_id trade=%s skip close", trade_id)
            return
        ...
        if section_decision.should_exit and section_decision.reason:
            LOG.info(
                "[EXIT-micro_pbema] section_exit trade=%s reason=%s detail=%s",
                trade_id,
        ...
        if pnl < 0 and self.tech_exit_enabled:
            hard_stop_ready = pnl <= -tech_neg_hard and hold_sec >= min_hold
    ...
    async def run(self) -> None:
        LOG.info(
            "[EXIT-micro_pbema] worker starting (interval=%.2fs tags=%s)",
            self.loop_interval,
            ...
            while True:
                ...
                for tr in trades:
                    try:
                        ...
                        LOG.exception("[EXIT-micro_pbema] review failed trade=%s", tr.get("trade_id"))
                        continue
        except asyncio.CancelledError:
            LOG.info("[EXIT-micro_pbema] worker cancelled")
            raise
        finally:
            try:
                self._pos_manager.close()
            except Exception:
                LOG.exception("[EXIT-micro_pbema] failed to close PositionManager")


async def micro_pullbackema_exit_worker() -> None:
    worker = MicroPullbackEMAExitWorker()
    await worker.run()


if __name__ == "__main__":  # pragma: no cover
    logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s", force=True)
    asyncio.run(micro_pullbackema_exit_worker())
