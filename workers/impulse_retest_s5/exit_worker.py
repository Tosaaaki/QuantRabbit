"""Exit loop for impulse_retest_s5 worker (scalp pocket) with technical filters."""
...
ALLOWED_TAGS = {"impulse_retest_s5"}
POCKET = "scalp"
...
class ImpulseRetestExitWorker:
    """PnL + RSI/ATR/VWAP/レンジ判定を組み合わせた impulse_retest_s5 EXIT."""
    ...
    def __init__(self) -> None:
        ...
        self.profit_take = max(1.0, _float_env("IMPULSE_RETEST_S5_EXIT_PROFIT_PIPS", 2.0))
        self.trail_start = max(1.0, _float_env("IMPULSE_RETEST_S5_EXIT_TRAIL_START_PIPS", 2.8))
        self.trail_backoff = max(0.2, _float_env("IMPULSE_RETEST_S5_EXIT_TRAIL_BACKOFF_PIPS", 0.9))
        self.stop_loss = max(0.8, _float_env("IMPULSE_RETEST_S5_EXIT_STOP_LOSS_PIPS", 1.5))
        self.max_hold_sec = max(60.0, _float_env("IMPULSE_RETEST_S5_EXIT_MAX_HOLD_SEC", 22 * 60))
        self.lock_trigger = max(0.5, _float_env("IMPULSE_RETEST_S5_EXIT_LOCK_TRIGGER_PIPS", 0.9))
        self.lock_buffer = max(0.1, _float_env("IMPULSE_RETEST_S5_EXIT_LOCK_BUFFER_PIPS", 0.5))

        self.range_profit_take = max(0.8, _float_env("IMPULSE_RETEST_S5_EXIT_RANGE_PROFIT_PIPS", 1.6))
        self.range_trail_start = max(0.9, _float_env("IMPULSE_RETEST_S5_EXIT_RANGE_TRAIL_START_PIPS", 2.1))
        self.range_trail_backoff = max(0.2, _float_env("IMPULSE_RETEST_S5_EXIT_RANGE_TRAIL_BACKOFF_PIPS", 0.65))
        self.range_stop_loss = max(0.6, _float_env("IMPULSE_RETEST_S5_EXIT_RANGE_STOP_LOSS_PIPS", 1.0))
        self.range_max_hold_sec = max(45.0, _float_env("IMPULSE_RETEST_S5_EXIT_RANGE_MAX_HOLD_SEC", 15 * 60))
        self.range_lock_trigger = max(0.35, _float_env("IMPULSE_RETEST_S5_EXIT_RANGE_LOCK_TRIGGER_PIPS", 0.8))
        self.range_lock_buffer = max(0.1, _float_env("IMPULSE_RETEST_S5_EXIT_RANGE_LOCK_BUFFER_PIPS", 0.35))
        ...
        self.rsi_fade_long = _float_env("IMPULSE_RETEST_S5_EXIT_RSI_FADE_LONG", 42.0)
        self.rsi_fade_short = _float_env("IMPULSE_RETEST_S5_EXIT_RSI_FADE_SHORT", 58.0)
        ...
        self.negative_hold_sec = max(20.0, _float_env("IMPULSE_RETEST_S5_EXIT_NEG_HOLD_SEC", 130.0))
        self.allow_negative_exit = _bool_env("IMPULSE_RETEST_S5_EXIT_ALLOW_NEGATIVE", True)

        self.vwap_grab_gap = max(0.15, _float_env("IMPULSE_RETEST_S5_EXIT_VWAP_GAP_PIPS", 0.9))

        self.atr_hot = max(0.5, _float_env("IMPULSE_RETEST_S5_EXIT_ATR_HOT_PIPS", 5.0))
        self.atr_cold = max(0.3, _float_env("IMPULSE_RETEST_S5_EXIT_ATR_COLD_PIPS", 1.2))
    ...
    async def _close(self, trade_id: str, units: int, reason: str, pnl: float, side: str, range_mode: bool) -> bool:
        ...
        if ok:
            LOG.info(
                "[EXIT-impulse_retest_s5] trade=%s units=%s reason=%s pnl=%.2fp range=%s",
                trade_id,
                units,
                reason,
                pnl,
            ...
            while True:
                ...
                for tr in trades:
                    try:
                        ...
                        LOG.exception("[EXIT-impulse_retest_s5] evaluate failed trade=%s", tr.get("trade_id"))
                        continue
                    if not reason:
                        continue
