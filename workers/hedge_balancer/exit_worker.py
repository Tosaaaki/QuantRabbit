...
ALLOWED_TAGS: Set[str] = {'HedgeBalancer'}
...
POCKET = _pocket_env("HEDGE_POCKET", "macro")
ALLOWED_TAGS = _tags_env("HEDGE_EXIT_TAGS", ALLOWED_TAGS)
...
class HedgeBalancerExitWorker:
    def __init__(self) -> None:
        self.loop_interval = max(
            0.5,
            _float_env("HEDGE_EXIT_LOOP_INTERVAL_SEC", 1.0),
        )
        self.min_hold_sec = max(
            5.0,
            _float_env("HEDGE_EXIT_MIN_HOLD_SEC", 10.0),
        )
        self.profit_take = max(
            0.5,
            _float_env("HEDGE_EXIT_PROFIT_PIPS", 1.0),
        )
        self.trail_start = max(
            0.5,
            _float_env("HEDGE_EXIT_TRAIL_START_PIPS", 1.5),
        )
        self.trail_backoff = max(
            0.1,
            _float_env("HEDGE_EXIT_TRAIL_BACKOFF_PIPS", 0.5),
        )
        self.lock_buffer = max(
            0.05,
            _float_env("HEDGE_EXIT_LOCK_BUFFER_PIPS", 0.3),
        )
        self.range_profit_take = max(
            0.4,
            _float_env("HEDGE_EXIT_RANGE_PROFIT_PIPS", 0.8),
        )
        self.range_trail_start = max(
            0.4,
            _float_env("HEDGE_EXIT_RANGE_TRAIL_START_PIPS", 1.2),
        )
        self.range_trail_backoff = max(
            0.1,
            _float_env("HEDGE_EXIT_RANGE_TRAIL_BACKOFF_PIPS", 0.4),
        )
        self.range_lock_buffer = max(
            0.05,
            _float_env("HEDGE_EXIT_RANGE_LOCK_BUFFER_PIPS", 0.25),
        )
        self.range_max_hold_sec = max(
            60.0,
            _float_env("HEDGE_EXIT_RANGE_MAX_HOLD_SEC", 1200.0),
        )
    ...
    async def _close(self, trade_id: str, units: int, reason: str, pnl: float, client_order_id: Optional[str]) -> None:
        ...
        if ok:
            LOG.info("[exit-hedge-balancer] trade=%s units=%s reason=%s pnl=%.2fp", trade_id, units, reason, pnl)
        else:
            LOG.error("[exit-hedge-balancer] close failed trade=%s units=%s reason=%s", trade_id, units, reason)
    ...
    async def run(self) -> None:
        LOG.info(
            "[exit-hedge-balancer] exit worker start interval=%.2fs tags=%s pocket=%s",
            self.loop_interval,
            ",".join(sorted(ALLOWED_TAGS)) if ALLOWED_TAGS else "none",
            POCKET,
        )
        if not ALLOWED_TAGS:
            LOG.info("[exit-hedge-balancer] no allowed tags configured; idle")
            try:
                while True:
            ...
            while True:
                ...
                for tr in trades:
                    try:
                        ...
                        LOG.exception("[exit-hedge-balancer] review failed trade=%s", tr.get("trade_id"))
        except asyncio.CancelledError:
            LOG.info("[exit-hedge-balancer] worker cancelled")
            raise
        finally:
            try:
                self._pos_manager.close()
            except Exception:
                LOG.exception("[exit-hedge-balancer] failed to close PositionManager")


async def hedge_balancer_exit_worker() -> None:
    worker = HedgeBalancerExitWorker()
    await worker.run()
