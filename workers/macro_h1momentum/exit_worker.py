"""Exit loop for H1Momentum macro worker – プラス決済専用。"""
...
class H1MomentumExitWorker:
    ...
    def __init__(self) -> None:
        ...
        self.profit_take = max(3.5, _float_env("H1MOMENTUM_EXIT_PROFIT_PIPS", 5.0))
        self.trail_start = max(4.0, _float_env("H1MOMENTUM_EXIT_TRAIL_START_PIPS", 6.5))
        self.trail_backoff = max(0.6, _float_env("H1MOMENTUM_EXIT_TRAIL_BACKOFF_PIPS", 2.0))
        self.lock_buffer = max(0.5, _float_env("H1MOMENTUM_EXIT_LOCK_BUFFER_PIPS", 0.9))
        self.min_hold_sec = max(900.0, _float_env("H1MOMENTUM_EXIT_MIN_HOLD_SEC", 15 * 60))

        self.range_profit_take = max(2.6, _float_env("H1MOMENTUM_EXIT_RANGE_PROFIT_PIPS", 4.2))
        self.range_trail_start = max(3.2, _float_env("H1MOMENTUM_EXIT_RANGE_TRAIL_START_PIPS", 5.6))
        self.range_trail_backoff = max(0.5, _float_env("H1MOMENTUM_EXIT_RANGE_TRAIL_BACKOFF_PIPS", 1.6))
        self.range_lock_buffer = max(0.35, _float_env("H1MOMENTUM_EXIT_RANGE_LOCK_BUFFER_PIPS", 0.7))
        self.range_max_hold_sec = max(1200.0, _float_env("H1MOMENTUM_EXIT_RANGE_MAX_HOLD_SEC", 2.5 * 3600))
    ...
    async def _close(
        ...
        if ok:
            LOG.info("[EXIT-h1momentum] trade=%s units=%s reason=%s pnl=%.2fp", trade_id, units, reason, pnl)
        else:
    ...
    async def _review_trade(
        ...
        if not client_id:
            LOG.warning("[EXIT-h1momentum] missing client_id trade=%s skip close", trade_id)
            return
        ...
        if pnl < 0:
            fac_h1 = all_factors().get("H1") or {}
            adx = _opt_float(fac_h1.get("adx"))
            atr_pips = _opt_float(fac_h1.get("atr_pips"))
            bbw = _opt_float(fac_h1.get("bbw"))
            vwap_gap = _opt_float(fac_h1.get("vwap_gap"))
            ma10 = _opt_float(fac_h1.get("ma10"))
            ma20 = _opt_float(fac_h1.get("ma20"))
...
async def macro_h1momentum_exit_worker() -> None:
...
if __name__ == "__main__":  # pragma: no cover
    ...
    asyncio.run(macro_h1momentum_exit_worker())
