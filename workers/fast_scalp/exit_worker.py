...
class FastScalpExitWorker:
    ...
    async def _review_trade(self, trade: dict, now: datetime, mid: float, rsi: Optional[float], range_active: bool) -> None:
        ...
        if pnl >= profit_take:
            await self._close(trade_id, -units, "take_profit", pnl, client_id, range_active=range_active)
            self._states.pop(trade_id, None)
            return

        if pnl > 0 and rsi is not None:
            if side == "long" and rsi >= self.rsi_take_long:
                await self._close(trade_id, -units, "rsi_take", pnl, client_id, range_active=range_active)
                self._states.pop(trade_id, None)
                return
        ...
        if range_active and hold_sec >= self.range_max_hold_sec and pnl > 0:
            await self._close(trade_id, -units, "range_timeout", pnl, client_id, range_active=range_active)
            self._states.pop(trade_id, None)
            return
