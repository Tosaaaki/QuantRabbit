...
MAX_SPREAD_PIPS: float = max(0.1, float(os.getenv("FAST_SCALP_MAX_SPREAD_PIPS", "0.5")))
ENTRY_THRESHOLD_PIPS: float = max(0.03, float(os.getenv("FAST_SCALP_ENTRY_MOM_PIPS", "0.2")))
ENTRY_SHORT_THRESHOLD_PIPS: float = max(
    0.015, float(os.getenv("FAST_SCALP_ENTRY_SHORT_MOM_PIPS", "0.12"))
)
ENTRY_RANGE_FLOOR_PIPS: float = max(0.05, float(os.getenv("FAST_SCALP_RANGE_FLOOR_PIPS", "0.4")))
...
MIN_ORDER_SPACING_SEC: float = max(
    0.25, float(os.getenv("FAST_SCALP_MIN_ORDER_SPACING_SEC", "1.0"))
)
...
LONG_WINDOW_SEC: float = max(
    SHORT_WINDOW_SEC + 0.2, float(os.getenv("FAST_SCALP_LONG_WINDOW_SEC", "3.5"))
)
MIN_TICK_COUNT: int = max(3, int(float(os.getenv("FAST_SCALP_MIN_TICK_COUNT", "5"))))
