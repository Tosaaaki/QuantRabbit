...
_DEFAULT_ALLOWED_TAGS = "PullbackContinuation"
        ...
        if pnl < 0:
            ...
            if not skip_soft:
                ...
                if (
                    ...
                    and abs(vwap_gap) <= vwap_gap_pips
                ):
                    ok = await _close(
                        trade_id,
                        -units,
                ...
                if not soft_failed and adx is not None and ma_pair is not None:
                    ...
                    if adx <= structure_adx and (
                        cross_bad or gap <= structure_gap_pips
                    ):
                        ok = await _close(
                            trade_id,
                            -units,
