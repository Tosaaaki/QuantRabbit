"""Exit loop for scalp multi-strategy worker with technical filters."""
...
class ScalpMultiExitWorker:
    """PnL + RSI/ATR/VWAP/レンジ判定を組み合わせた scalp_multistrat EXIT."""
    ...
    def __init__(self) -> None:
        ...
        self.profit_take = max(0.8, _float_env("SCALP_MULTI_EXIT_PROFIT_PIPS", 1.6))
        self.trail_start = max(1.0, _float_env("SCALP_MULTI_EXIT_TRAIL_START_PIPS", 2.2))
        self.trail_backoff = max(0.2, _float_env("SCALP_MULTI_EXIT_TRAIL_BACKOFF_PIPS", 0.8))
        self.stop_loss = max(0.5, _float_env("SCALP_MULTI_EXIT_STOP_LOSS_PIPS", 1.0))
        self.max_hold_sec = max(20.0, _float_env("SCALP_MULTI_EXIT_MAX_HOLD_SEC", 12 * 60))
        self.lock_trigger = max(0.3, _float_env("SCALP_MULTI_EXIT_LOCK_TRIGGER_PIPS", 0.7))
        self.lock_buffer = max(0.05, _float_env("SCALP_MULTI_EXIT_LOCK_BUFFER_PIPS", 0.35))

        self.range_profit_take = max(0.6, _float_env("SCALP_MULTI_EXIT_RANGE_PROFIT_PIPS", 1.35))
        ...
        self.range_stop_loss = max(0.4, _float_env("SCALP_MULTI_EXIT_RANGE_STOP_LOSS_PIPS", 0.9))
        self.range_max_hold_sec = max(20.0, _float_env("SCALP_MULTI_EXIT_RANGE_MAX_HOLD_SEC", 10 * 60))
        self.range_lock_trigger = max(0.25, _float_env("SCALP_MULTI_EXIT_RANGE_LOCK_TRIGGER_PIPS", 0.65))
        ...
        self.negative_hold_sec = max(12.0, _float_env("SCALP_MULTI_EXIT_NEG_HOLD_SEC", 90.0))
        self.allow_negative_exit = _bool_env("SCALP_MULTI_EXIT_ALLOW_NEGATIVE", True)

        self.vwap_grab_gap = max(0.1, _float_env("SCALP_MULTI_EXIT_VWAP_GAP_PIPS", 0.75))

        self.atr_hot = max(0.4, _float_env("SCALP_MULTI_EXIT_ATR_HOT_PIPS", 4.5))
        self.atr_cold = max(0.2, _float_env("SCALP_MULTI_EXIT_ATR_COLD_PIPS", 1.0))
    ...
    async def _close(self, trade_id: str, units: int, reason: str, pnl: float, side: str, range_mode: bool) -> bool:
        ...
        if ok:
            LOG.info(
                "[EXIT-scalp_multistrat] trade=%s units=%s reason=%s pnl=%.2fp range=%s",
                trade_id,
                units,
                reason,
                pnl,
            ...
            log_metric(
                "scalp_multistrat_exit",
                pnl,
                tags={"reason": reason, "range": str(range_mode), "side": side},
                ts=_utc_now(),
            )
        else:
            LOG.error("[EXIT-scalp_multistrat] close failed trade=%s units=%s reason=%s", trade_id, units, reason)
        return ok
    ...
    def _evaluate(self, trade: dict, ctx: _Context, now: datetime) -> Optional[str]:
        ...
        if atr >= self.atr_hot:
            ...
            profit_take = max(0.9, profit_take * 0.88)
            stop_loss = max(0.5, stop_loss * 0.9)
    ...
    async def run(self) -> None:
        LOG.info(
            "[EXIT-scalp_multistrat] worker starting (interval=%.2fs tags=%s)",
            self.loop_interval,
        ...
        try:
            while True:
                ...
                for tr in trades:
                    try:
                        ...
                        LOG.exception("[EXIT-scalp_multistrat] evaluate failed trade=%s", tr.get("trade_id"))
                        continue
                    if not reason:
                        continue
            ...
            LOG.info("[EXIT-scalp_multistrat] worker cancelled")
            raise
        finally:
            try:
                self._pos_manager.close()
            except Exception:
                LOG.exception("[EXIT-scalp_multistrat] failed to close PositionManager")
