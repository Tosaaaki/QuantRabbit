...
def _bb_style_for_trade(trade: dict) -> str:
    return "reversion" if _is_reversion_candidate(trade) else "scalp"
...
class ScalpMultiExitWorker:
    ...
    def __init__(self) -> None:
        ...
        self.lock_trigger = max(0.6, _float_env("SCALP_MULTI_EXIT_LOCK_TRIGGER_PIPS", self.profit_take * 0.35))
        ...
        self.range_lock_trigger = max(
            0.5, _float_env("SCALP_MULTI_EXIT_RANGE_LOCK_TRIGGER_PIPS", self.range_profit_take * 0.35)
        )
        ...
        self.tech_exit_enabled = _bool_env("SCALP_MULTI_EXIT_TECH_ENABLED", True)
        self.tech_take_enabled = _bool_env("SCALP_MULTI_EXIT_TECH_TAKE_ENABLED", True)
        self.tech_take_min_pips = max(
            0.2, _float_env("SCALP_MULTI_EXIT_TECH_TAKE_MIN_PIPS", self.profit_take * 0.5)
        )
        self.rsi_fade_long = _float_env("SCALP_MULTI_EXIT_RSI_FADE_LONG", 44.0)
        self.rsi_fade_short = _float_env("SCALP_MULTI_EXIT_RSI_FADE_SHORT", 56.0)
        self.vwap_gap_pips = _float_env("SCALP_MULTI_EXIT_VWAP_GAP_PIPS", 0.8)
        self.structure_adx = _float_env("SCALP_MULTI_EXIT_STRUCTURE_ADX", 20.0)
        self.structure_gap_pips = _float_env("SCALP_MULTI_EXIT_STRUCTURE_GAP_PIPS", 1.8)
        self.atr_spike_pips = _float_env("SCALP_MULTI_EXIT_ATR_SPIKE_PIPS", 5.0)
        self.tech_neg_min_pips = max(0.2, _float_env("SCALP_MULTI_EXIT_TECH_NEG_MIN_PIPS", self.profit_take * 0.6))
        hard_default = max(self.tech_neg_min_pips + 0.6, self.profit_take * 1.5)
        self.tech_neg_hard_pips = max(
            self.tech_neg_min_pips,
            _float_env("SCALP_MULTI_EXIT_TECH_NEG_HARD_PIPS", hard_default),
        )
        self.tech_neg_hold_sec = max(self.min_hold_sec, _float_env("SCALP_MULTI_EXIT_TECH_NEG_HOLD_SEC", 20.0))
        self.tech_score_min = max(1, int(_float_env("SCALP_MULTI_EXIT_TECH_SCORE_MIN", 2.0)))
    ...
    async def _review_trade(self, trade: dict, now: datetime, mid: float, rsi: Optional[float], range_active: bool) -> None:
        ...
        if pnl < 0 and self.tech_exit_enabled and not _is_reversion_candidate(trade):
            ...
            if hard_stop_ready or soft_ready:
                ...
                if hard_stop_ready and score == 0:
                    await self._close(trade_id, -units, "tech_hard_stop", pnl, client_id, bb_style=bb_style, range_active=range_active)
                    self._states.pop(trade_id, None)
                    return
                if (hard_stop_ready and score > 0) or (soft_ready and score >= self.tech_score_min):
                    await self._close(trade_id, -units, reason or "tech_exit", pnl, client_id, bb_style=bb_style, range_active=range_active)
                    self._states.pop(trade_id, None)
                    return
        ...
        if pnl > 0 and _is_reversion_candidate(trade):
            ...
            if tp_decision.should_exit:
                ...
                await self._close(trade_id, -units, "take_profit_zone", pnl, client_id, bb_style=bb_style, range_active=range_active)
                self._states.pop(trade_id, None)
                return
        ...
        if (
            ...
            await self._close(trade_id, -units, "lock_floor", pnl, client_id, bb_style=bb_style, range_active=range_active)
            self._states.pop(trade_id, None)
            return

        if pnl > 0 and self.tech_take_enabled and pnl >= self.tech_take_min_pips:
            await self._close(trade_id, -units, "tech_take", pnl, client_id, bb_style=bb_style, range_active=range_active)
            self._states.pop(trade_id, None)
            return

        if state.peak > 0 and state.peak >= trail_start and pnl > 0 and pnl <= state.peak - trail_backoff:
            await self._close(trade_id, -units, "trail_take", pnl, client_id, bb_style=bb_style, range_active=range_active)
            self._states.pop(trade_id, None)
            return

        if pnl >= profit_take:
            await self._close(trade_id, -units, "take_profit", pnl, client_id, bb_style=bb_style, range_active=range_active)
            self._states.pop(trade_id, None)
            return

        if pnl > 0 and rsi is not None:
            if side == "long" and rsi >= self.rsi_take_long:
                await self._close(trade_id, -units, "rsi_take", pnl, client_id, bb_style=bb_style, range_active=range_active)
                self._states.pop(trade_id, None)
                return
        ...
        if range_active and hold_sec >= self.range_max_hold_sec and pnl > 0:
            await self._close(trade_id, -units, "range_timeout", pnl, client_id, bb_style=bb_style, range_active=range_active)
            self._states.pop(trade_id, None)
            return
