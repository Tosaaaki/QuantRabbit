...
ALLOWED_TAGS: Set[str] = {"MicroVWAPBound"}
POCKET = "micro"
...
class MicroVWAPBoundExitWorker:
    """
    MicroVWAPBound 専用 EXIT。
    - 最低保有 20s までクローズ禁止
    - PnL>0 のみクローズ（TP/トレール/RSI利確/レンジ長時間微益）
    """

    def __init__(self) -> None:
        self.loop_interval = max(0.5, _float_env("MICRO_VWAP_EXIT_LOOP_INTERVAL_SEC", 1.0))
        self._pos_manager = PositionManager()
        self._states: Dict[str, _TradeState] = {}

        self.profit_take = max(1.2, _float_env("MICRO_VWAP_EXIT_PROFIT_PIPS", 1.8))
    ...
    async def _close(
        ...
        if ok:
            ...
            LOG.error("[EXIT-micro_vwap] close failed trade=%s units=%s reason=%s", trade_id, units, reason)
    ...
    async def _review_trade(self, trade: dict, now: datetime, mid: float, rsi: Optional[float], range_active: bool) -> None:
        ...
        if not client_id:
            LOG.warning("[EXIT-micro_vwapbound] missing client_id trade=%s skip close", trade_id)
            return
    ...
    async def run(self) -> None:
        LOG.info(
            "[EXIT-micro_vwap] worker starting (interval=%.2fs tags=%s)",
            self.loop_interval,
            ...
            while True:
                ...
                for tr in trades:
                    try:
                        ...
                        LOG.exception("[EXIT-micro_vwap] review failed trade=%s", tr.get("trade_id"))
                        continue
        except asyncio.CancelledError:
            LOG.info("[EXIT-micro_vwap] worker cancelled")
            raise
        finally:
            try:
                self._pos_manager.close()
            except Exception:
                LOG.exception("[EXIT-micro_vwap] failed to close PositionManager")


async def micro_vwapbound_exit_worker() -> None:
    worker = MicroVWAPBoundExitWorker()
    await worker.run()
