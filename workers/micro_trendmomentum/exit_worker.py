"""Exit loop for TrendMomentumMicro (micro pocket) – プラス決済専用。"""
...
class TrendMomentumMicroExitWorker:
    ...
    def __init__(self) -> None:
        self.loop_interval = max(0.5, _float_env("TRENDMM_EXIT_LOOP_INTERVAL_SEC", 1.0))
        self._pos_manager = PositionManager()
        self._states: Dict[str, _TradeState] = {}

        self.profit_take = max(1.6, _float_env("TRENDMM_EXIT_PROFIT_PIPS", 2.4))
        self.trail_start = max(1.8, _float_env("TRENDMM_EXIT_TRAIL_START_PIPS", 3.0))
        self.trail_backoff = max(0.25, _float_env("TRENDMM_EXIT_TRAIL_BACKOFF_PIPS", 0.9))
        self.lock_buffer = max(0.2, _float_env("TRENDMM_EXIT_LOCK_BUFFER_PIPS", 0.5))
        self.min_hold_sec = max(5.0, _float_env("TRENDMM_EXIT_MIN_HOLD_SEC", 20.0))

        self.range_profit_take = max(1.2, _float_env("TRENDMM_EXIT_RANGE_PROFIT_PIPS", 1.8))
        self.range_trail_start = max(1.4, _float_env("TRENDMM_EXIT_RANGE_TRAIL_START_PIPS", 2.4))
        self.range_trail_backoff = max(0.2, _float_env("TRENDMM_EXIT_RANGE_TRAIL_BACKOFF_PIPS", 0.7))
        self.range_lock_buffer = max(0.15, _float_env("TRENDMM_EXIT_RANGE_LOCK_BUFFER_PIPS", 0.35))
        self.range_max_hold_sec = max(90.0, _float_env("TRENDMM_EXIT_RANGE_MAX_HOLD_SEC", 30 * 60))

        self.rsi_take_long = _float_env("TRENDMM_EXIT_RSI_TAKE_LONG", 70.0)
        self.rsi_take_short = _float_env("TRENDMM_EXIT_RSI_TAKE_SHORT", 30.0)
        self.tech_exit_enabled = _bool_env("TRENDMM_EXIT_TECH_ENABLED", True)
        self.rsi_fade_long = _float_env("TRENDMM_EXIT_RSI_FADE_LONG", 44.0)
        self.rsi_fade_short = _float_env("TRENDMM_EXIT_RSI_FADE_SHORT", 56.0)
        self.vwap_gap_pips = _float_env("TRENDMM_EXIT_VWAP_GAP_PIPS", 0.8)
        self.structure_adx = _float_env("TRENDMM_EXIT_STRUCTURE_ADX", 20.0)
        self.structure_gap_pips = _float_env("TRENDMM_EXIT_STRUCTURE_GAP_PIPS", 1.8)
        self.atr_spike_pips = _float_env("TRENDMM_EXIT_ATR_SPIKE_PIPS", 5.0)
        self.tech_neg_min_pips = max(0.2, _float_env("TRENDMM_EXIT_TECH_NEG_MIN_PIPS", self.profit_take * 0.6))
        hard_default = max(self.tech_neg_min_pips + 0.8, self.profit_take * 1.6)
        self.tech_neg_hard_pips = max(
            self.tech_neg_min_pips,
            _float_env("TRENDMM_EXIT_TECH_NEG_HARD_PIPS", hard_default),
        )
        self.tech_neg_hold_sec = max(self.min_hold_sec, _float_env("TRENDMM_EXIT_TECH_NEG_HOLD_SEC", 45.0))
        self.tech_score_min = max(1, int(_float_env("TRENDMM_EXIT_TECH_SCORE_MIN", 2.0)))
    ...
    def _context(self) -> tuple[Optional[float], Optional[float], bool]:
        ...
        try:
            range_active = bool(
                getattr(
                    __import__("analysis.range_guard", fromlist=["detect_range_mode"]).range_guard.detect_range_mode(
                        fac_m1, fac_h4
                    ).active
                )
            )
    ...
    async def _close(
        ...
        if ok:
            ...
            LOG.error("[EXIT-trendmm] close failed trade=%s units=%s reason=%s", trade_id, units, reason)
    ...
    async def _review_trade(self, trade: dict, now: datetime, mid: float, rsi: Optional[float], range_active: bool) -> None:
        ...
        if not client_id:
            LOG.warning("[EXIT-micro_tm] missing client_id trade=%s skip close", trade_id)
            return
        ...
        if section_decision.should_exit and section_decision.reason:
            LOG.info(
                "[EXIT-trendmm] section_exit trade=%s reason=%s detail=%s",
                trade_id,
    ...
    async def run(self) -> None:
        LOG.info(
            "[EXIT-trendmm] worker starting (interval=%.2fs tags=%s)",
            self.loop_interval,
            ...
            while True:
                ...
                for tr in trades:
                    try:
                        ...
                        LOG.exception("[EXIT-trendmm] review failed trade=%s", tr.get("trade_id"))
                        continue
        except asyncio.CancelledError:
            LOG.info("[EXIT-trendmm] worker cancelled")
            raise
        finally:
            try:
                self._pos_manager.close()
            except Exception:
                LOG.exception("[EXIT-trendmm] failed to close PositionManager")


async def trendmomentum_micro_exit_worker() -> None:
    worker = TrendMomentumMicroExitWorker()
    await worker.run()


if __name__ == "__main__":  # pragma: no cover
    logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s", force=True)
    asyncio.run(trendmomentum_micro_exit_worker())
