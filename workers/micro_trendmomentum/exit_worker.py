"""Dedicated exit worker for TrendMomentumMicro strategy.
...
os.environ.setdefault("MICRO_MULTI_LOG_PREFIX", "[TrendMomentumMicro]")
os.environ.setdefault("MICRO_MULTI_EXIT_TAG_ALLOWLIST", "TrendMomentumMicro")
...
async def micro_trendmomentum_exit_worker() -> None:
    await micro_runtime_exit_worker()


if __name__ == "__main__":  # pragma: no cover
    logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s", force=True)
    asyncio.run(micro_trendmomentum_exit_worker())
