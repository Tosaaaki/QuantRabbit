"""Exit loop for impulse_momentum_s5 worker (scalp pocket)."""
...
ALLOWED_TAGS: Set[str] = {"impulse_momentum_s5"}
POCKET = "scalp"
...
class ImpulseMomentumExitWorker:
    """
    impulse_momentum_s5 用の専用 EXIT。
    - 最低保有を超えるまでクローズ禁止（スプレッド負け防止）
    - PnL>0 のみクローズ（TP/トレール/構造ブレイクがプラスのときだけ）
    ...
    def __init__(self) -> None:
        ...
        # 通常時パラメータ
        self.profit_take = max(1.2, _float_env("IMPULSE_MOMENTUM_S5_EXIT_PROFIT_PIPS", 2.4))
        self.trail_start = max(1.4, _float_env("IMPULSE_MOMENTUM_S5_EXIT_TRAIL_START_PIPS", 3.2))
        self.trail_backoff = max(0.2, _float_env("IMPULSE_MOMENTUM_S5_EXIT_TRAIL_BACKOFF_PIPS", 1.0))
        self.lock_buffer = max(0.2, _float_env("IMPULSE_MOMENTUM_S5_EXIT_LOCK_BUFFER_PIPS", 0.7))
        self.min_hold_sec = max(1.0, _float_env("IMPULSE_MOMENTUM_S5_EXIT_MIN_HOLD_SEC", 10.0))

        # レンジ時は閾値をタイトに（ただしプラス決済のみ）
        self.range_profit_take = max(0.9, _float_env("IMPULSE_MOMENTUM_S5_EXIT_RANGE_PROFIT_PIPS", 1.8))
        self.range_trail_start = max(1.1, _float_env("IMPULSE_MOMENTUM_S5_EXIT_RANGE_TRAIL_START_PIPS", 2.4))
        self.range_trail_backoff = max(0.2, _float_env("IMPULSE_MOMENTUM_S5_EXIT_RANGE_TRAIL_BACKOFF_PIPS", 0.7))
        self.range_lock_buffer = max(0.15, _float_env("IMPULSE_MOMENTUM_S5_EXIT_RANGE_LOCK_BUFFER_PIPS", 0.4))
        self.range_max_hold_sec = max(60.0, _float_env("IMPULSE_MOMENTUM_S5_EXIT_RANGE_MAX_HOLD_SEC", 12 * 60))

        # レンジ判定閾値（range_guard と整合）
        ...
        # RSI で利確を早める（プラス圏限定）
        self.rsi_take_long = _float_env("IMPULSE_MOMENTUM_S5_EXIT_RSI_TAKE_LONG", 71.0)
        self.rsi_take_short = _float_env("IMPULSE_MOMENTUM_S5_EXIT_RSI_TAKE_SHORT", 29.0)
    ...
    async def _review_trade(self, trade: dict, now: datetime, mid: float, rsi: Optional[float], range_active: bool) -> None:
        ...
        # レンジ時の長時間ホールドは微益で逃げる（マイナスでは閉じない）
            ...
            while True:
                ...
                for tr in trades:
                    try:
                        ...
                        LOG.exception("[EXIT-impulse_momentum_s5] review failed trade=%s", tr.get("trade_id"))
                        continue
