...
ACTIVE_HOURS_UTC = frozenset(
    _parse_hours("IMPULSE_BREAK_S5_ACTIVE_HOURS", "1,5-8,11,13,15,17-18")
)
...
MAX_SPREAD_PIPS: float = max(0.1, _float("IMPULSE_BREAK_S5_MAX_SPREAD_PIPS", 0.75))
ENTRY_UNITS: int = max(1000, _int("IMPULSE_BREAK_S5_ENTRY_UNITS", 5000))
MAX_ACTIVE_TRADES: int = max(1, _int("IMPULSE_BREAK_S5_MAX_ACTIVE_TRADES", 1))
COOLDOWN_SEC: float = max(20.0, _float("IMPULSE_BREAK_S5_COOLDOWN_SEC", 150.0))
...
NEWS_BLOCK_MINUTES: float = max(
    0.0, _float("IMPULSE_BREAK_S5_NEWS_BLOCK_MINUTES", 20.0)
)
NEWS_BLOCK_MIN_IMPACT: int = max(
    1, _int("IMPULSE_BREAK_S5_NEWS_BLOCK_MIN_IMPACT", 2)
)
