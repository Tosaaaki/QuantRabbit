...
ENABLED: bool = _bool("MIRROR_SPIKE_TIGHT_ENABLED", False)
...
LOSS_STREAK_MAX: int = max(0, _int("MIRROR_SPIKE_TIGHT_MAX_CONSEC_LOSSES", 2))
...
NEWS_BLOCK_MINUTES: float = max(
    0.0, _float("MIRROR_SPIKE_TIGHT_NEWS_BLOCK_MINUTES", 30.0)
)
NEWS_BLOCK_MIN_IMPACT: int = max(
    1, _int("MIRROR_SPIKE_TIGHT_NEWS_BLOCK_MIN_IMPACT", 2)
)
BLOCK_REGIMES = tuple(
    regime.strip()
