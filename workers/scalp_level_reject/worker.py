"""Scalp Level Reject dedicated ENTRY worker."""
...
ENV_PREFIX = "SCALP_LEVEL_REJECT"
STRATEGY_TAG = "LevelReject"
PIP = 0.01
...
LEVEL_REJECT_TICK_STRENGTH_MIN = _env_float("LEVEL_REJECT_TICK_STRENGTH_MIN", 0.0)
LEVEL_REJECT_ALLOWED_REGIMES = _env_set("LEVEL_REJECT_ALLOWED_REGIMES", "")
...
def _signal_level_reject(
    ...
    *,
    ...
    conf = 58 + int(min(12, abs(rsi - 50.0) * 0.6)) + int(min(10, tick_strength * 6.0))
...
def _build_entry_thesis(signal: Dict[str, object], fac_m1: Dict[str, object], range_ctx) -> Dict[str, object]:
    ...
    return {
        ...
        "confidence": conf,
        "entry_probability": _to_probability(conf),
...
async def _run_worker() -> None:
    ...
    LOG.info("%s worker start mode=%s", LOG_PREFIX, STRATEGY_TAG)
