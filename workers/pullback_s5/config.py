...
TOUCH_ENABLED: bool = _bool("PULLBACK_S5_TOUCH_ENABLED", True)
TOUCH_WINDOW_SEC: float = max(30.0, _float("PULLBACK_S5_TOUCH_WINDOW_SEC", 240.0))
TOUCH_PULLBACK_ATR_MULT: float = max(0.1, _float("PULLBACK_S5_TOUCH_PULLBACK_ATR_MULT", 0.8))
TOUCH_PULLBACK_MIN_PIPS: float = max(0.1, _float("PULLBACK_S5_TOUCH_PULLBACK_MIN_PIPS", 0.6))
TOUCH_PULLBACK_MAX_PIPS: float = max(
    TOUCH_PULLBACK_MIN_PIPS,
    _float("PULLBACK_S5_TOUCH_PULLBACK_MAX_PIPS", 2.2),
)
TOUCH_TREND_ATR_MULT: float = max(0.1, _float("PULLBACK_S5_TOUCH_TREND_ATR_MULT", 1.6))
TOUCH_TREND_MIN_PIPS: float = max(0.2, _float("PULLBACK_S5_TOUCH_TREND_MIN_PIPS", 1.2))
TOUCH_TREND_MAX_PIPS: float = max(
    TOUCH_TREND_MIN_PIPS,
    _float("PULLBACK_S5_TOUCH_TREND_MAX_PIPS", 4.5),
)
TOUCH_RESET_RATIO: float = max(0.1, min(0.9, _float("PULLBACK_S5_TOUCH_RESET_RATIO", 0.45)))
TOUCH_SOFT_COUNT: int = max(1, _int("PULLBACK_S5_TOUCH_SOFT_COUNT", 2))
TOUCH_HARD_COUNT: int = max(TOUCH_SOFT_COUNT + 1, _int("PULLBACK_S5_TOUCH_HARD_COUNT", 4))
TOUCH_UNIT_FACTOR: float = max(0.1, min(1.0, _float("PULLBACK_S5_TOUCH_UNIT_FACTOR", 0.7)))
TOUCH_CONF_PENALTY: int = max(0, _int("PULLBACK_S5_TOUCH_CONF_PENALTY", 8))
