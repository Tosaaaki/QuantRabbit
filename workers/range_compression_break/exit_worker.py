"""Exit worker for Range Compression Break."""
...
class RangeCompressionBreakExitWorker:
    ...
    async def _close(
        ...
        if ok:
            ...
            LOG.error("[EXIT-range_comp_break] close failed trade=%s units=%s reason=%s", trade_id, units, reason)
    ...
    async def _review_trade(self, trade: dict, now: datetime, mid: float) -> None:
        ...
        if pnl < 0:
            ...
            reentry = decide_reentry(
                prefix="RANGE_COMPRESSION_BREAK",
                side=side,
                pnl_pips=pnl,
                rsi=rsi,
                adx=adx,
    ...
    async def run(self) -> None:
        LOG.info("[EXIT-range_comp_break] worker starting (interval=%.2fs)", self.loop_interval)
        while True:
            ...
            for tr in trades:
                ...
                try:
                    ...
                    LOG.exception("[EXIT-range_comp_break] review failed")


async def range_compression_break_exit_worker() -> None:
    worker = RangeCompressionBreakExitWorker()
    try:
        await worker.run()
    except asyncio.CancelledError:
        LOG.info("[EXIT-range_comp_break] worker cancelled")
        raise
    finally:
        try:
            worker._pos_manager.close()
        except Exception:
            LOG.exception("[EXIT-range_comp_break] failed to close PositionManager")


if __name__ == "__main__":
    asyncio.run(range_compression_break_exit_worker())
