"""Exit worker for Scalp Reversal-NWave."""
...
def _latest_mid() -> float:
    ticks = tick_window.recent_ticks(seconds=2.5, limit=1)
    if ticks:
        tick = ticks[-1]
...
class ScalpReversalNWaveExitWorker:
    ...
    async def _close(
        ...
        if ok:
            LOG.info("[EXIT-scalp_rev_nwave] trade=%s units=%s reason=%s pnl=%.2fp", trade_id, units, reason, pnl)
        else:
            LOG.error("[EXIT-scalp_rev_nwave] close failed trade=%s units=%s reason=%s", trade_id, units, reason)
    ...
    async def run(self) -> None:
        LOG.info("[EXIT-scalp_rev_nwave] worker starting (interval=%.2fs)", self.loop_interval)
        while True:
            ...
            for tr in trades:
                ...
                try:
                    ...
                    LOG.exception("[EXIT-scalp_rev_nwave] review failed")


async def scalp_reversal_nwave_exit_worker() -> None:
    worker = ScalpReversalNWaveExitWorker()
    try:
        await worker.run()
    except asyncio.CancelledError:
        LOG.info("[EXIT-scalp_rev_nwave] worker cancelled")
        raise
    finally:
        try:
            worker._pos_manager.close()
        except Exception:
            LOG.exception("[EXIT-scalp_rev_nwave] failed to close PositionManager")


if __name__ == "__main__":
    asyncio.run(scalp_reversal_nwave_exit_worker())
