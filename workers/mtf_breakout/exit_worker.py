...
ALLOWED_TAGS: Set[str] = {'mtf_breakout_m5_h1', 'mtf_breakout'}
...
POCKET = _pocket_env("MTF_BREAKOUT_POCKET", "micro")
ALLOWED_TAGS = _tags_env("MTF_BREAKOUT_EXIT_TAGS", ALLOWED_TAGS)
...
class MtfBreakoutExitWorker:
    def __init__(self) -> None:
        self.loop_interval = max(
            0.5,
            _float_env("MTF_BREAKOUT_EXIT_LOOP_INTERVAL_SEC", 1.0),
        )
        self.min_hold_sec = max(
            5.0,
            _float_env("MTF_BREAKOUT_EXIT_MIN_HOLD_SEC", 30.0),
        )
        self.profit_take = max(
            0.5,
            _float_env("MTF_BREAKOUT_EXIT_PROFIT_PIPS", 2.2),
        )
        self.trail_start = max(
            0.5,
            _float_env("MTF_BREAKOUT_EXIT_TRAIL_START_PIPS", 2.8),
        )
        self.trail_backoff = max(
            0.1,
            _float_env("MTF_BREAKOUT_EXIT_TRAIL_BACKOFF_PIPS", 0.8),
        )
        self.lock_buffer = max(
            0.05,
            _float_env("MTF_BREAKOUT_EXIT_LOCK_BUFFER_PIPS", 0.5),
        )
        self.range_profit_take = max(
            0.4,
            _float_env("MTF_BREAKOUT_EXIT_RANGE_PROFIT_PIPS", 1.8),
        )
        self.range_trail_start = max(
            0.4,
            _float_env("MTF_BREAKOUT_EXIT_RANGE_TRAIL_START_PIPS", 2.2),
        )
        self.range_trail_backoff = max(
            0.1,
            _float_env("MTF_BREAKOUT_EXIT_RANGE_TRAIL_BACKOFF_PIPS", 0.6),
        )
        self.range_lock_buffer = max(
            0.05,
            _float_env("MTF_BREAKOUT_EXIT_RANGE_LOCK_BUFFER_PIPS", 0.35),
        )
        self.range_max_hold_sec = max(
            60.0,
            _float_env("MTF_BREAKOUT_EXIT_RANGE_MAX_HOLD_SEC", 2700.0),
        )
    ...
    async def _close(self, trade_id: str, units: int, reason: str, pnl: float, client_order_id: Optional[str]) -> None:
        ...
        if ok:
            LOG.info("[exit-mtf-breakout] trade=%s units=%s reason=%s pnl=%.2fp", trade_id, units, reason, pnl)
        else:
            LOG.error("[exit-mtf-breakout] close failed trade=%s units=%s reason=%s", trade_id, units, reason)
    ...
    async def run(self) -> None:
        LOG.info(
            "[exit-mtf-breakout] exit worker start interval=%.2fs tags=%s pocket=%s",
            self.loop_interval,
            ",".join(sorted(ALLOWED_TAGS)) if ALLOWED_TAGS else "none",
            POCKET,
        )
        if not ALLOWED_TAGS:
            LOG.info("[exit-mtf-breakout] no allowed tags configured; idle")
            try:
                while True:
            ...
            while True:
                ...
                for tr in trades:
                    try:
                        ...
                        LOG.exception("[exit-mtf-breakout] review failed trade=%s", tr.get("trade_id"))
        except asyncio.CancelledError:
            LOG.info("[exit-mtf-breakout] worker cancelled")
            raise
        finally:
            try:
                self._pos_manager.close()
            except Exception:
                LOG.exception("[exit-mtf-breakout] failed to close PositionManager")


async def mtf_breakout_exit_worker() -> None:
    worker = MtfBreakoutExitWorker()
    await worker.run()
