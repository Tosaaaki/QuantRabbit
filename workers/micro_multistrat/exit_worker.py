"""Exit loop for micro multi-strategy worker with technical filters."""
...
class MicroMultiExitWorker:
    """PnL + RSI/ATR/VWAP/レンジ判定を組み合わせた micro_multistrat EXIT."""
    ...
    def __init__(self) -> None:
        ...
        self.profit_take = max(1.0, _float_env("MICRO_MULTI_EXIT_PROFIT_PIPS", 2.0))
        self.trail_start = max(1.2, _float_env("MICRO_MULTI_EXIT_TRAIL_START_PIPS", 2.8))
        self.trail_backoff = max(0.2, _float_env("MICRO_MULTI_EXIT_TRAIL_BACKOFF_PIPS", 0.9))
        self.stop_loss = max(0.6, _float_env("MICRO_MULTI_EXIT_STOP_LOSS_PIPS", 1.4))
        self.max_hold_sec = max(60.0, _float_env("MICRO_MULTI_EXIT_MAX_HOLD_SEC", 45 * 60))
        self.lock_trigger = max(0.4, _float_env("MICRO_MULTI_EXIT_LOCK_TRIGGER_PIPS", 0.9))
        self.lock_buffer = max(0.1, _float_env("MICRO_MULTI_EXIT_LOCK_BUFFER_PIPS", 0.4))

        self.range_profit_take = max(0.7, _float_env("MICRO_MULTI_EXIT_RANGE_PROFIT_PIPS", 1.6))
        self.range_trail_start = max(1.0, _float_env("MICRO_MULTI_EXIT_RANGE_TRAIL_START_PIPS", 2.1))
        self.range_trail_backoff = max(0.2, _float_env("MICRO_MULTI_EXIT_RANGE_TRAIL_BACKOFF_PIPS", 0.6))
        self.range_stop_loss = max(0.5, _float_env("MICRO_MULTI_EXIT_RANGE_STOP_LOSS_PIPS", 1.05))
        self.range_max_hold_sec = max(60.0, _float_env("MICRO_MULTI_EXIT_RANGE_MAX_HOLD_SEC", 36 * 60))
        self.range_lock_trigger = max(0.3, _float_env("MICRO_MULTI_EXIT_RANGE_LOCK_TRIGGER_PIPS", 0.75))
        ...
        self.rsi_fade_long = _float_env("MICRO_MULTI_EXIT_RSI_FADE_LONG", 42.0)
        self.rsi_fade_short = _float_env("MICRO_MULTI_EXIT_RSI_FADE_SHORT", 58.0)
        ...
        self.negative_hold_sec = max(25.0, _float_env("MICRO_MULTI_EXIT_NEG_HOLD_SEC", 180.0))
        self.allow_negative_exit = _bool_env("MICRO_MULTI_EXIT_ALLOW_NEGATIVE", True)

        self.vwap_grab_gap = max(0.1, _float_env("MICRO_MULTI_EXIT_VWAP_GAP_PIPS", 0.85))

        self.atr_hot = max(0.5, _float_env("MICRO_MULTI_EXIT_ATR_HOT_PIPS", 6.0))
        self.atr_cold = max(0.3, _float_env("MICRO_MULTI_EXIT_ATR_COLD_PIPS", 1.2))
    ...
    async def _close(self, trade_id: str, units: int, reason: str, pnl: float, side: str, range_mode: bool) -> bool:
        ...
        if ok:
            LOG.info(
                "[EXIT-micro_multistrat] trade=%s units=%s reason=%s pnl=%.2fp range=%s",
                trade_id,
                units,
                reason,
                pnl,
            ...
            log_metric(
                "micro_multistrat_exit",
                pnl,
                tags={"reason": reason, "range": str(range_mode), "side": side},
                ts=_utc_now(),
            )
        else:
            LOG.error("[EXIT-micro_multistrat] close failed trade=%s units=%s reason=%s", trade_id, units, reason)
        return ok
    ...
    def _evaluate(self, trade: dict, ctx: _Context, now: datetime) -> Optional[str]:
        ...
        if atr >= self.atr_hot:
            ...
            stop_loss = max(0.55, stop_loss * 0.9)
            ...
            while True:
                ...
                for tr in trades:
                    try:
                        ...
                        LOG.exception("[EXIT-micro_multistrat] evaluate failed trade=%s", tr.get("trade_id"))
                        continue
                    if not reason:
                        continue
