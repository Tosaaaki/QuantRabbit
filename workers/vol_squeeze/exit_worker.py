...
POCKET = _pocket_env("VOL_SQUEEZE_POCKET", "micro")
ALLOWED_TAGS = _tags_env("VOL_SQUEEZE_EXIT_TAGS", ALLOWED_TAGS)
...
class VolSqueezeExitWorker:
    def __init__(self) -> None:
        self.loop_interval = max(
            0.5,
            _float_env("VOL_SQUEEZE_EXIT_LOOP_INTERVAL_SEC", 1.0),
        )
        self.min_hold_sec = max(
            5.0,
            _float_env("VOL_SQUEEZE_EXIT_MIN_HOLD_SEC", 25.0),
        )
        self.profit_take = max(
            0.5,
            _float_env("VOL_SQUEEZE_EXIT_PROFIT_PIPS", 2.0),
        )
        self.trail_start = max(
            0.5,
            _float_env("VOL_SQUEEZE_EXIT_TRAIL_START_PIPS", 2.6),
        )
        self.trail_backoff = max(
            0.1,
            _float_env("VOL_SQUEEZE_EXIT_TRAIL_BACKOFF_PIPS", 0.8),
        )
        self.lock_buffer = max(
            0.05,
            _float_env("VOL_SQUEEZE_EXIT_LOCK_BUFFER_PIPS", 0.5),
        )
        self.range_profit_take = max(
            0.4,
            _float_env("VOL_SQUEEZE_EXIT_RANGE_PROFIT_PIPS", 1.6),
        )
        self.range_trail_start = max(
            0.4,
            _float_env("VOL_SQUEEZE_EXIT_RANGE_TRAIL_START_PIPS", 2.0),
        )
        self.range_trail_backoff = max(
            0.1,
            _float_env("VOL_SQUEEZE_EXIT_RANGE_TRAIL_BACKOFF_PIPS", 0.6),
        )
        self.range_lock_buffer = max(
            0.05,
            _float_env("VOL_SQUEEZE_EXIT_RANGE_LOCK_BUFFER_PIPS", 0.35),
        )
    ...
    async def _close(self, trade_id: str, units: int, reason: str, pnl: float, client_order_id: Optional[str], allow_negative: bool = False) -> None:
        ...
        if ok:
            LOG.info("[exit-vol-squeeze] trade=%s units=%s reason=%s pnl=%.2fp", trade_id, units, reason, pnl)
        else:
            LOG.error("[exit-vol-squeeze] close failed trade=%s units=%s reason=%s", trade_id, units, reason)
    ...
    async def _review_trade(self, trade: dict, now: datetime, mid: float, range_active: bool) -> None:
        ...
        if not client_id:
            LOG.warning("[exit-vol-squeeze] missing client_id trade=%s skip close", trade_id)
            return
    ...
    async def run(self) -> None:
        LOG.info(
            "[exit-vol-squeeze] exit worker start interval=%.2fs tags=%s pocket=%s",
            self.loop_interval,
            ",".join(sorted(ALLOWED_TAGS)) if ALLOWED_TAGS else "none",
            POCKET,
        )
        if not ALLOWED_TAGS:
            LOG.info("[exit-vol-squeeze] no allowed tags configured; idle")
            try:
                while True:
            ...
            while True:
                ...
                for tr in trades:
                    try:
                        ...
                        LOG.exception("[exit-vol-squeeze] review failed trade=%s", tr.get("trade_id"))
        except asyncio.CancelledError:
            LOG.info("[exit-vol-squeeze] worker cancelled")
            raise
        finally:
            try:
                self._pos_manager.close()
            except Exception:
                LOG.exception("[exit-vol-squeeze] failed to close PositionManager")


async def vol_squeeze_exit_worker() -> None:
    worker = VolSqueezeExitWorker()
    await worker.run()
