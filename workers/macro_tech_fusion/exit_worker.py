"""Exit worker for Macro Tech Fusion."""
...
def _latest_mid() -> float:
    ticks = tick_window.recent_ticks(seconds=5.0, limit=1)
    if ticks:
        tick = ticks[-1]
...
class MacroTechFusionExitWorker:
    ...
    async def _close(
        ...
        if ok:
            LOG.info("[EXIT-macro_techfusion] trade=%s units=%s reason=%s pnl=%.2fp", trade_id, units, reason, pnl)
        else:
            LOG.error("[EXIT-macro_techfusion] close failed trade=%s units=%s reason=%s", trade_id, units, reason)
    ...
    async def run(self) -> None:
        LOG.info("[EXIT-macro_techfusion] worker starting (interval=%.2fs)", self.loop_interval)
        while True:
            ...
            for tr in trades:
                ...
                try:
                    ...
                    LOG.exception("[EXIT-macro_techfusion] review failed")


async def macro_tech_fusion_exit_worker() -> None:
    worker = MacroTechFusionExitWorker()
    try:
        await worker.run()
    except asyncio.CancelledError:
        LOG.info("[EXIT-macro_techfusion] worker cancelled")
        raise
    finally:
        try:
            worker._pos_manager.close()
        except Exception:
            LOG.exception("[EXIT-macro_techfusion] failed to close PositionManager")


if __name__ == "__main__":
    asyncio.run(macro_tech_fusion_exit_worker())
