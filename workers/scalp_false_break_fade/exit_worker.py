...
_EXIT_ENV_PREFIX = "SCALP_FALSE_BREAK_FADE_EXIT"
...
def _map_exit_env_prefix() -> None:
    ...
    for key, value in list(os.environ.items()):
        if not key.startswith(source):
            continue
        target = f"SCALP_FALSE_BREAK_FADE_{key[source_len:]}"
        os.environ.setdefault(target, value)

    os.environ.setdefault(
        "SCALP_FALSE_BREAK_FADE_EXIT_TAGS",
        os.getenv(f"{_EXIT_ENV_PREFIX}_TAGS", "FalseBreakFade"),
    )
    os.environ.setdefault(
        "SCALP_FALSE_BREAK_FADE_EXIT_LOG_PREFIX",
        os.getenv(f"{_EXIT_ENV_PREFIX}_LOG_PREFIX", "[ScalpExit:FalseBreakFade]"),
    )
    os.environ.setdefault(
        "SCALP_FALSE_BREAK_FADE_POCKET",
        os.getenv(f"{_EXIT_ENV_PREFIX}_POCKET", "scalp_fast"),
    )
    os.environ.setdefault(
        "SCALP_FALSE_BREAK_FADE_EXIT_PROFILE_ENABLED",
        os.getenv(
            f"{_EXIT_ENV_PREFIX}_PROFILE_ENABLED",
            os.getenv("SCALP_FALSE_BREAK_FADE_EXIT_PROFILE_ENABLED", "1"),
        ),
    )
    os.environ.setdefault(
        "SCALP_FALSE_BREAK_FADE_EXIT_PROFILE_PATH",
        os.getenv(
    ...
    os.environ.setdefault(
        "SCALP_FALSE_BREAK_FADE_EXIT_PROFILE_TTL_SEC",
...
_STRATEGY_PROTECTION_ENABLED = (
    os.getenv("SCALP_FALSE_BREAK_FADE_EXIT_PROFILE_ENABLED", "1").strip().lower()
    not in _FALSEY
)
_STRATEGY_PROTECTION_TTL_SEC = max(
    2.0, float(os.getenv("SCALP_FALSE_BREAK_FADE_EXIT_PROFILE_TTL_SEC", "12.0") or 12.0)
)
...
LOG_PREFIX = os.getenv("SCALP_FALSE_BREAK_FADE_EXIT_LOG_PREFIX", "[ScalpPrecisionExit]")


# Tags that scalp_false_break_fade exit_worker is allowed to manage. Missing tags effectively disable
...
POCKET = _pocket_env("SCALP_FALSE_BREAK_FADE_POCKET", "scalp")
ALLOWED_TAGS = _tags_env("SCALP_FALSE_BREAK_FADE_EXIT_TAGS", ALLOWED_TAGS)
...
class RangeFaderExitWorker:
    def __init__(self) -> None:
        ...
        self.exit_policy_start_ts = load_rollout_start_ts(
            "SCALP_FALSE_BREAK_FADE_EXIT_POLICY_START_TS"
        )
    ...
    def _maybe_log_rollout_skip(
        ...
        *,
        ...
        log_metric(
            "scalp_false_break_fade_rollout_skip",
            float(pnl),
            tags={
                "reason": reason,
                "side": side,
            ...
            if (
                ...
                if (
                    ...
                    if ok:
                        ...
                        log_metric(
                            "scalp_false_break_fade_tick_imb_partial_take",
                            pnl,
                            tags={"side": side},
                            ts=now,
                        )
                        ...
                        if side == "long":
                            ...
                            if (
                                ...
                                if ok:
                                    ...
                                    log_metric(
                                        "scalp_false_break_fade_profit_lock_sl",
                                        lock_pips,
...
async def scalp_false_break_fade_exit_worker() -> None:
...
if __name__ == "__main__":
    asyncio.run(scalp_false_break_fade_exit_worker())
