"""Exit loop for vwap_magnet_s5 worker (scalp pocket) with technical filters."""
...
class VWAPMagnetExitWorker:
    """PnL + RSI/ATR/VWAP/レンジ判定を組み合わせた vwap_magnet_s5 EXIT."""

    def __init__(self) -> None:
        self.tp_scale = TPScaleConfig()
        self.loop_interval = max(0.3, _float_env("VWAP_MAGNET_S5_EXIT_LOOP_INTERVAL_SEC", 1.0))
        self._pos_manager = PositionManager()
        self._states: Dict[str, _TradeState] = {}

        self.profit_take = max(1.0, _float_env("VWAP_MAGNET_S5_EXIT_PROFIT_PIPS", 2.1))
        self.trail_start = max(1.2, _float_env("VWAP_MAGNET_S5_EXIT_TRAIL_START_PIPS", 2.9))
        self.trail_backoff = max(0.25, _float_env("VWAP_MAGNET_S5_EXIT_TRAIL_BACKOFF_PIPS", 0.9))
        self.stop_loss = max(0.8, _float_env("VWAP_MAGNET_S5_EXIT_STOP_LOSS_PIPS", 1.5))
        self.max_hold_sec = max(60.0, _float_env("VWAP_MAGNET_S5_EXIT_MAX_HOLD_SEC", 22 * 60))
        self.lock_trigger = max(0.5, _float_env("VWAP_MAGNET_S5_EXIT_LOCK_TRIGGER_PIPS", 1.0))
        self.lock_buffer = max(0.1, _float_env("VWAP_MAGNET_S5_EXIT_LOCK_BUFFER_PIPS", 0.5))

        self.range_profit_take = max(0.8, _float_env("VWAP_MAGNET_S5_EXIT_RANGE_PROFIT_PIPS", 1.7))
        self.range_trail_start = max(1.0, _float_env("VWAP_MAGNET_S5_EXIT_RANGE_TRAIL_START_PIPS", 2.2))
        self.range_trail_backoff = max(0.2, _float_env("VWAP_MAGNET_S5_EXIT_RANGE_TRAIL_BACKOFF_PIPS", 0.65))
        self.range_stop_loss = max(0.6, _float_env("VWAP_MAGNET_S5_EXIT_RANGE_STOP_LOSS_PIPS", 1.1))
        self.range_max_hold_sec = max(45.0, _float_env("VWAP_MAGNET_S5_EXIT_RANGE_MAX_HOLD_SEC", 16 * 60))
        self.range_lock_trigger = max(0.35, _float_env("VWAP_MAGNET_S5_EXIT_RANGE_LOCK_TRIGGER_PIPS", 0.8))
        self.range_lock_buffer = max(0.1, _float_env("VWAP_MAGNET_S5_EXIT_RANGE_LOCK_BUFFER_PIPS", 0.35))

        self.range_adx = max(5.0, _float_env("VWAP_MAGNET_S5_EXIT_RANGE_ADX", 22.0))
        self.range_bbw = max(0.02, _float_env("VWAP_MAGNET_S5_EXIT_RANGE_BBW", 0.20))
        self.range_atr = max(0.4, _float_env("VWAP_MAGNET_S5_EXIT_RANGE_ATR", 6.0))

        self.rsi_fade_long = _float_env("VWAP_MAGNET_S5_EXIT_RSI_FADE_LONG", 42.0)
        self.rsi_fade_short = _float_env("VWAP_MAGNET_S5_EXIT_RSI_FADE_SHORT", 58.0)
        self.rsi_take_long = _float_env("VWAP_MAGNET_S5_EXIT_RSI_TAKE_LONG", 70.0)
        self.rsi_take_short = _float_env("VWAP_MAGNET_S5_EXIT_RSI_TAKE_SHORT", 30.0)
        self.negative_hold_sec = max(20.0, _float_env("VWAP_MAGNET_S5_EXIT_NEG_HOLD_SEC", 130.0))
        self.allow_negative_exit = _bool_env("VWAP_MAGNET_S5_EXIT_ALLOW_NEGATIVE", True)

        self.vwap_grab_gap = max(0.15, _float_env("VWAP_MAGNET_S5_EXIT_VWAP_GAP_PIPS", 0.8))

        self.atr_hot = max(0.5, _float_env("VWAP_MAGNET_S5_EXIT_ATR_HOT_PIPS", 5.0))
        self.atr_cold = max(0.3, _float_env("VWAP_MAGNET_S5_EXIT_ATR_COLD_PIPS", 1.2))
    ...
    async def _close(
        ...
        if ok:
            LOG.info(
                "[EXIT-vwap_magnet_s5] trade=%s units=%s reason=%s pnl=%.2fp range=%s",
                trade_id,
                units,
                reason,
                pnl,
            ...
            LOG.error("[EXIT-vwap_magnet_s5] close failed trade=%s units=%s reason=%s", trade_id, units, reason)
        return ok
    ...
    def _evaluate(self, trade: dict, ctx: _Context, now: datetime) -> Optional[str]:
        ...
        if pnl > 0:
            ...
            if tp_decision.should_exit:
                LOG.info(
                    "[EXIT-vwap_magnet_s5] tp_zone trade=%s detail=%s",
                    trade_id,
                    tp_decision.debug,
                )
    ...
    async def run(self) -> None:
        LOG.info(
            "[EXIT-vwap_magnet_s5] worker starting (interval=%.2fs tags=%s)",
            self.loop_interval,
        ...
        try:
            while True:
                ...
                for tr in trades:
                    ...
                    try:
                        ...
                        LOG.exception("[EXIT-vwap_magnet_s5] evaluate failed trade=%s", tr.get("trade_id"))
                        continue
                    if not reason:
                        continue
                    trade_id = str(tr.get("trade_id"))
                    ...
                    if not client_id:
                        LOG.warning("[EXIT-vwap_magnet_s5] missing client_id trade=%s skip close", trade_id)
                        continue
                    side = "long" if units > 0 else "short"
            ...
            LOG.info("[EXIT-vwap_magnet_s5] worker cancelled")
            raise
        finally:
            try:
                self._pos_manager.close()
            except Exception:
                LOG.exception("[EXIT-vwap_magnet_s5] failed to close PositionManager")


async def vwap_magnet_s5_exit_worker() -> None:
    worker = VWAPMagnetExitWorker()
    await worker.run()
