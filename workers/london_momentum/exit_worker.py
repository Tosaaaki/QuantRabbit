...
class LondonMomentumExitWorker:
    ...
    def _evaluate(self, trade: dict, ctx: _Context, now: datetime) -> Optional[str]:
        ...
        if section_decision.should_exit and section_decision.reason:
            LOG.info(
                "[EXIT-london_momentum] section_exit trade=%s reason=%s detail=%s",
                trade_id,
        ...
        if state.hard_stop:
            stop_loss = max(stop_loss, max(1.0, state.hard_stop * 0.5))
            lock_trigger = max(lock_trigger, max(0.5, state.hard_stop * 0.25))
            trail_start = max(trail_start, max(1.8, state.hard_stop * 0.6))
        ...
        try:
            while True:
                ...
                for tr in trades:
                    ...
                    if not client_id:
                        LOG.warning("[EXIT-london_momentum] missing client_id trade=%s skip close", trade_id)
                        continue
                    side = "long" if units > 0 else "short"
