...
BLOCK_RANGE_MODE: bool = _bool("PULLBACK_RUNNER_S5_BLOCK_RANGE_MODE", False)
...
MAX_MARGIN_USAGE: float = max(0.1, min(1.0, _float("PULLBACK_RUNNER_S5_MAX_MARGIN_USAGE", 0.9)))

# Guard against chasing local extremes
EXTREME_GUARD_ENABLED: bool = _bool("PULLBACK_RUNNER_S5_EXTREME_GUARD_ENABLED", True)
EXTREME_LOOKBACK_SEC: float = max(20.0, _float("PULLBACK_RUNNER_S5_EXTREME_LOOKBACK_SEC", 90.0))
EXTREME_ATR_MULT: float = max(0.1, _float("PULLBACK_RUNNER_S5_EXTREME_ATR_MULT", 0.8))
EXTREME_MIN_PIPS: float = max(0.2, _float("PULLBACK_RUNNER_S5_EXTREME_MIN_PIPS", 0.6))
EXTREME_MAX_PIPS: float = max(0.4, _float("PULLBACK_RUNNER_S5_EXTREME_MAX_PIPS", 2.0))
...
TOUCH_ENABLED: bool = _bool("PULLBACK_RUNNER_S5_TOUCH_ENABLED", True)
TOUCH_WINDOW_SEC: float = max(30.0, _float("PULLBACK_RUNNER_S5_TOUCH_WINDOW_SEC", 240.0))
TOUCH_PULLBACK_ATR_MULT: float = max(0.1, _float("PULLBACK_RUNNER_S5_TOUCH_PULLBACK_ATR_MULT", 0.8))
TOUCH_PULLBACK_MIN_PIPS: float = max(0.1, _float("PULLBACK_RUNNER_S5_TOUCH_PULLBACK_MIN_PIPS", 0.6))
TOUCH_PULLBACK_MAX_PIPS: float = max(
    TOUCH_PULLBACK_MIN_PIPS,
    _float("PULLBACK_RUNNER_S5_TOUCH_PULLBACK_MAX_PIPS", 2.4),
)
TOUCH_TREND_ATR_MULT: float = max(0.1, _float("PULLBACK_RUNNER_S5_TOUCH_TREND_ATR_MULT", 1.6))
TOUCH_TREND_MIN_PIPS: float = max(0.2, _float("PULLBACK_RUNNER_S5_TOUCH_TREND_MIN_PIPS", 1.2))
TOUCH_TREND_MAX_PIPS: float = max(
    TOUCH_TREND_MIN_PIPS,
    _float("PULLBACK_RUNNER_S5_TOUCH_TREND_MAX_PIPS", 4.5),
)
TOUCH_RESET_RATIO: float = max(0.1, min(0.9, _float("PULLBACK_RUNNER_S5_TOUCH_RESET_RATIO", 0.45)))
