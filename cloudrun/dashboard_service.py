  ...
  <script>
    ...
    async function loadAll() {
      ...
      try {
        ...
        es.onopen = () => { window.useSSE = true; try { const s = document.getElementById('sse'); s.textContent='SSE 接続'; s.className='badge badge-green'; } catch(_) {} };
        es.onerror = () => { window.useSSE = false; try { const s = document.getElementById('sse'); s.textContent='SSE 切断'; s.className='badge badge-red'; } catch(_) {} };
        es.addEventListener('kpi', (ev) => {
          try {
            ...
            // Trade state badges
            try {
              const microOk = !!t.can_trade_micro;
              const macroOk = !!t.can_trade_macro;
              const gddStop = !!t.global_dd_stop;
              const microState = document.getElementById('state-micro');
              const macroState = document.getElementById('state-macro');
              const gddState = document.getElementById('state-gdd');
              if (microState) { microState.textContent = microOk ? 'MICRO 可' : 'MICRO 停止'; setClass(microState, 'badge', microOk ? 'badge-green' : 'badge-red'); }
              if (macroState) { macroState.textContent = macroOk ? 'MACRO 可' : 'MACRO 停止'; setClass(macroState, 'badge', macroOk ? 'badge-green' : 'badge-red'); }
              if (gddState) { gddState.textContent = gddStop ? 'GLOBAL DD STOP' : 'GLOBAL OK'; setClass(gddState, 'badge', gddStop ? 'badge-red' : 'badge-green'); }
            } catch(_) {}
            // Pocket weights doughnut
            try {
              const wMacro = Math.max(0, Math.min(1, Number(t.weight_macro ?? 0)));
              const wMicro = Math.max(0, Math.min(1, Number(t.weight_micro ?? 0)));
              const wScalp = Math.max(0, Math.min(1, Number(t.weight_scalp ?? 0)));
              const ctxw = document.getElementById('chartWeights') ? document.getElementById('chartWeights').getContext('2d') : null;
              if (window.Chart && ctxw) {
                const d = [wMacro, wMicro, wScalp];
                if (!chartWeights) {
                  chartWeights = new Chart(ctxw, { type:'doughnut', data:{ labels:['Macro','Micro','Scalp'], datasets:[{ data:d, backgroundColor:['#3b82f6','#10b981','#f59e0b'] }] }, options:{ responsive:true, cutout:'60%' } });
                } else {
                  chartWeights.data.datasets[0].data = d; chartWeights.update();
                }
              }
            } catch(_) {}
...
<body>
  <header>
    <h1>QuantRabbit ダッシュボード</h1>
    <span id="sse" class="badge">SSE 準備中</span>
  </header>
  <div class="container">
    <div class="cards kpis">
      <div class="card"><div class="muted">最終更新</div><div id="updated" class="value">-</div></div>
      <div class="card"><div class="muted">バケット</div><div id="bucket" class="value">-</div></div>
      <div class="card"><div class="muted">未処理（raw）</div><div id="raw" class="value">-</div></div>
      <div class="card"><div class="muted">サマリ件数</div><div id="sum" class="value">-</div></div>
      <div class="card"><div class="muted">エクイティ</div><div id="equity" class="value">-</div></div>
      <div class="card"><div class="muted">許容ロット</div><div id="lot" class="value">-</div></div>
      <div class="card"><div class="muted">モード</div><div id="mode" class="value">-</div></div>
      <div class="card"><div class="muted">銘柄</div><div id="instruments" class="value">-</div></div>
      <div class="card"><div class="muted">分散スロット</div><div id="divslots" class="value">-</div></div>
      <div class="card"><div class="muted">リソースTTL</div><div id="ttl" class="value">-</div></div>
      <div class="card"><div class="muted">本日PnL</div><div id="pnl" class="value">-</div><div class="bar"><div id="bar-pnl-target" class="fill"></div></div></div>
      <div class="card"><div class="muted">本日の進捗</div><div id="progress" class="value">-</div><div class="bar"><div id="bar-progress" class="fill"></div></div></div>
      <div class="card"><div class="muted">デイリー目標 (<span id="target-pct">-</span>%)</div><div id="target-status" class="value">-</div><div class="bar"><div id="bar-target" class="fill fill-amber"></div></div></div>
      <div class="card"><div class="muted">取引状態</div>
        <div style="display:flex; gap:6px; flex-wrap:wrap; margin-top:4px;">
          <span id="state-micro" class="badge">MICRO -</span>
          <span id="state-macro" class="badge">MACRO -</span>
          <span id="state-gdd" class="badge">GLOBAL -</span>
        </div>
      </div>
    </div>
    <div class="row" style="margin-top:12px;">
      <div class="card w50">
        <div class="muted">1時間あたりのサマリ</div>
        <canvas id="chart" height="120"></canvas>
      </div>
      <div class="card w50">
        <div class="muted">レジーム / フォーカス / 指標</div>
        <div>マクロ: <span id="badge-reg-macro" class="badge">-</span></div>
        <div>マイクロ: <span id="badge-reg-micro" class="badge">-</span></div>
        <div style="margin-top:6px;">フォーカス: <span id="focus">-</span></div>
        <div style="margin-top:6px;">
          <div class="muted">マクロ比率</div>
          <div class="bar"><div id="bar-weight" class="fill fill-blue" style="width:0%"></div></div>
        </div>
        <div style="margin-top:8px;">イベント: <span id="badge-event" class="badge">-</span></div>
        <div style="margin-top:12px;">
          <div class="muted">Pocket Weights</div>
    ...
    <div class="row" style="margin-top:12px;">
      <div class="card w50">
        <div class="muted">最新ニュース</div>
        <div id="news" class="list"></div>
      </div>
      <div class="card w50">
        <div class="muted">直近トレード</div>
