...
from execution.scalp_engine import get_scalp_state, run_scalp_once, get_tick_metrics_snapshot
...
from execution.micro_guard import is_micro_entry_blocked
import httpx
from utils.firestore_helpers import apply_filter
from utils.trade_logger import log_trade_snapshot
...
SCALP_DISABLE_ON_EVENT = os.environ.get("SCALP_DISABLE_ON_EVENT", "true").lower() == "true"

STRONG_TREND_VELOCITY = float(os.environ.get("STRONG_TREND_VELOCITY_30S", "7.5"))
STRONG_TREND_RANGE = float(os.environ.get("STRONG_TREND_RANGE_30S", "10.0"))
...
MIN_SCALP_WEIGHT = max(0.0, _env_float("MIN_SCALP_WEIGHT", 0.06))
MAX_SCALP_WEIGHT = max(MIN_SCALP_WEIGHT, _env_float("MAX_SCALP_WEIGHT", 0.45))
MACRO_SCALP_CAP = max(0.6, _env_float("MACRO_SCALP_SUM_CAP", 0.9))
POCKET_BIAS_SCALE = _env_float("POCKET_WEIGHT_BIAS_SCALE", 0.12)  # score → weight 可変係数
POCKET_BIAS_MAX = _env_float("POCKET_WEIGHT_BIAS_MAX", 0.05)      # 一度に動かす上限
ALLOW_STRATEGY_FALLBACK = os.environ.get("ALLOW_STRATEGY_FALLBACK", "false").lower() == "true"
MICRO_FALLBACK_MIN_ATR_PIPS = max(0.4, _env_float("MICRO_FALLBACK_MIN_ATR_PIPS", 0.8))
MICRO_FALLBACK_MAX_ATR_PIPS = max(
    MICRO_FALLBACK_MIN_ATR_PIPS, _env_float("MICRO_FALLBACK_MAX_ATR_PIPS", 4.6)
)
MICRO_FALLBACK_MIN_DEVIATION_PIPS = max(
    0.6, _env_float("MICRO_FALLBACK_MIN_DEVIATION_PIPS", 1.4)
)
MICRO_FALLBACK_MAX_VELOCITY = max(
    0.5, _env_float("MICRO_FALLBACK_MAX_VELOCITY_30S", 5.2)
)
MICRO_FALLBACK_MIN_RANGE = max(0.0, _env_float("MICRO_FALLBACK_MIN_RANGE_30S", 0.8))
MICRO_FALLBACK_MAX_RANGE = max(
    MICRO_FALLBACK_MIN_RANGE, _env_float("MICRO_FALLBACK_MAX_RANGE_30S", 7.5)
)
MICRO_FALLBACK_LOT_FACTOR = max(
    0.1, min(1.0, _env_float("MICRO_FALLBACK_LOT_FACTOR", 0.55))
)
MICRO_FALLBACK_MAX_LOT = max(0.0, _env_float("MICRO_FALLBACK_MAX_LOT", 0.45))
MICRO_FALLBACK_SL_ATR_MULT = max(
    0.7, _env_float("MICRO_FALLBACK_SL_ATR_MULT", 1.2)
)
MICRO_FALLBACK_TP_MULT = max(1.2, _env_float("MICRO_FALLBACK_TP_MULT", 1.7))
...
def _aggregate_pocket_stats(stats: dict[str, dict]) -> dict[str, dict]:
    """Aggregate stats by pocket across strategies: macro/micro/scalp."""
    pockets = {
        "macro": {"trades": 0, "wins": 0, "losses": 0, "sum_pips": 0.0},
        "micro": {"trades": 0, "wins": 0, "losses": 0, "sum_pips": 0.0},
        "scalp": {"trades": 0, "wins": 0, "losses": 0, "sum_pips": 0.0},
    }
    for s in stats.values():
        pb = s.get("pockets", {}) or {}
        for name in ("macro", "micro", "scalp"):
            b = pb.get(name)
            if not b:
                continue
            pockets[name]["trades"] += int(b.get("trades", 0) or 0)
            pockets[name]["wins"] += int(b.get("wins", 0) or 0)
            pockets[name]["losses"] += int(b.get("losses", 0) or 0)
            try:
                pockets[name]["sum_pips"] += float(b.get("sum_pips", 0.0) or 0.0)
            except (TypeError, ValueError):
                pass
    # compute avg + score
    for name, b in pockets.items():
        t = b["trades"]
        avg = (b["sum_pips"] / t) if t else 0.0
        b["avg_pips"] = avg
        b["score"] = _score_row(t, b["wins"], b["losses"], avg)
    return pockets


def _apply_pocket_weight_bias(weight_macro: float, weight_scalp: float, stats: dict[str, dict], *, event_soon: bool) -> tuple[float, float]:
    """Shift weights toward recently profitable pockets within safe bounds.

    Multiplicative nudges by pocket score, capped by POCKET_BIAS_MAX, then
    renormalize and enforce caps.
    """
    if not stats:
        return weight_macro, weight_scalp
    pocket = _aggregate_pocket_stats(stats)
    d_macro = max(min(pocket["macro"]["score"] * POCKET_BIAS_SCALE, POCKET_BIAS_MAX), -POCKET_BIAS_MAX)
    d_micro = max(min(pocket["micro"]["score"] * POCKET_BIAS_SCALE, POCKET_BIAS_MAX), -POCKET_BIAS_MAX)
    d_scalp = max(min(pocket["scalp"]["score"] * POCKET_BIAS_SCALE, POCKET_BIAS_MAX), -POCKET_BIAS_MAX)
    s_macro = max(0.0, weight_macro)
    s_scalp = max(MIN_SCALP_WEIGHT, min(MAX_SCALP_WEIGHT, weight_scalp))
    s_micro = max(0.0, 1.0 - s_macro - s_scalp)
    # Apply multiplicative adjustments
    s_macro *= (1.0 + d_macro)
    s_scalp *= (1.0 + d_scalp)
    # micro is remainder; apply bias by shrinking others if micro strong/weak
    if d_micro != 0.0:
        # shift 50% of micro adjustment proportionally from macro+scalp
        shift = d_micro * 0.5
        s_macro *= (1.0 - max(min(shift, 0.05), -0.05))
        s_scalp *= (1.0 - max(min(shift, 0.05), -0.05))
    # Renormalize to 1.0
    total = s_macro + s_scalp + s_micro
    if total <= 0:
        return weight_macro, weight_scalp
    s_macro /= total
    s_scalp /= total
    s_micro = max(0.0, 1.0 - s_macro - s_scalp)
    # Enforce caps
    s_scalp = max(MIN_SCALP_WEIGHT, min(MAX_SCALP_WEIGHT, s_scalp))
    if s_macro + s_scalp > MACRO_SCALP_CAP:
        excess = (s_macro + s_scalp) - MACRO_SCALP_CAP
        reduce_macro = min(s_macro - 0.05, excess * 0.7)
        if reduce_macro > 0:
            s_macro -= reduce_macro
            excess -= reduce_macro
        if excess > 0:
            s_scalp = max(MIN_SCALP_WEIGHT, s_scalp - excess)
    if event_soon:
        # Do not over-allocate scalping during events
        s_scalp = min(s_scalp, weight_scalp)
    return max(0.0, s_macro), max(MIN_SCALP_WEIGHT, min(MAX_SCALP_WEIGHT, s_scalp))
...
def _get_price_and_spread() -> tuple[float, float]:
    """Return (mid_price, spread_pips) using OANDA pricing snapshot."""
    try:
        from utils.secrets import get_secret
        token = get_secret("oanda_token")
        account = get_secret("oanda_account_id")
        ...
        with httpx.Client(timeout=5.0) as client:
            ...
            spread_pips = max((ask - bid) * 100, 0.0)
            mid = (bid + ask) / 2.0
            return mid, spread_pips
    except Exception:
        return 0.0, 0.0
    ...
    try:
        if "state" not in doc:
            if doc.get("order_error"):
                doc["state"] = "ERROR"
            elif doc.get("close_time"):
                doc["state"] = "CLOSED"
            elif doc.get("trade_id"):
                doc["state"] = "OPEN"
            else:
                doc["state"] = "PLACED"
        ...
        logging.warning("[trade_log] Firestore insert failed: %s", exc)
    try:
        log_trade_snapshot(doc)
    except Exception as exc:
        logging.warning("[trade_log] SQLite write failed: %s", exc)
...
def run_once():
    try:
        if os.environ.get("TRADING_DISABLED", "false").lower() == "true":
        ...
        if RUN_SCALP_ON_REQUEST:
            try:
                mid, spread_for_scalp = _get_price_and_spread()
                if event_soon and SCALP_DISABLE_ON_EVENT:
                    # 更新用にティックをバッファへ投入しつつ、巨大スプレッドで確実にスキップ
                    tmp = run_scalp_once(fac_m1, now_utc, spread_pips=1e9, tick_price=mid)
                    scalp_result = dict(tmp)
                    scalp_result["executed"] = False
                    scalp_result["reason"] = "event_mode"
                else:
                    # スプレッド取得失敗時は None を渡し、エンジン側のデフォルトを使用
                    spread_arg = spread_for_scalp if (spread_for_scalp and spread_for_scalp > 0.0) else None
                    scalp_result = run_scalp_once(
                        fac_m1, now_utc, spread_pips=spread_arg, tick_price=mid
                    )
        ...
        # Prefer real-time tick metrics captured by the scalper's internal buffer
        tickm_snapshot = get_tick_metrics_snapshot(0.01)
        tick_velocity = abs(float(tickm_snapshot.get("tick_velocity_30s") or 0.0))
        tick_range = float(tickm_snapshot.get("tick_range_30s") or 0.0)
        ...
        # Prefer environment override for quicker ops tuning; fall back to Firestore config, then default.
        env_cd = os.environ.get("DECIDER_COOLDOWN_MIN")
        if env_cd is not None:
            try:
                cooldown_min = int(env_cd)
            except (TypeError, ValueError):
                cooldown_min = 30
        else:
        ...
        allowed = ("TrendMA", "Donchian55", "BB_RSI", "NewsSpikeReversal", "MicroTrendPullback", "RangeBounce")
        ...
        # Pocket performance bias: tilt toward better-performing pockets
        try:
            weight_macro, weight_scalp = _apply_pocket_weight_bias(weight_macro, weight_scalp, learning_stats, event_soon=event_soon)
            micro_share = max(0.0, 1.0 - weight_macro - weight_scalp)
            # Recompute mode after bias
            denom = max(1e-6, weight_macro + micro_share)
            macro_ratio = weight_macro / denom
            if macro_ratio <= 0.4:
                mode = "micro"
            ...
            logging.warning("[WEIGHT_BIAS] apply failed: %s", exc)
        ...
        if check_global_drawdown():
            ...
            try:
                risk_cap = float(os.environ.get("RISK_CAP_PCT", "0.02"))
            except Exception:
                risk_cap = 0.02
            try:
                risk_floor = float(os.environ.get("RISK_FLOOR_PCT", "0.0005"))
            except Exception:
                risk_floor = 0.0005
            ...
            floor_meta = None
            if min_units_cfg > 0:
                base_units, floor_meta = apply_min_units_floor(
                    base_units,
                    min_units_cfg,
                    units_by_risk,
                    units_by_margin,
                ...
                if floor_meta:
                    floor_meta.update(
                        ...
                        base_units_initial=base_units_initial,
                    )
                    level = logging.INFO
                    if not floor_meta.get("applied") and floor_meta.get("clipped_by") == "risk":
                        level = logging.WARNING
                    _log_event(level, "min_units_floor", **floor_meta)
            ...
            def _micro_fallback_signal() -> Dict[str, float] | None:
                if not ALLOW_STRATEGY_FALLBACK:
                    return None
                if event_soon:
                    return None
                if is_micro_entry_blocked():
                    logging.info("[MICRO_FALLBACK_BYPASS] reason=guard_active")
                    return None
                ...
                pip = 0.01
                atr_pips = max(atr_val * 100.0, 0.1)
                if (
                    atr_pips < MICRO_FALLBACK_MIN_ATR_PIPS
                    or atr_pips > MICRO_FALLBACK_MAX_ATR_PIPS
                ):
                    logging.info(
                        "[MICRO_FALLBACK_BYPASS] reason=atr window atr=%.2f", atr_pips
                    )
                    return None

                tick_range = float(fac_m1.get("tick_range_30s", 0.0) or 0.0)
                if not (MICRO_FALLBACK_MIN_RANGE <= tick_range <= MICRO_FALLBACK_MAX_RANGE):
                    logging.info(
                        "[MICRO_FALLBACK_BYPASS] reason=range window range=%.2f",
                        tick_range,
                    )
                    return None

                velocity = float(fac_m1.get("tick_velocity_30s", 0.0) or 0.0)
                if abs(velocity) > MICRO_FALLBACK_MAX_VELOCITY:
                    logging.info(
                        "[MICRO_FALLBACK_BYPASS] reason=velocity velocity=%.2f",
                        velocity,
                    )
                    return None

                deviation_pips = (price_val - ma20_val) / pip
                if abs(deviation_pips) < MICRO_FALLBACK_MIN_DEVIATION_PIPS:
                    logging.info(
                        "[MICRO_FALLBACK_BYPASS] reason=deviation deviation=%.2f",
                        deviation_pips,
                    )
                    return None

                tick_mom5 = float(fac_m1.get("tick_momentum_5", 0.0) or 0.0)
                tick_mom10 = float(fac_m1.get("tick_momentum_10", 0.0) or 0.0)

                if micro_regime in ("Trend", "Breakout"):
                    action = "buy" if ma10_val >= ma20_val else "sell"
                elif micro_regime in ("Range", "Mixed"):
                    action = "buy" if deviation_pips <= 0 else "sell"
                else:
                    action = "buy" if deviation_pips <= 0 else "sell"

                if action == "buy":
                    if tick_mom5 < -0.6 and tick_mom10 < -0.4 and velocity < -0.6:
                        logging.info("[MICRO_FALLBACK_BYPASS] reason=momo_downtrend")
                        return None
                else:
                    if tick_mom5 > 0.6 and tick_mom10 > 0.4 and velocity > 0.6:
                        logging.info("[MICRO_FALLBACK_BYPASS] reason=momo_uptrend")
                        return None

                sl_candidates = [
                    8.0,
                    atr_pips * MICRO_FALLBACK_SL_ATR_MULT,
                    abs(deviation_pips) * 0.7,
                ]
                sl_pips = max(sl_candidates)
                tp_floor = sl_pips + max(5.0, atr_pips * 0.9)
                tp_pips = max(sl_pips * MICRO_FALLBACK_TP_MULT, tp_floor)
            ...
            for sname in ranked:
                ...
                if sname == "NewsSpikeReversal":
                    ...
                    # RangeBounce needs both frames and regimes for gating
                ...
                if min_units > 0 and abs(units) < min_units:
                    ...
                    skip_reasons.append(
                        {"strategy": sname, "reason": "units_floor", "pocket": pocket, "floor": min_units}
                    )
                    continue
                if units == 0:
                    logging.info(
                ...
                if floor_meta:
                    trade_req["min_units_floor"] = floor_meta
                order_ctx = {
                    "strategy": sname,
                    "pocket": pocket,
                    "units": units,
                ...
                if floor_meta:
                    order_ctx["min_units_floor"] = {
                        ...
                        if floor_meta.get(k) is not None
                    }
                _log_event(logging.INFO, "order_submit", **order_ctx)
        ...
        if (
            os.environ.get("ENABLE_MICRO_FALLBACK", "false").lower() == "true"
            and not micro_executed
            ...
            if fallback_sig:
                ...
                lot = round(
                    lot
                    * float(
                        _risk_multiplier_from_stats(
                            learning_stats, "MicroTrendPullback", "micro"
                        )
                    ),
                    3,
                )
                lot = round(lot * exposure_factor_base, 4)
                lot = round(lot * MICRO_FALLBACK_LOT_FACTOR, 3)
                if MICRO_FALLBACK_MAX_LOT > 0.0:
                    lot = round(min(lot, MICRO_FALLBACK_MAX_LOT), 3)
                try:
                    sl_for_risk = max(
                        1.0, float(fallback_sig.get("sl_pips", 10.0) or 10.0)
                    )
                except Exception:
                    sl_for_risk = 10.0
                per_signal_cap = allowed_lot(equity, sl_pips=sl_for_risk)
                lot = round(min(lot, per_signal_cap), 3)
                if lot < MIN_BASELINE_POCKET_LOT:
                    logging.info(
                        "[MICRO_FALLBACK_BYPASS] reason=lot_floor lot=%.4f", lot
                    )
                    lot = 0.0
                if lot > 0:
                    ...
                    if units != 0:
                        ...
                        _log_event(logging.INFO, "order_submit", **fallback_ctx)
                        result = asyncio.run(
                            market_order(
                                INSTRUMENT,
                                units,
        ...
        try:
            ...
            status_doc = _status_payload({
                ...
                "weights_final": {
                    "macro": weight_macro,
                    "micro": max(0.0, 1.0 - weight_macro - weight_scalp),
                    "scalp": weight_scalp,
                },
                "mode": mode,
                "selected_instruments": [INSTRUMENT],
