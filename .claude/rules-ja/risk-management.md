...
**サイズ = 現在のNAVの%。サイジング前にsession_data.pyのライブNAVを確認すること。**

| 確度 | pretradeスコア | 条件 | サイズ（NAVの%） | 例 |
|------|------|------|--------|-----|
| **S（鉄板）** | 8+ | H1+H4+マクロ全一致、Div確認 | **NAVの約8-10%** | H4 ADX>30 + H1同方向 + M5押し目 |
| **A（高確度）** | 6-7 | H1方向一致 + M5タイミング確認 | **NAVの約3-5%** | H1 bullish + M5 StochRSI=0.0 |
| **B（普通）** | 4-5 | 1TFのシグナルのみ | **NAVの約1%** | M5 Divのみ、H1不明確 |
| **C（試し打ち）** | 0-3 | 根拠薄い | **NAVの約0.5%** | レンジ内逆張り |

NAV 200kの例: S≈16000-20000u, A≈6000-10000u, B≈2000u, C≈1000u。

**サイジング手順 — エントリー前に必ず現在ポジを確認:**
1. session_data.py出力（またはOANDA口座サマリー）から `NAV` と `marginUsed` を読む
2. 上の表からターゲットユニット数を計算する
3. **このエントリーでmarginUsedが90%を超えないか確認。** (marginUsed + 新ポジのmargin) > NAV×0.90 なら縮小かスキップ
4. OANDAの `marginAvailable` を見れば残り余裕が即わかる
