...
build_cmd=("$GCLOUD_BIN")
deploy_cmd=("$GCLOUD_BIN")
if [ ${#impersonate_args[@]:-0} -gt 0 ]; then
  build_cmd+=("${impersonate_args[@]}")
  deploy_cmd+=("${impersonate_args[@]}")
fi
...
for r in "${REGIONS[@]}"; do
  ...
  echo "  -> news-ingestor (${r})"
  "${deploy_cmd[@]}" run deploy news-ingestor \
    --image gcr.io/${PROJECT}/news-summarizer \
    --region=${r} --platform=managed \
    --service-account=news-summarizer-sa@${PROJECT}.iam.gserviceaccount.com \
    --set-env-vars=BUCKET=quantrabbit-fx-news,GUNICORN_APP=cloudrun.news_ingestor_service:app --quiet
done
