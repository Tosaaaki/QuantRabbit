...
4. **口座確認** — OANDA APIで残高・オープンポジション取得
5. **マクロビュー確認** — `python3 tools/macro_view.py` で通貨強弱・テーマ・MTF一致ペアを把握
6. **市況チェック** — 主要ペアのプライス取得、H1キャンドルで方向感確認
7. **今日の日次ディレクトリ作成** — `daily/YYYY-MM-DD/`
8. **メモリ検索** — 保有ペアごとに過去の教訓を引く:
   ```bash
   cd /Users/tossaki/App/QuantRabbit/collab_trade/memory
   # 保有ペアごとに教訓を引く:
   python3 recall.py search 'GBP_USD 教訓 失敗' --top 2
   python3 recall.py search 'EUR_USD 教訓 失敗' --top 2
   # pretrade_checkも動作確認:
   python3 pretrade_check.py GBP_USD LONG
   ```
9. **ユーザーに一言報告して、トレード開始**（過去の関連記憶があれば併せて共有）

> **注**: analyst/secretaryタスクは廃止済み（v6で廃止）。shared_state.jsonは存在しない。
> マクロ分析は `tools/macro_view.py` がtechnicals_*.jsonから通貨強弱を合成する。
> 記録は全て自分でやれ。セーフティネットはない。
...
- Slack通知: `python3 tools/slack_trade_notify.py {entry|modify|close} --pair {PAIR} ...` で `#qr-trades` に投稿
...
```
┌─→ ★ macro_view.py で通貨強弱・テーマ・MTF一致ペアを把握
│   OANDA APIで価格取得（その場で叩く。裏で回さない）
│   quick_calc.pyでテクニカル計算（パラメータいじりながら）
│   マクロビュー + 自分のテクニカル → 市況判断、テーゼ
│   MTF一致ペアを優先。通貨強弱に逆らわない
...
```bash
python3 tools/macro_view.py
```

4行で出る:
- **通貨強弱**: USD(-0.3) > JPY(+0.1) > ... — 最強/最弱通貨
- **テーマ**: USD弱 + JPY強(リスクオフ) 等 — 今日のマクロ環境
- **MTF一致**: GBP_USD BULL(H1 ADX=30 M5 ADX=25) — H1とM5が同方向 = 高確度チャンス
- **H1 Div**: USD_JPY RegBull=1.0 等 — 転換予兆

**使い方:**
- **セッション開始時**: 必ず実行。これが今の市場の「地図」
- **エントリー前**: MTF一致ペアを優先。通貨強弱に逆行するエントリーは避けろ
- **ポジション管理**: 保有通貨がmacro_viewの最弱 → テーゼに逆風、警戒
- **大きな判断の前**: 利確・損切り・方向転換の前にmacro_viewを再確認
...
### エントリー — 確度は分析の深さで決まる。指標の数ではない

**過去データ: 確度S条件のトレード7件中5件がBサイズで入っていた。原因: いつもの2-3指標だけ見てB判定→止まった。もう少し見ればSだった。6,740-13,140円の損失。**

**6つの指標カテゴリ（FORとDifferent lensで使う）:**
① Direction (ADX/DI, EMAスロープ, MACD方向) ② Timing (StochRSI, RSI, CCI, BB位置) ③ Momentum (MACDヒスト, ROC, EMAクロス) ④ Structure (Fib, cluster, swing, Ichimoku) ⑤ Cross-pair (相関ペア, 通貨強弱) ⑥ Macro (ニュース, イベント, フロー)

**エントリー前に必ず書け:**
```
テーゼ: [1文 — 何をなぜ今。「USD弱い」じゃなく直近20分で何が起きたか]
タイプ: [Scalp / Momentum / Swing]
FOR:  ___ (カテゴリ) + ___ (カテゴリ) + ___ (カテゴリ)
別の目: [FORで使ってないカテゴリから確認] → 支持 / 矛盾 / 中立
AGAINST: ___ [具体的に。「なし」は確認済みの場合のみ]
負けるなら: ___ [このトレードが失敗するシナリオと価格]
→ 確度: [S/A/B/C] | サイズ: ___u (___% NAV)
```

**「別の目」が最重要行。** FORで使った以外のカテゴリから指標を見る:
- 支持 → 確度UP（**BがSに昇格するのはここ。金を拾う行為**）
- 矛盾 → 確度DOWN。AGAINSTに移動、サイズ調整
- テーゼ覆す → エントリー中止。損失回避

| 確度 | 意味 | サイズ @NAV 200k |
|------|------|-----------------|
| **S** | FOR 3+カテゴリ + 別の目も支持 + AGAINSTなし + 負けるならが具体的で今起きてない | **10,000u** (30% NAV) |
| **A** | FORあり + 別の目ほぼ支持 + AGAINSTは具体的だが対処可能 | **5,000u** (15% NAV) |
| **B** | FOR 1-2カテゴリ + 別の目未確認 or 矛盾 + 全体像が不完全 | **1,667u** (5% NAV) |
| **C** | FOR薄い + 別の目矛盾 + 負けるならが書けない | **667u** (2% NAV) |
...
### 利確 — ATR対比で意味のある利幅を取れ
- **ATR50%未満で切るな**: まだ動く余地がある。意味のない小さい利確は時間の無駄
- **「スプレッド+手間に見合うか？」を自問**: 見合わないなら切るな
- **テーゼ射程で判断**: TP目標の半分来たら半利確は合理的。10%で切るな
- **切らない忍耐が利益を生む**: エントリーは悪くない(勝率65%)。利確が早すぎるのが元凶
- **含み益は幻。意味のある額になったら確定**: +2,833円→クローズアウトの教訓。しすぎもしなさすぎもNG
...
| パターン | 実例 | 対策 |
|----------|------|------|
| 小さすぎる利確 | +40円×13件。時間の無駄 | ATR50%未満で切るな。スプレッド+手間に見合うか自問 |
| 利確遅延 | +2,833→クローズアウト、+244→+54 | 意味のある額になったら確定。しすぎもしなさすぎもNG |
...
**自動監視(secretary)は廃止済み。自分で自分を監視しろ。**

### 危険パターン（自覚しろ）

| パターン | 兆候 | 対処 |
|----------|------|------|
| PANIC_TRADING | 30分で8トレード超 | 5分止まれ。macro_view見直せ |
| LOSING_STREAK | 3連続負け | サイズ半減。テーゼ再確認 |
| POST_LOSS_CHASING | 大損後15分で3+新エントリー | 全部閉じてmacro_viewから再出発 |
| TILT_SIZING | 負けた後にロット増 | 最小ロット(500u)に戻せ |

### 共同トレードでの劣化

長時間セッションではコンテキストが長くなりルールを忘れる。2026-03-23の教訓:
- -3,832円大損 → 18分で5エントリー → 3つSL hit → SLなし宣言
- **気づいたら止まれ。macro_viewとstate.mdを読み直せ。**
