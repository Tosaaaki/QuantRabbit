...
- [4/15] **De-botified Pullback Quality**: Original 4/14 version used NOISE/SQUEEZE/DISTRIBUTION scoring + rule tables ("if NOISE → ATR×1.5"). Same bot pattern as profit_check verdict-following. Removed all scoring/verdicts. Now outputs raw data panel (12 indicators in 6 categories). Trader must write "I see / This tells me / So I'm doing" block — forces reading the data instead of following a label. Root cause unchanged: S-trades captured 12-14pip (trail ATR×0.6) vs 4/7's 25-30pip (trail ATR×1.5). Fix is not "better rules" but "force the trader to think". Track: S-trade pip capture + whether "Pullback read" blocks show real interpretation vs template filling
