...
- **確度は分析の深さで決まる。指標の数じゃない。「Different lens」で使い慣れてない指標を見ろ:**
  - S: FOR 3+カテゴリ + Different lensも支持 + AGAINSTなし → 30%NAV
  - A: FOR + 軽微なAGAINST + 対処法あり → 15%NAV
  - B: FOR薄い or Different lens未確認 → 5%NAV
  - C: FOR弱い or 矛盾あり or 「負けるなら」が書けない → 2%NAV
...
- **[4/3] ユーザー指示無視→パニッククローズ→慌てて入り直し = 最悪パターン。** AUD_JPY「SLいらない」→13:44勝手にクローズ(-338円)→13:51に110.118で再エントリー(Sp1.8pip)。持ってれば損失ゼロだった。**ユーザーがHOLDと言ったらHOLD。閉じたいならSlackで提案して返事を待て**
