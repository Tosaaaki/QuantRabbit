...
- 2026-03-12 追記: `workers/scalp_wick_reversal_blend` 系の `PrecisionLowVol` / `DroughtRevert` / `WickReversalBlend` は、current live で worker-local の `setup_pressure` / `flow_guard` / `RR` 修正を優先する。shared prefix `SCALP_PRECISION` の perf guard が soft block (`pf<1` や `failfast_soft`) で entry starvation を起こしている間は、dedicated env で `SCALP_PRECISION_PERF_GUARD_ENABLED=0` を明示して strategy-local guard に委ねる。`VwapRevertS` のような `hard:failfast` lane は reopen せず、hard loser は別扱いとする。
