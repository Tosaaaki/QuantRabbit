...
class PositionManagerTest(unittest.TestCase):
    ...
    def test_fresh_broken_thesis_carries_close_review_without_market_close(self) -> None:
        ...
        try:
            with tempfile.TemporaryDirectory() as tmp:
                ...
                self.assertEqual(result.positions[0].close_review_action, ACTION_REVIEW_EXIT)
                report = (root / "pm.md").read_text()
                self.assertIn("thesis_evolution BROKEN/RECOMMEND_CLOSE", report)
                self.assertIn("loss-side market close still requires GPT CLOSE Gate A/B", report)
        finally:
            if prior is None:
    ...
    def test_forecast_persistence_close_with_poor_rr_carries_close_review(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            decision = _decision(root, long_score=160, short_score=120)
            snapshot = _snapshot(
            ...
            self.assertEqual(result.positions[0].close_review_action, ACTION_REVIEW_EXIT)
            report = (root / "pm.md").read_text()
            self.assertIn("forecast_persistence RECOMMEND_CLOSE", report)
            self.assertIn("remaining reward/risk", report)
            self.assertIn("loss-side market close still requires GPT CLOSE Gate A/B", report)

    def test_forecast_persistence_close_keeps_good_rr_hold(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            decision = _decision(root, long_score=160, short_score=120)
            snapshot = _snapshot(
    ...
    def test_operator_manual_auto_tp_modify_false_blocks_missing_tp_repair(self) -> None:
        prior = os.environ.get("QR_ENABLE_MISSING_TP_REPAIR")
        os.environ["QR_ENABLE_MISSING_TP_REPAIR"] = "1"
        try:
            with tempfile.TemporaryDirectory() as tmp:
                ...
                self.assertNotIn("manual/tagless TP repair candidate", report)
        finally:
            if prior is None:
                os.environ.pop("QR_ENABLE_MISSING_TP_REPAIR", None)
            else:
    ...
    def test_operator_manual_auto_tp_modify_false_keeps_existing_tp_unchanged(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            decision = _decision(root, long_score=120, short_score=160)
            snapshot = BrokerSnapshot(
            ...
            self.assertIsNone(managed.recommended_stop_loss)
            self.assertIsNone(managed.recommended_take_profit)
            self.assertIn("auto_tp_modify_allowed=false", (root / "pm.md").read_text())
