...
def test_signal_extrema_reversal_blocks_positive_gap_short_under_setup_pressure(
    ...
    monkeypatch.setattr(worker, "EXTREMA_SHORT_ENABLED", True)
    monkeypatch.setattr(worker, "EXTREMA_LONG_ENABLED", True)
    monkeypatch.setattr(worker, "EXTREMA_HIGH_BAND_PIPS", 1.0)
    monkeypatch.setattr(worker, "EXTREMA_RSI_SHORT_MIN", 54.0)
    monkeypatch.setattr(worker, "EXTREMA_SHORT_SUPPORT_ENABLED", False)
    monkeypatch.setattr(worker, "EXTREMA_SHORT_COUNTERTREND_GAP_BLOCK_PIPS", 0.0)
    monkeypatch.setattr(worker, "EXTREMA_SHORT_SHALLOW_PROBE_DIST_HIGH_MAX_PIPS", 0.0)
    monkeypatch.setattr(worker, "EXTREMA_SHORT_MID_RSI_PROBE_DIST_HIGH_MAX_PIPS", 0.0)
    monkeypatch.setattr(worker, "EXTREMA_SHORT_DRIFT_PROBE_DIST_HIGH_MAX_PIPS", 0.0)
    monkeypatch.setattr(worker, "EXTREMA_SHORT_SETUP_PRESSURE_DIST_HIGH_MAX_PIPS", 0.0)
    monkeypatch.setattr(worker, "EXTREMA_SWEEP_MIN_PIPS", 0.06)
    ...
    monkeypatch.setattr(
        worker,
        "get_candles_snapshot",
        lambda *_args, **_kwargs: [{"high": 158.457, "low": 158.380}] * 80,
    )
    monkeypatch.setattr(
        worker,
        "compute_range_snapshot",
        lambda *_args, **_kwargs: SimpleNamespace(high=158.457, low=158.380),
    )
    monkeypatch.setattr(
        worker,
        "tick_snapshot",
...
def test_signal_extrema_reversal_keeps_more_extended_positive_gap_short_under_setup_pressure(
    monkeypatch,
):
    monkeypatch.setattr(worker, "EXTREMA_ALLOWED_REGIMES", set())
    monkeypatch.setattr(worker, "EXTREMA_SPREAD_P25_MAX", 0.0)
    ...
    monkeypatch.setattr(
        worker,
        "get_candles_snapshot",
        lambda *_args, **_kwargs: [{"high": 158.4555, "low": 158.380}] * 80,
    )
    monkeypatch.setattr(
        worker,
        "compute_range_snapshot",
        lambda *_args, **_kwargs: SimpleNamespace(high=158.4555, low=158.380),
    )
    monkeypatch.setattr(
        worker,
        "tick_snapshot",
    ...
    assert signal is not None
    assert signal["action"] == "OPEN_SHORT"
    assert signal["extrema"]["short_positive_gap_probe_block"] is False


def test_signal_extrema_reversal_blocks_long_under_recent_setup_pressure(monkeypatch):
    monkeypatch.setattr(worker, "EXTREMA_ALLOWED_REGIMES", set())
    monkeypatch.setattr(worker, "EXTREMA_SPREAD_P25_MAX", 0.0)
    monkeypatch.setattr(worker, "EXTREMA_ADX_MAX", 35.0)
    monkeypatch.setattr(worker, "EXTREMA_ATR_MAX", 0.0)
    ...
    monkeypatch.setattr(worker, "EXTREMA_LONG_SETUP_PRESSURE_MIN_TRADES", 6)
    ...
    monkeypatch.setattr(worker, "EXTREMA_LONG_SETUP_PRESSURE_DIST_LOW_MAX_PIPS", 0.90)
    monkeypatch.setattr(worker, "EXTREMA_LONG_SETUP_PRESSURE_BOUNCE_MAX_PIPS", 0.35)
    monkeypatch.setattr(worker, "EXTREMA_LONG_SETUP_PRESSURE_TICK_STRENGTH_MAX", 0.30)
    monkeypatch.setattr(worker, "EXTREMA_LONG_SETUP_PRESSURE_MA_GAP_MAX_PIPS", 0.0)
    monkeypatch.setattr(worker, "EXTREMA_LONG_SETUP_PRESSURE_RANGE_SCORE_MIN", 0.45)
    monkeypatch.setattr(worker, "EXTREMA_LONG_SETUP_PRESSURE_RANGE_SCORE_MAX", 0.55)
    monkeypatch.setattr(worker, "EXTREMA_LONG_SETUP_PRESSURE_ADX_MAX", 23.0)
    monkeypatch.setattr(worker, "_latest_price", lambda *_args, **_kwargs: 158.450)
    monkeypatch.setattr(worker, "_atr_pips", lambda *_args, **_kwargs: 1.8)
    monkeypatch.setattr(
        worker,
        "get_candles_snapshot",
        lambda *_args, **_kwargs: [{"high": 158.470, "low": 158.447}] * 80,
    )
    monkeypatch.setattr(
        worker,
        "compute_range_snapshot",
        lambda *_args, **_kwargs: SimpleNamespace(high=158.470, low=158.447),
    )
    monkeypatch.setattr(
        worker,
        "tick_snapshot",
...
def test_signal_extrema_reversal_keeps_stronger_long_even_under_setup_pressure(
    ...
    assert signal is not None
    assert signal["action"] == "OPEN_LONG"
    assert signal["extrema"]["long_setup_pressure_block"] is False
...
def test_signal_extrema_reversal_blocks_neutral_gap_long_under_setup_pressure(
    ...
    monkeypatch.setattr(worker, "EXTREMA_LONG_SETUP_PRESSURE_BOUNCE_MAX_PIPS", 0.35)
    monkeypatch.setattr(worker, "EXTREMA_LONG_SETUP_PRESSURE_TICK_STRENGTH_MAX", 0.30)
    monkeypatch.setattr(worker, "EXTREMA_LONG_SETUP_PRESSURE_MA_GAP_MAX_PIPS", 0.10)
    monkeypatch.setattr(worker, "EXTREMA_LONG_SETUP_PRESSURE_RANGE_SCORE_MIN", 0.45)
    monkeypatch.setattr(worker, "EXTREMA_LONG_SETUP_PRESSURE_RANGE_SCORE_MAX", 0.60)
    monkeypatch.setattr(worker, "EXTREMA_LONG_SETUP_PRESSURE_ADX_MAX", 23.0)
    monkeypatch.setattr(worker, "_latest_price", lambda *_args, **_kwargs: 158.450)
    monkeypatch.setattr(worker, "_atr_pips", lambda *_args, **_kwargs: 1.8)
    monkeypatch.setattr(
        worker,
        "get_candles_snapshot",
        lambda *_args, **_kwargs: [{"high": 158.470, "low": 158.449}] * 80,
    )
    monkeypatch.setattr(
        worker,
        "compute_range_snapshot",
        lambda *_args, **_kwargs: SimpleNamespace(high=158.470, low=158.449),
    )
    monkeypatch.setattr(
        worker,
        "tick_snapshot",
...
def test_signal_extrema_reversal_keeps_bearish_gap_short_outside_drift_probe(
    ...
    monkeypatch.setattr(
        worker,
        "get_candles_snapshot",
        lambda *_args, **_kwargs: [{"high": 158.4585, "low": 158.370}] * 80,
    )
    monkeypatch.setattr(
        worker,
        "compute_range_snapshot",
        lambda *_args, **_kwargs: SimpleNamespace(high=158.4585, low=158.370),
    )
    monkeypatch.setattr(
        worker,
        "tick_snapshot",
...
def test_signal_extrema_reversal_blocks_non_supportive_long_drift_probe(monkeypatch):
    monkeypatch.setattr(worker, "EXTREMA_ALLOWED_REGIMES", set())
    monkeypatch.setattr(worker, "EXTREMA_SPREAD_P25_MAX", 0.0)
    monkeypatch.setattr(worker, "EXTREMA_ADX_MAX", 35.0)
    monkeypatch.setattr(worker, "EXTREMA_ATR_MAX", 0.0)
    ...
    monkeypatch.setattr(worker, "EXTREMA_LONG_SHALLOW_PROBE_DIST_LOW_MAX_PIPS", 0.0)
    monkeypatch.setattr(worker, "EXTREMA_LONG_MID_RSI_PROBE_DIST_LOW_MAX_PIPS", 0.0)
    monkeypatch.setattr(worker, "EXTREMA_LONG_DRIFT_PROBE_DIST_LOW_MAX_PIPS", 0.35)
    monkeypatch.setattr(worker, "EXTREMA_LONG_DRIFT_PROBE_BOUNCE_MAX_PIPS", 0.35)
    monkeypatch.setattr(worker, "EXTREMA_LONG_DRIFT_PROBE_TICK_STRENGTH_MAX", 0.25)
    monkeypatch.setattr(worker, "EXTREMA_LONG_DRIFT_PROBE_ADX_MIN", 24.0)
    monkeypatch.setattr(worker, "EXTREMA_LONG_DRIFT_PROBE_RANGE_SCORE_MIN", 0.38)
    monkeypatch.setattr(worker, "EXTREMA_LONG_DRIFT_PROBE_MA_GAP_MIN_PIPS", 0.15)
    monkeypatch.setattr(worker, "EXTREMA_LONG_DRIFT_PROBE_RSI_MIN", 36.0)
    monkeypatch.setattr(worker, "_latest_price", lambda *_args, **_kwargs: 158.450)
    monkeypatch.setattr(worker, "_atr_pips", lambda *_args, **_kwargs: 1.8)
    monkeypatch.setattr(
        worker,
        "get_candles_snapshot",
        lambda *_args, **_kwargs: [{"high": 158.470, "low": 158.4470}] * 80,
    )
    monkeypatch.setattr(
        worker,
        "compute_range_snapshot",
        lambda *_args, **_kwargs: SimpleNamespace(high=158.470, low=158.4470),
    )
    monkeypatch.setattr(
        worker,
        "tick_snapshot",
...
def test_signal_extrema_reversal_keeps_non_supportive_long_without_drift_probe_headwind(
    ...
    monkeypatch.setattr(
        worker,
        "get_candles_snapshot",
        lambda *_args, **_kwargs: [{"high": 158.470, "low": 158.4418}] * 80,
    )
    monkeypatch.setattr(
        worker,
        "compute_range_snapshot",
        lambda *_args, **_kwargs: SimpleNamespace(high=158.470, low=158.4418),
    )
    monkeypatch.setattr(
        worker,
        "tick_snapshot",
