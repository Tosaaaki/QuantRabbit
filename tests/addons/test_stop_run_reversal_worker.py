...
def _bars():
    ...
    for i in range(50):
        if i == 48:
            out.append(
                {"open": px, "high": px + 2.0, "low": px - 0.2, "close": px + 0.1}
            )
            px += 0.1
        elif i == 49:
            out.append(
                {"open": px, "high": px + 0.1, "low": px - 0.1, "close": px - 1.0}
            )
            px -= 1.0
        else:
...
def test_construct_ok():
    bars = _bars()
    w = StopRunReversalWorker(
        {**CFG, "universe": ["X"], "place_orders": False},
        broker=None,
        datafeed=DummyFeed(bars),
    )
    intents = w.run_once()
