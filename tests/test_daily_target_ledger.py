...
class DailyTargetLedgerTest(unittest.TestCase):
    ...
    def test_explicit_operator_manual_position_is_not_system_profitability(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            now = datetime.now(timezone.utc)
            snapshot = BrokerSnapshot(
                ...
                quotes={"USD_JPY": Quote("USD_JPY", 161.92, 161.93, timestamp_utc=now)},
                account=AccountSummary(
                    nav_jpy=237_700.0,
            ...
            summary = DailyTargetLedger(
                state_path=root / "target.json",
                report_path=root / "target.md",
            ).run(start_balance_jpy=250_000, daily_risk_budget_jpy=1_000, snapshot=snapshot)

            self.assertEqual(summary.status, "PURSUE_TARGET")
            self.assertEqual(summary.remaining_risk_budget_jpy, 1000.0)
            self.assertEqual(summary.progress_jpy, 0.0)
            payload = json.loads((root / "target.json").read_text())
            self.assertEqual(payload["unrealized_pl_jpy"], 0.0)
