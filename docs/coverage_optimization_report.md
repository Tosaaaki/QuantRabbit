...
- Generated at UTC: `2026-05-04T12:30:34.909101+00:00`
- Status: `COVERAGE_GAP`
...
- `failure_trader:AUD_JPY:LONG:BREAKOUT_FAILURE` status=`DRY_RUN_BLOCKED` reward=`7680` risk=`960` rr=`8.00` live_ready=`False` promotion_candidate=`False`
  - blocker: AUD_JPY quote is stale: 55.6s > 20s
- `failure_trader:EUR_JPY:LONG:BREAKOUT_FAILURE` status=`DRY_RUN_BLOCKED` reward=`1968` risk=`1008` rr=`1.95` live_ready=`False` promotion_candidate=`False`
  - blocker: EUR_JPY quote is stale: 55.1s > 20s
  - blocker: EUR_JPY spread 2.1pip exceeds 2.5x normal 0.8pip
- `failure_trader:EUR_USD:LONG:BREAKOUT_FAILURE` status=`DRY_RUN_BLOCKED` reward=`4758` risk=`1001` rr=`4.76` live_ready=`False` promotion_candidate=`False`
  - blocker: EUR_USD quote is stale: 56.2s > 20s
  - blocker: USD_JPY conversion quote is stale: 55.0s > 20s
- `failure_trader:EUR_USD:SHORT:BREAKOUT_FAILURE` status=`DRY_RUN_BLOCKED` reward=`6004` risk=`1001` rr=`6.00` live_ready=`False` promotion_candidate=`False`
  - blocker: EUR_USD quote is stale: 56.2s > 20s
  - blocker: USD_JPY conversion quote is stale: 55.0s > 20s
- `failure_trader:GBP_USD:LONG:BREAKOUT_FAILURE` status=`DRY_RUN_BLOCKED` reward=`2174` risk=`980` rr=`2.22` live_ready=`False` promotion_candidate=`False`
  - blocker: GBP_USD quote is stale: 55.2s > 20s
  - blocker: USD_JPY conversion quote is stale: 55.0s > 20s
...
- `range_trader:EUR_JPY:LONG:RANGE_ROTATION` status=`DRY_RUN_BLOCKED` reward=`1968` risk=`1008` rr=`1.95` live_ready=`False` promotion_candidate=`False`
  - blocker: EUR_JPY quote is stale: 55.1s > 20s
  - blocker: EUR_JPY spread 2.1pip exceeds 2.5x normal 0.8pip
- `range_trader:EUR_USD:LONG:RANGE_ROTATION` status=`DRY_RUN_BLOCKED` reward=`4758` risk=`1001` rr=`4.76` live_ready=`False` promotion_candidate=`False`
  - blocker: EUR_USD quote is stale: 56.2s > 20s
  - blocker: USD_JPY conversion quote is stale: 55.0s > 20s
- `range_trader:EUR_USD:SHORT:RANGE_ROTATION` status=`DRY_RUN_BLOCKED` reward=`6004` risk=`1001` rr=`6.00` live_ready=`False` promotion_candidate=`False`
  - blocker: EUR_USD quote is stale: 56.2s > 20s
  - blocker: USD_JPY conversion quote is stale: 55.0s > 20s
...
- `trend_trader:EUR_JPY:LONG:TREND_CONTINUATION` status=`DRY_RUN_BLOCKED` reward=`1968` risk=`1008` rr=`1.95` live_ready=`False` promotion_candidate=`False`
  - blocker: EUR_JPY quote is stale: 55.1s > 20s
  - blocker: EUR_JPY spread 2.1pip exceeds 2.5x normal 0.8pip
- `trend_trader:EUR_USD:LONG:TREND_CONTINUATION` status=`DRY_RUN_BLOCKED` reward=`4758` risk=`1001` rr=`4.76` live_ready=`False` promotion_candidate=`False`
  - blocker: EUR_USD quote is stale: 56.2s > 20s
  - blocker: USD_JPY conversion quote is stale: 55.0s > 20s
- `trend_trader:EUR_USD:SHORT:TREND_CONTINUATION` status=`DRY_RUN_BLOCKED` reward=`6004` risk=`1001` rr=`6.00` live_ready=`False` promotion_candidate=`False`
  - blocker: EUR_USD quote is stale: 56.2s > 20s
  - blocker: USD_JPY conversion quote is stale: 55.0s > 20s
