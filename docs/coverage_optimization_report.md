...
- Generated at UTC: `2026-05-04T22:39:24.409993+00:00`
- Status: `COVERAGE_GAP`
- Remaining target: `18441 JPY`
...
- live-ready reward misses remaining target by 18441 JPY
- even promoted dry-run reward misses remaining target by 18441 JPY
...
- `failure_trader:AUD_JPY:LONG:BREAKOUT_FAILURE` status=`DRY_RUN_BLOCKED` reward=`8064` risk=`1008` rr=`8.00` live_ready=`False` promotion_candidate=`False`
  - blocker: AUD_JPY quote is stale: 57.6s > 20s
...
- `failure_trader:EUR_JPY:LONG:BREAKOUT_FAILURE` status=`DRY_RUN_BLOCKED` reward=`1799` risk=`924` rr=`1.95` live_ready=`False` promotion_candidate=`False`
  - blocker: EUR_JPY quote is stale: 58.9s > 20s
...
- `failure_trader:EUR_USD:LONG:BREAKOUT_FAILURE` status=`DRY_RUN_BLOCKED` reward=`4660` risk=`981` rr=`4.75` live_ready=`False` promotion_candidate=`False`
  - blocker: EUR_USD quote is stale: 76.8s > 20s
  - blocker: USD_JPY conversion quote is stale: 51.5s > 20s
...
- `failure_trader:GBP_USD:LONG:BREAKOUT_FAILURE` status=`DRY_RUN_BLOCKED` reward=`2176` risk=`981` rr=`2.22` live_ready=`False` promotion_candidate=`False`
  - blocker: GBP_USD quote is stale: 70.7s > 20s
  - blocker: USD_JPY conversion quote is stale: 51.5s > 20s
