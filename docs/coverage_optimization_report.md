...
- Generated at UTC: `2026-05-04T15:54:27.269648+00:00`
- Status: `COVERAGE_GAP`
...
- `failure_trader:AUD_JPY:LONG:BREAKOUT_FAILURE` status=`DRY_RUN_BLOCKED` reward=`7680` risk=`960` rr=`8.00` live_ready=`False` promotion_candidate=`False`
  - blocker: AUD_JPY quote is stale: 51.8s > 20s
- `failure_trader:EUR_JPY:LONG:BREAKOUT_FAILURE` status=`DRY_RUN_BLOCKED` reward=`1968` risk=`1008` rr=`1.95` live_ready=`False` promotion_candidate=`False`
  - blocker: EUR_JPY quote is stale: 51.8s > 20s
...
- `failure_trader:EUR_USD:LONG:BREAKOUT_FAILURE` status=`DRY_RUN_BLOCKED` reward=`4830` risk=`1019` rr=`4.74` live_ready=`False` promotion_candidate=`False`
  - blocker: EUR_USD quote is stale: 51.9s > 20s
  - blocker: USD_JPY conversion quote is stale: 51.9s > 20s
- `failure_trader:EUR_USD:SHORT:BREAKOUT_FAILURE` status=`DRY_RUN_BLOCKED` reward=`6094` risk=`1019` rr=`5.98` live_ready=`False` promotion_candidate=`False`
  - blocker: EUR_USD quote is stale: 51.9s > 20s
  - blocker: USD_JPY conversion quote is stale: 51.9s > 20s
- `failure_trader:GBP_USD:LONG:BREAKOUT_FAILURE` status=`DRY_RUN_BLOCKED` reward=`2176` risk=`981` rr=`2.22` live_ready=`False` promotion_candidate=`False`
  - blocker: GBP_USD quote is stale: 51.7s > 20s
  - blocker: USD_JPY conversion quote is stale: 51.9s > 20s
...
- `range_trader:EUR_USD:LONG:RANGE_ROTATION` status=`DRY_RUN_BLOCKED` reward=`4830` risk=`1019` rr=`4.74` live_ready=`False` promotion_candidate=`False`
  - blocker: EUR_USD quote is stale: 51.9s > 20s
  - blocker: USD_JPY conversion quote is stale: 51.9s > 20s
- `range_trader:EUR_USD:SHORT:RANGE_ROTATION` status=`DRY_RUN_BLOCKED` reward=`6094` risk=`1019` rr=`5.98` live_ready=`False` promotion_candidate=`False`
  - blocker: EUR_USD quote is stale: 51.9s > 20s
  - blocker: USD_JPY conversion quote is stale: 51.9s > 20s
...
- `trend_trader:EUR_USD:LONG:TREND_CONTINUATION` status=`DRY_RUN_BLOCKED` reward=`4830` risk=`1019` rr=`4.74` live_ready=`False` promotion_candidate=`False`
  - blocker: EUR_USD quote is stale: 51.9s > 20s
  - blocker: USD_JPY conversion quote is stale: 51.9s > 20s
- `trend_trader:EUR_USD:SHORT:TREND_CONTINUATION` status=`DRY_RUN_BLOCKED` reward=`6094` risk=`1019` rr=`5.98` live_ready=`False` promotion_candidate=`False`
  - blocker: EUR_USD quote is stale: 51.9s > 20s
  - blocker: USD_JPY conversion quote is stale: 51.9s > 20s
