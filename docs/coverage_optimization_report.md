...
- Generated at UTC: `2026-05-04T14:29:27.496053+00:00`
- Status: `COVERAGE_GAP`
- Remaining target: `19605 JPY`
...
- live-ready reward misses remaining target by 19605 JPY
- even promoted dry-run reward misses remaining target by 19605 JPY
...
- `failure_trader:EUR_JPY:LONG:BREAKOUT_FAILURE` status=`DRY_RUN_BLOCKED` reward=`1899` risk=`972` rr=`1.95` live_ready=`False` promotion_candidate=`False`
  ...
  - blocker: open risk 1001 JPY + candidate risk 972 JPY exceeds portfolio cap 1051 JPY
- `failure_trader:EUR_USD:LONG:BREAKOUT_FAILURE` status=`DRY_RUN_BLOCKED` reward=`4656` risk=`980` rr=`4.75` live_ready=`False` promotion_candidate=`False`
  - blocker: EUR_USD quote is stale: 47.7s > 20s
  - blocker: USD_JPY conversion quote is stale: 46.8s > 20s
...
- `failure_trader:GBP_USD:LONG:BREAKOUT_FAILURE` status=`DRY_RUN_BLOCKED` reward=`2174` risk=`980` rr=`2.22` live_ready=`False` promotion_candidate=`False`
  - blocker: GBP_USD quote is stale: 46.3s > 20s
  - blocker: USD_JPY conversion quote is stale: 46.8s > 20s
  - blocker: open risk 1001 JPY + candidate risk 980 JPY exceeds portfolio cap 1051 JPY
