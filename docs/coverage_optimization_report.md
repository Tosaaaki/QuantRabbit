...
- Generated at UTC: `2026-05-04T20:10:32.850250+00:00`
- Status: `COVERAGE_GAP`
- Remaining target: `18788 JPY`
...
- live-ready reward misses remaining target by 18788 JPY
- even promoted dry-run reward misses remaining target by 18788 JPY
...
- `failure_trader:AUD_JPY:LONG:BREAKOUT_FAILURE` status=`DRY_RUN_BLOCKED` reward=`7680` risk=`960` rr=`8.00` live_ready=`False` promotion_candidate=`False`
  - blocker: AUD_JPY quote is stale: 51.5s > 20s
- `failure_trader:EUR_JPY:LONG:BREAKOUT_FAILURE` status=`DRY_RUN_BLOCKED` reward=`1872` risk=`960` rr=`1.95` live_ready=`False` promotion_candidate=`False`
  - blocker: EUR_JPY quote is stale: 51.6s > 20s
...
- `failure_trader:EUR_USD:LONG:BREAKOUT_FAILURE` status=`DRY_RUN_BLOCKED` reward=`4659` risk=`981` rr=`4.75` live_ready=`False` promotion_candidate=`False`
  - blocker: EUR_USD quote is stale: 46.8s > 20s
  - blocker: USD_JPY conversion quote is stale: 47.4s > 20s
