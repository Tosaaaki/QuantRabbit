...
- Generated at UTC: `2026-05-04T12:18:41.245156+00:00`
- Status: `COVERAGE_GAP`
...
- `failure_trader:AUD_JPY:LONG:BREAKOUT_FAILURE` status=`DRY_RUN_BLOCKED` reward=`7680` risk=`960` rr=`8.00` live_ready=`False` promotion_candidate=`False`
  - blocker: AUD_JPY quote is stale: 53.4s > 20s
- `failure_trader:EUR_JPY:LONG:BREAKOUT_FAILURE` status=`DRY_RUN_BLOCKED` reward=`1872` risk=`960` rr=`1.95` live_ready=`False` promotion_candidate=`False`
  - blocker: EUR_JPY quote is stale: 52.3s > 20s
  - blocker: EUR_JPY spread 2.0pip exceeds 2.5x normal 0.8pip
- `failure_trader:EUR_USD:LONG:BREAKOUT_FAILURE` status=`DRY_RUN_BLOCKED` reward=`4697` risk=`980` rr=`4.79` live_ready=`False` promotion_candidate=`False`
  - blocker: EUR_USD quote is stale: 53.2s > 20s
  - blocker: USD_JPY conversion quote is stale: 53.2s > 20s
- `failure_trader:EUR_USD:SHORT:BREAKOUT_FAILURE` status=`DRY_RUN_BLOCKED` reward=`5922` risk=`980` rr=`6.04` live_ready=`False` promotion_candidate=`False`
  - blocker: EUR_USD quote is stale: 53.2s > 20s
  - blocker: USD_JPY conversion quote is stale: 53.2s > 20s
...
- `range_trader:EUR_JPY:LONG:RANGE_ROTATION` status=`DRY_RUN_BLOCKED` reward=`1872` risk=`960` rr=`1.95` live_ready=`False` promotion_candidate=`False`
  - blocker: EUR_JPY quote is stale: 52.3s > 20s
  - blocker: EUR_JPY spread 2.0pip exceeds 2.5x normal 0.8pip
- `range_trader:EUR_USD:LONG:RANGE_ROTATION` status=`DRY_RUN_BLOCKED` reward=`4697` risk=`980` rr=`4.79` live_ready=`False` promotion_candidate=`False`
  - blocker: EUR_USD quote is stale: 53.2s > 20s
  - blocker: USD_JPY conversion quote is stale: 53.2s > 20s
- `range_trader:EUR_USD:SHORT:RANGE_ROTATION` status=`DRY_RUN_BLOCKED` reward=`5922` risk=`980` rr=`6.04` live_ready=`False` promotion_candidate=`False`
  - blocker: EUR_USD quote is stale: 53.2s > 20s
  - blocker: USD_JPY conversion quote is stale: 53.2s > 20s
...
- `trend_trader:EUR_JPY:LONG:TREND_CONTINUATION` status=`DRY_RUN_BLOCKED` reward=`1872` risk=`960` rr=`1.95` live_ready=`False` promotion_candidate=`False`
  - blocker: EUR_JPY quote is stale: 52.3s > 20s
  - blocker: EUR_JPY spread 2.0pip exceeds 2.5x normal 0.8pip
- `trend_trader:EUR_USD:LONG:TREND_CONTINUATION` status=`DRY_RUN_BLOCKED` reward=`4697` risk=`980` rr=`4.79` live_ready=`False` promotion_candidate=`False`
  - blocker: EUR_USD quote is stale: 53.2s > 20s
  - blocker: USD_JPY conversion quote is stale: 53.2s > 20s
- `trend_trader:EUR_USD:SHORT:TREND_CONTINUATION` status=`DRY_RUN_BLOCKED` reward=`5922` risk=`980` rr=`6.04` live_ready=`False` promotion_candidate=`False`
  - blocker: EUR_USD quote is stale: 53.2s > 20s
  - blocker: USD_JPY conversion quote is stale: 53.2s > 20s
