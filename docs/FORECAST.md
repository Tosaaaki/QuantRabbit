...
同日 2026-02-23 の追加改善（`dynamic_meta_rnd087`）では、
`rnd056` を基準に局所探索（`91候補→上位20候補を多窓再評価`）を行い、
`5m/10m` の hit と MAE を同時に押し上げる設定へ更新しました。
- 比較ファイル:
...
- 運用反映値:
  ...
  - `FORECAST_TECH_DYNAMIC_SESSION_BIAS_GAIN=0.22`
  - `FORECAST_TECH_DYNAMIC_SESSION_REGIME_GAIN=0.02`
- 同一スナップショットでの `rnd056` 比:
  - `24h`: `5m hit +0.003440 / mae -0.005085`, `10m hit +0.007991 / mae -0.010956`
  - `72h`: `5m hit +0.005579 / mae -0.005690`, `10m hit +0.008809 / mae -0.015130`
  - `full`: `5m hit +0.002847 / mae -0.003387`, `10m hit +0.004307 / mae -0.009587`
  - `1m` は hit が小幅悪化する窓があるが、MAE は概ね改善。

判定:
- `5m/10m` の精度（hit+MAE）を同時改善できるため採用。

同日 2026-02-24 の追加改善（`dynamic_meta_rnd090`）では、
`rnd087` を基準に `1m` 劣化ペナルティ付きで局所探索（`106候補→上位20候補多窓再評価`）を行い、
`5m/10m` の hit と MAE をさらに押し上げる設定を採用しました。
- 比較ファイル:
...
- 同一スナップショットでの `rnd087` 比:
  - `24h`: `5m hit +0.006881 / mae -0.006449`, `10m hit +0.023973 / mae -0.022077`
  - `72h`: `5m hit +0.006219 / mae -0.002442`, `10m hit +0.011427 / mae -0.008303`
  - `full`: `5m hit +0.003891 / mae -0.001912`, `10m hit +0.008765 / mae -0.007722`
  - `1m` は hit同等で MAE 小幅改善（`full mae -0.000185`）。

判定:
- `1m` を維持しながら `5m/10m` の hit と MAE を同時改善できるため採用。

同日 2026-02-24 の追加改善（`dynamic_meta_rnd081`）では、
`rnd090` を基準に `1m` 非劣化制約付きの多窓再探索（`121候補→上位20候補再評価`）を行い、
`5m/10m` の hit と MAE を同時に上積みしました。
- 比較ファイル:
...
- 同一スナップショットでの `rnd090` 比:
  - `24h`: `5m hit +0.005734 / mae -0.004041`, `10m hit +0.004566 / mae -0.009280`
  - `72h`: `5m hit +0.003601 / mae -0.001168`, `10m hit +0.001306 / mae -0.002970`
  - `full`: `5m hit +0.004041 / mae -0.001019`, `10m hit +0.001783 / mae -0.002777`
  - `1m` は hit 同等で MAE 改善（`24h -0.000038 / 72h -0.000193 / full -0.000142`）。
...
- 2026-02-24 追記（`scalp_ping_5s_b_live`）:
  ...
  - 2026-02-24 の再調整では、
    ...
    を維持しつつ、hard block は `SCALP_PING_5S_B_BLOCK_HOURS_JST=0,3,15,23`
    へ縮小して throughput を回復、残り時間帯は `PERF_GUARD_MODE=reduce`
    でロット縮小運用する。
