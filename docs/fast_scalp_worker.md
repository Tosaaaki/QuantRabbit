...
- サイジング: `allowed_lot` を使い pocket `scalp_fast` で上限チェックし、`MAX_LOT`/`MIN_UNITS`/`MAX_ACTIVE_TRADES`/`MAX_PER_DIRECTION` と StageTracker クールダウンを併用。既定は `ORDER_FIXED_SL_MODE=0` で物理SL無効、`ORDER_FIXED_SL_MODE=1` で TP+SLの物理SL付与へ切替。
