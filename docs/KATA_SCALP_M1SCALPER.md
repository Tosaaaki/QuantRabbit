...
- 実装は各 worker 内で `PositionManager.get_open_positions(include_unknown=False)` を参照し、各自の `strategy_tag` 単位で block する。
- `M1SCALP_FAIL_CLOSED_ON_POSITIONS_ERROR=1` を既定とし、`position_manager` stale/busy/unavailable 時は fail-open しない。
- 監視ポイント:
  - `quant-scalp-trend-breakout.log` / `quant-scalp-pullback-continuation.log` に `open_trades_block` が出ること。
