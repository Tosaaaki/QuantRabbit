...
| Forecast Service | `workers/forecast/worker.py` / `workers/common/forecast_gate.py` / `quant-forecast.service` | `strategy_tag/pocket/side/units` を入力として予測判定（allow/reduce/block）を返却。`expected_pips` に加え `anchor/target/tp/sl/rr` メタを `order_manager`/`entry_intent_board` へ連携 |
