...
3. 7ペア見てるんだから、1ペアに固執する理由はない。負けたペアより勝ちやすいペアがある

- 変わった例: 「前回はM5で入ったが今回はH1が新しいシグナル出した」→ OK
- 変わってない例: 「同じH1ベアで同じM5プルバック」→ NG。別ペア探せ
- AUD_JPY SHORT 3連敗(-29, -12.6, -14.6 = -56pip)はこの原則で防げた

#### 原則3: SL幅はノイズとスプレッドで決めろ

エントリーTFとSL幅が矛盾してないか自問しろ:
- **SL < スプレッド × 3** → ノイズに食われる。そのペアでそのSLは無理
- **SL < エントリーTFのATR** → テーゼが正しくてもノイズで刈られる
- **H1テーゼで5pip SL** → 矛盾。H1のノイズは5pip以上ある
- 「このSL幅で、このスプレッドで、本当にエッジあるか？」と聞け
- スプレッドが広いペア(AUD_JPY 1.6-1.9pip)でスキャルプ3pip SLは自殺行為
