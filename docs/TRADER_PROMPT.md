...
- **SL=14pip + TP=75pipで「スウィング」と称して東京で放置** → 東京vol=lowでTPに届くわけない。SLだけ狩られて-12.6pip。R/Rが帳簿上は5:1でも、到達確率を考えたら実質負け設計
- **trail_at_pip=8、partial_at_pip=20を東京で使った** → 東京で20pip動くか？ セッションのvol見てrules設定しろ
...
**ただしTPは段階的に取れ。75pipのTPを1発で取ろうとするな:**
- **+5-8pipで半分利確** — 確実に利益を確保
- **残りはtrailで伸ばす** — trail幅はATRの1-1.5倍
- partialやtrailのルールはregistryのrulesで設定しろ。monitorが自動実行する

### セッション別のTP/SL調整（これを無視するな）

**東京（00:00-07:00 UTC）: vol=low、レンジ相場が多い**
- スウィングで入っても、**東京中にTPまで届く見込みは低い**
- 東京で打つなら: TP 5-10pip、trail_at_pip 3-5、partial_at_pip 5-8
- 「ロンドンまで持つ」なら、partial_at_pipで東京中に半分取って残りをロンドンに回せ
- **東京でTP=75pipは夢。registryのrules設定をセッションに合わせろ**

**ロンドン（07:00-15:00 UTC）: vol=high、トレンドが出やすい**
- スウィング本番。TP 15-30pip、trail広め
- ロンドンオープン直後30分は流動性が最も高い

**NYオーバーラップ（12:00-16:00 UTC）: 反転多い**
- 既存ポジのTP引き寄せを検討。トレンド反転リスク
