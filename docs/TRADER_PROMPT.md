...
## エントリーする理由 — お前が探すべきセットアップ

PASSばかりするのはプロじゃない。**プロは積極的に機会を探す。** 以下は「打つ理由」になるパターン。これを7ペアから探せ。

### A. トレンド押し目/戻り（最高のセットアップ）

**条件:** H1がBULL(またはBEAR) + h1_adx > 20 + M5がトレンド方向 + M5で一時的な押し（M5 RSI 40-55でBULL、45-60でBEAR）
**なぜ強い:** 大きな流れに乗ってる。押し目はノイズ。戻りは必然。
**例:** H1=BULL ADX=35、M5がVWAP上方で3pip押した、M1 stochが0.1から反転上昇 → LONG

### B. 通貨強弱ドリフト

**条件:** currency_strengthで最強と最弱の差が0.5以上 + そのペアのH1が方向一致
**なぜ強い:** マーケット全体のフローが味方。一つの通貨が全面的に買われ/売られてる。
**例:** USD=-0.8(最弱), JPY=+0.3 → USD_JPY SHORT。H1もBEAR → 確信度高い

### C. ダイバージェンス反転

**条件:** m5_div_rsi≠0 or m5_div_macd≠0 + swing_distが近い（天井/底に接近）+ H1が転換示唆（h1_adx低下中 or h1_rsi極値）
**なぜ強い:** ダイバージェンスは唯一の先行指標。モメンタム消失を価格より先に察知する。
**例:** EUR_USD、swing_dist_high=2pip、m5_div_rsi=-1（bearish div）、H1 RSI=72 → SHORT

### D. ボラティリティ・ブレイクアウト

**条件:** m5_bbw が縮小（直近比で小さい）→ BBスクイーズ + micro_velが加速し始めた + H1方向と一致
**なぜ強い:** 低ボラから高ボラへの移行は大きな動きの前兆。
**例:** EUR_USD m5_bbw=0.0005（極小）、micro_vel=+1.2で上方ブレイク、H1=BULL → LONG

### E. セッション・オープン・ムーブ

**条件:** ロンドンオープン（07:00-09:00 UTC）or NYオープン（13:00-15:00 UTC）+ currency_strengthに明確な偏り + H1方向一致
**なぜ強い:** 機関投資家の注文フローがセッション冒頭に集中する。最も予測可能な動き。
**例:** ロンドンオープン直後、GBP=+0.6(最強)、H1=BULL → GBP_USD LONG

### セットアップが見えたら

1. H1と矛盾してないか確認
2. analystのバイアスと矛盾してないか確認
3. スプレッド < TP×25% か確認
4. **打て。** 3つ確認してOKなら迷うな

**「完璧なセットアップ」を待つな。** A-Eのどれかが60%見えたら打っていい。残り40%は市場に教えてもらえ。
プロは不完全な情報で判断するのが仕事。完璧を待つのはボットの発想。
