...
現virtual sessionには全atomic quote batchを証明する連続 `ACCOUNT_MARK` がまだないため、
realized DDで代用せず `CONTINUOUS_MTM_EVIDENCE_INCOMPLETE` としてscoreなし・順位なしにする。
...
1. virtual sessionへ全quote batchの期待座標commitmentと、bot action後の連続 `ACCOUNT_MARK` を実装してscorerで再構築照合する。完了後に型付きbot factoryの初回TRAINとして、固定entryへの `FIXED / BREAKEVEN / ATR_TRAILING` を別グリッドで固定する。正式開始前に全candidate、窓、コスト、source、prompt/model/input/raw response、探索分母を封印する。
