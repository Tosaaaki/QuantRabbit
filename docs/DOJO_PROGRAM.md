...
| room-meta-01 anomaly admission | `RUNNER_INTEGRATED / DECISION_SCORER_AVAILABLE_NOT_FIXED_DENOMINATOR` | room-03の相対強度alphaから、極端momentum・急反転・volatility・spread/ATR・相関集中・通貨gross exposureによる採用判定を分離。exact-28同期済み完成H1だけで `ENTER_OK / HOLD / REDUCE_SIZE` と `0 / 0.25 / 0.5 / 1.0` を決定し、HOLDは枠を消費せず次のupstream順位へ進む。長期economic runnerへcapability-closed runtimeとして接続し、fractional units、HOLD/resize/次順位選択をhash-chain summaryへ固定する。独立decision scorerは1座標のsource由来snapshotからsealed upstream＋metaを再実行できるが、runnerの固定全座標関門とHOLD候補の仮想P/L scorerは未接続。official evidence対象外で、利益改善は未実測 |
...
全proposal batchとhidden candidate/HOLD chainを照合する。ただし固定全座標関門へ未接続で、HOLD候補の仮想P/Lも再実行しないため、
`SELF_ATTESTED_UNVERIFIED_DIAGNOSTIC`を越えず、正負のedge証拠ではない。
実planと同じ非alphabet順exact-28、156固定座標のtiny M5 integrationは全座標完走したが、これはrunner接続試験であって
相場稽古ではない。現行long source manifestはmissing-slot/calendar-open legitimacyが未証明のため、実期間replayはなおfail closedする。
...
5. `room-meta-01`の純粋controllerとfractional-unit adapterは長期economic runnerへ接続済み。1座標decision scorerも実装済み。次はこれをrunnerの固定全座標関門へ接続し、HOLD候補を別accountで満期まで追う仮想P/L scorerを追加してから、base、momentum veto、reversal veto、volatility/correlation sizing、combined admissionを同じupstream signal列・固定policy・固定分母で走らせる。それまでは利益候補としてrankしない。
