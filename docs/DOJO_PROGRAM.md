...
| room-meta-01 anomaly admission | `RUNNER_INTEGRATED / INDEPENDENT_COUNTERFACTUAL_SCORER_PENDING` | room-03の相対強度alphaから、極端momentum・急反転・volatility・spread/ATR・相関集中・通貨gross exposureによる採用判定を分離。exact-28同期済み完成H1だけで `ENTER_OK / HOLD / REDUCE_SIZE` と `0 / 0.25 / 0.5 / 1.0` を決定し、HOLDは枠を消費せず次のupstream順位へ進む。長期economic runnerへcapability-closed runtimeとして接続し、fractional units、HOLD/resize/次順位選択をhash-chain summaryへ固定する。ただし独立scorerが全HOLD反実仮想をsourceから再構築するまではofficial evidence対象外で、利益改善は未実測 |
...
変更する。全候補、HOLD、resize、次順位選択はruntime hash chainへ残す。ただし独立scorerがsource quoteと上流proposalから
全counterfactualを再構築する部分は未接続のため、`SELF_ATTESTED_UNVERIFIED_DIAGNOSTIC`を越えず、正負のedge証拠ではない。
...
5. `room-meta-01`の純粋controllerとfractional-unit adapterは長期economic runnerへ接続済み。次はbase、momentum veto、reversal veto、volatility/correlation sizing、combined admissionを同じupstream signal列・固定policy・固定分母で走らせる前に、HOLDした候補のcounterfactualと次順位採用をsource quoteから独立再構築するtranscript scorerを接続する。それまでは利益候補としてrankしない。
