...
| AI prompt phase-1 v2 | `COLLECTING_SOURCE / HYPOTHESIS` | 30 cutoff・90固定cellとA/B/Cを事前封印。source/response/truth/score lifecycleと24時間後の市場truth採点器は実装済みだが、0/30日、0/90応答、truth 0。モデル実行・provider identity attestationは未接続 |
...
90 truth scoreから固定分母phase scoreを導出する。市場truth取得・answer key生成・採点は実装済みだが、CLI自体は
モデルを呼ばない。`seal-response` へ渡すJSONだけではgpt-5.5実行、provider identity、fresh-context隔離を証明できないため、
モデルexecutorとattestationが未接続の現状ではAI試験を実行済みとは数えない。
...
1. worker v2 smokeの各UTC日を順番どおり、当日終了後12時間以内にsource receiptとday sealをappend-only封印する。
2. AI phase-1 v2へprovider identityとfresh-contextを証明するモデルexecutorを接続し、固定済み30日・90セルを採取する。
3. 各response封印後かつ24時間horizon成熟後にmarket truthを採取し、欠測を落とさずA/B/C別NAVとpaired contrastを採点する。
4. `[2026-07-20, 2026-08-03)` 終了後にworker v2を全12候補×両intrabarの固定分母でderived executionする。
   この14日窓はsmoke/pilotであり、3ヶ月・worker 60 active days・AI 90 active daysの証明閾値を満たさない。
5. 経済event replayと外部witness、V2推移的invalidity registryを接続し、self-attested ledger/scoreを証拠へ昇格させない。
6. 同じ固定候補を事前宣言した継続窓で必要日数まで採取し、途中結果で候補や評価器を変えない。
7. 外部検証済み独立laneが増えるまでportfolioは `3X_NOT_REACHABLE` を維持し、実弾昇段を行わない。
