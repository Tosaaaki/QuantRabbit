...
| room-meta-01 anomaly admission | `IMPLEMENTED / PREREGISTERED_NOT_EXECUTED` | room-03の相対強度alphaから、極端momentum・急反転・volatility・spread/ATR・相関集中・通貨gross exposureによる採用判定を分離。exact-28同期済み完成H1だけで `ENTER_OK / HOLD / REDUCE_SIZE` と `0 / 0.25 / 0.5 / 1.0` を決定し、HOLDは枠を消費せず次のupstream順位へ進む。runner未接続のため利益改善は未実測 |
...
CVaR、ruin、margin closeout、機会損失、資金稼働率を同時に要求する。現時点のallocation出力は
`SELF_ATTESTED_UNVERIFIED_DIAGNOSTIC`で、長期runner・独立scorerへ未接続のため正負のedge証拠ではない。
