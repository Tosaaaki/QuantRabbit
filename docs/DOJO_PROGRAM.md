...
| 疎同期M1/M5 source | `OBSERVED-ONLY REPLAY READY / CORPUS RUN PENDING` | raw file SHAと取得receiptを再認証し、各通貨の観測epoch和集合をmaterializeする。未観測pairをworker・SL/TP・指値・CLOSEの実行quoteに使わず、synthetic/carry-forward約定は0。内部MTMだけは最後の実markを使い、全open pairがfreshでないmargin closeoutはfail closedする。V1同期sourceは後方互換のまま。tiny integrationと独立economic再実行は通過したが、長期corpusの再材料化・arm稽古は未実施 |
