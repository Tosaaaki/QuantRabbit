...
| 5通貨・4戦略 first wave | `SEALED TRAIN / RESULT UNOPENED` | ユーザー指定の戦略多様化を先に診断する追加TRAIN。2025-06、20万円、compression/daily-break/range-fade/spike-fadeを全てFIXEDで固定し、4候補×4セル×本体+5 LOPO=96 replayを封印。study SHA `bacf5389b53cf1c1077aa867d8ae8c145df74101b3b94c4f171f65806304f1d7`。exit overlay比較の代替ではなく、その前段のentry-family基準線である。 |
...
| room-meta-01 anomaly admission | `FIXED-DENOMINATOR READY / ARM RUNS PENDING` | room-03の相対強度alphaから、極端momentum・急反転・volatility・spread/ATR・相関集中・通貨gross exposureによる採用判定を分離。exact-28同期済み完成H1だけで `ENTER_OK / HOLD / REDUCE_SIZE` と `0 / 0.25 / 0.5 / 1.0` を決定する。長期economic runnerの156全座標を別processでdecision再実行し、1座標欠落も拒否する固定分母関門まで接続済み。HOLDの経済効果は、見送った1件の事後価格ではなく、同じsource・upstream・policy・全座標のBASE口座とarm口座を独立完走させるpaired portfolio scorerで比較する。scorer実装は完了したが長期arm実走は未実施で、利益改善・月3倍は未実測 |
| 2025H1 G2六か月稽古 | `WORN TRAIN / RUNNING` | 2020始点の全期間計画とは別に、現在相場へ近い `2025-01〜2025-06` を固定した迅速TRAIN profileを新設。6戦略・28通貨、独立月、OHLC/OLHC、BASE/STRESSのmain口座を36 job・816セルへ封印した。毎月のM5は2セルでscreenし、main結果内のfamily/pair/currency attributionで候補を診断する。全LOPO・連続口座・M1精査は生存候補のfull profileで課す。六か月完了時だけAI trainerを開き、parameter変更は新generationへ送る。2025は摩耗済みなのでholdout/proofには数えない |
| 疎同期M1/M5 source | `AUTHENTICATED OBSERVED-ONLY / TRAIN RUNNING` | raw file SHAと取得完了receiptを再認証し、各通貨の観測epoch和集合をmaterializeする。未観測pairをworker・SL/TP・指値・CLOSEの実行quoteに使わず、synthetic/carry-forward約定は0。内部MTMだけは最後の実markを使い、全open pairがfreshでないmargin closeoutはfail closedする。観測行の取得完全性と全calendar slotのquote完全性を別フラグに分離し、後者は未証明のままなのでofficial evidence/promotion対象外。実corpusの2025H1 TRAINを開始した |
...
training-room registry V1の10室は同一SHAのまま保持し、V2でroom-03をalpha専任へ改訂して
room-meta-01を追加した。`dojo_anomaly_admission_controller.py` V1は `room-meta-01` の純粋な採用判定層である。upstream botが固定した
candidate順位・pair・side・full-size unitsを変更せず、exact-28の同期済み完成H1 bid/askからmomentum z-score、
反転shock、短長volatility比、spread/ATR、pairwise absolute correlationを再計算し、判断時点以前の通貨別gross
exposureを加えて倍率を絞る。HOLDはseatを消費せず次順位を検査し、既に選んだpairとの相関が事前固定上限以上なら
次の非相関候補へ進む。room-03はalpha生成、room-meta-01はdirection-neutral risk filter、room-ai-01は
1判断=1 fresh contextの出口・資金解放と分離する。比較armはbase、momentum veto、reversal veto、
volatility/correlation sizing、combined admission、AI exit/capital releaseで、評価指標は全コスト後利益、連続MTM DD、
CVaR、ruin、margin closeout、機会損失、資金稼働率を同時に要求する。allocation出力は長期economic
runnerへ接続済みで、gap時にH1履歴をresetし、上流のpair・side・vehicle・TP/SL・期限を変えず、採用とunitsだけを
変更する。全候補、HOLD、resize、次順位選択はruntime hash chainへ残す。独立decision scorerは、独立economic再実行済み
V1 transcript、job、runtime seal、前月carry、runner summaryを相互束縛し、sealed upstream bot＋metaをsnapshot列から再生して
全proposal batchとhidden candidate/HOLD chainを照合する。固定全156座標を一つの別process requestへ束縛し、1座標でも
transcript・economic attestation・carry・runtime summaryが欠ければ全体を拒否する。HOLDの経済効果は
`dojo_anomaly_portfolio_counterfactual.py` がBASEとcandidate armの独立完走口座を同じjob/source/batch chain/upstream/policyへ
co-bindし、全コスト後equity、DD、margin拘束時間、reject、closeout、ruinを座標ごとに比較する。感度座標の損益を合算した
架空利益は出さない。実planと同じ非alphabet順exact-28、156固定座標のtiny M5 integrationは全座標完走したが、これは
runner/scorer接続試験であって相場稽古ではない。長期BASE/各armのpaired実走が0なので、正負のedge証拠ではない。

同期を仮定できない実M1/M5向けには、raw parent fileをSHAと取得receiptで再認証した後、観測epochの和集合だけを
source slice V2へmaterializeする。各epochでfreshなpairだけをworkerとexecutionへ渡し、未観測pairのcarry-forward quoteで
SL/TP・指値・CLOSE・新規entryを発火させない。口座の非実行MTMは最後の実markを保持するが、全open pairがfreshでない
margin closeoutは安全側で停止する。これにより欠測を同期済みに偽装せず長期稽古へ進める経路はできたが、production corpusの
疎source receipt材料化と長期arm実走はまだ行っていない。
...
5. `room-meta-01`のcontroller、fractional-unit adapter、156全座標decision関門、BASE対armのpaired portfolio scorerは実装済み。次はproduction corpusをobserved-only疎sourceへ一度materializeし、base、momentum veto、reversal veto、volatility/correlation sizing、combined admissionを同じupstream signal列・固定policy・固定分母で独立完走させる。それまでは利益候補としてrankしない。
6. 資金占有3案の比較は完了。次は5x基礎分割へ信頼度順に未使用余力を動的上乗せし、60分stale解放を組み合わせる。全candidate intent、pending影余力、通貨因子、拒否signalを順序非依存allocatorで記録する。
7. `KEEPならfull HOLD / FLATならrecycle` の固定TRAIN v2もreserveに劣後したため棄却する。次のAI裁量はローソク方向当てを増やさず、公開時刻を封印できるニュース/カレンダー/フローか、出口判断の独立前向き課題へ限定する。
8. 旧TRAIN生存3候補の別regime固定追試は生存0で完了。pullbackは凍結し、tailguardは換算source同期の契約修理後も同じ固定設定・固定分母でだけ再試行する。
9. worker v2 smokeの各UTC日を順番どおり、当日終了後12時間以内にsource receiptとday sealをappend-only封印する。
10. AI entryの前向き版は、ニュース・経済カレンダー・フローの公開時刻とcutoff時点bytesを封印できる因果的multi-source契約を作ってから別experimentとして開始する。
11. 各response封印後かつhorizon成熟後にmarket truthを採取し、欠測を落とさず固定分母でpaired contrastを採点する。
12. `[2026-07-20, 2026-08-03)` 終了後にworker v2を全12候補×両intrabarの固定分母でderived executionする。
   この14日窓はsmoke/pilotであり、3ヶ月・worker 60 active days・AI 90 active daysの証明閾値を満たさない。
13. worker経済event replayをsource coverage・pinned bot actionへ接続し、両laneへ外部monotonic witnessを追加する。
14. 同じ固定候補を事前宣言した継続窓で必要日数まで採取し、途中結果で候補や評価器を変えない。
15. 外部検証済み独立laneが増えるまでportfolioは `3X_NOT_REACHABLE` を維持し、実弾昇段を行わない。
