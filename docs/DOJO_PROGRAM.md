...
| HOLD資金配分worker | `TRAIN / 3 POLICY SURVIVORS` | 同じ4通貨spike-fadeで480分full HOLD、5x分割、20x/60分解放を比較。悲観OLHCは順に `+¥90,672 / +¥43,704 / +¥166,991`。方向勝率は75–77%と近いが、60分解放が利益・DD・取引数でfull HOLDを上回った |
...
HOLDの機会損失は、同じ4通貨spike-fade signal列を三つの資金政策へ通して実売買した。artifactは
...
30日換算4.6101も摩耗TRAINの11日窓を換算した値で、月次3倍のproofではない。

同時刻candidateを資金効率順に記録する `dojo_portfolio_allocator.py` はread-only診断器として追加した。
open/pendingのgross margin、通貨shadow、owner別同時保有上限、candidate単体SL上限をfail closedで検査するが、
broker submit/fillへは未接続で注文権限を持たない。V1は円建て4ペアだけを許可し換算率を厳密に1.0へ固定する。
非JPY建ては独立検証可能な市場換算receiptができるまで拒否する。open/pending/candidateを合算したportfolio全体の
stop-loss riskと、既存建玉解放後を含む機会損失は未実装なので、現段階のreceiptを資金配分proofに使わない。
...
`research/registries/dojo_goal_board_20260719_2627b7fc34f64fe14fd5ab2415a400d3e783614640d631108350fe85a07427a8.json`。
...
1. round-number TP 2→3 ATRの別TRAIN追試は両経路で負となったため凍結し、同familyの後追いparameter探索を止める。
2. 資金占有3案の比較は完了。次は5x基礎分割へ信頼度順に未使用余力を動的上乗せし、60分stale解放を組み合わせる。全candidate intent、pending影余力、通貨因子、拒否signalを順序非依存allocatorで記録する。
3. AI資金配分は方向gateとallocationを分離し、`KEEPなら原則full HOLD / FLATならrecycle` を別TRAIN窓へ事前登録する。新規候補は既存の残存期待値を明確に上回る場合だけ分割を許す。
4. spike-fade tailguard 2候補と低頻度pullback A2をTRAINの別regimeへ移し、設定を途中変更せず再現性と通貨別寄与を測る。
5. worker v2 smokeの各UTC日を順番どおり、当日終了後12時間以内にsource receiptとday sealをappend-only封印する。
6. AI entryの前向き版は、ニュース・経済カレンダー・フローの公開時刻とcutoff時点bytesを封印できる因果的multi-source契約を作ってから別experimentとして開始する。
7. 各response封印後かつhorizon成熟後にmarket truthを採取し、欠測を落とさず固定分母でpaired contrastを採点する。
8. `[2026-07-20, 2026-08-03)` 終了後にworker v2を全12候補×両intrabarの固定分母でderived executionする。
   この14日窓はsmoke/pilotであり、3ヶ月・worker 60 active days・AI 90 active daysの証明閾値を満たさない。
9. worker経済event replayをsource coverage・pinned bot actionへ接続し、両laneへ外部monotonic witnessを追加する。
10. 同じ固定候補を事前宣言した継続窓で必要日数まで採取し、途中結果で候補や評価器を変えない。
11. 外部検証済み独立laneが増えるまでportfolioは `3X_NOT_REACHABLE` を維持し、実弾昇段を行わない。
