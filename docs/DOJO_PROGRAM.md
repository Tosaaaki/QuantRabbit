...
| AI prompt phase-1 v3 | `IMPLEMENTATION_ONLY / HYPOTHESIS` | 1 cell 1 fresh process、first-attempt transcript、失敗固定、推移的invalidity registryを実装済み。precommit/startはまだなく、0/30日、0/90実行 |
...
`research/registries/dojo_goal_board_20260719_f962517572391880bd2f25e0841aaefd206339e3e20b1e44e2defb268352fa93.json`。
...
2. AI phase-1 v3を最初のcutoff前にprecommit/startし、固定30日・90セルを一回ずつ採取する。
...
5. worker経済event replayをsource coverage・pinned bot actionへ接続し、両laneへ外部monotonic witnessを追加する。
