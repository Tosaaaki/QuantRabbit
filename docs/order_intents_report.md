...
- Generated at UTC: `2026-05-04T07:57:37.744756+00:00`
...
- `failure_trader:AUD_JPY:LONG:BREAKOUT_FAILURE` status=`LIVE_READY`
  ...
  - intent: `AUD_JPY LONG STOP-ENTRY` units=17000 entry=112.869 tp=113.829 sl=112.749
...
- `failure_trader:EUR_USD:LONG:BREAKOUT_FAILURE` status=`LIVE_READY`
  - note: Dry-run geometry built from current snapshot; live use still requires fresh snapshot at send time.
  - intent: `EUR_USD LONG STOP-ENTRY` units=27000 entry=1.17216 tp=1.17444 sl=1.17168
  - risk metrics: risk=`2033.1 JPY` reward=`9657.3 JPY` rr=`4.75` spread=`0.8pip`
- `failure_trader:EUR_USD:SHORT:BREAKOUT_FAILURE` status=`LIVE_READY`
  - note: Dry-run geometry built from current snapshot; live use still requires fresh snapshot at send time.
  - intent: `EUR_USD SHORT STOP-ENTRY` units=27000 entry=1.17168 tp=1.1688 sl=1.17216
  - risk metrics: risk=`2033.1 JPY` reward=`12198.7 JPY` rr=`6.00` spread=`0.8pip`
- `range_trader:AUD_JPY:LONG:RANGE_ROTATION` status=`LIVE_READY`
  - note: Dry-run geometry built from current snapshot; live use still requires fresh snapshot at send time.
  - intent: `AUD_JPY LONG LIMIT` units=17000 entry=112.769 tp=113.729 sl=112.649
  - risk metrics: risk=`2040.0 JPY` reward=`16320.0 JPY` rr=`8.00` spread=`2.0pip`
- `range_trader:EUR_USD:LONG:RANGE_ROTATION` status=`LIVE_READY`
  - note: Dry-run geometry built from current snapshot; live use still requires fresh snapshot at send time.
  - intent: `EUR_USD LONG LIMIT` units=27000 entry=1.17168 tp=1.17396 sl=1.1712
  - risk metrics: risk=`2033.1 JPY` reward=`9657.3 JPY` rr=`4.75` spread=`0.8pip`
- `range_trader:EUR_USD:SHORT:RANGE_ROTATION` status=`LIVE_READY`
  - note: Dry-run geometry built from current snapshot; live use still requires fresh snapshot at send time.
  - intent: `EUR_USD SHORT LIMIT` units=27000 entry=1.17216 tp=1.16928 sl=1.17264
  - risk metrics: risk=`2033.1 JPY` reward=`12198.7 JPY` rr=`6.00` spread=`0.8pip`
...
- `failure_trader:EUR_JPY:LONG:BREAKOUT_FAILURE` status=`DRY_RUN_BLOCKED`
  - note: Dry-run geometry built from current snapshot; live use still requires fresh snapshot at send time.
  - intent: `EUR_JPY LONG STOP-ENTRY` units=14000 entry=183.92 tp=184.201 sl=183.776
  - risk metrics: risk=`2016.0 JPY` reward=`3934.0 JPY` rr=`1.95` spread=`2.4pip`
...
- `failure_trader:GBP_USD:LONG:BREAKOUT_FAILURE` status=`LIVE_READY`
  - note: Dry-run geometry built from current snapshot; live use still requires fresh snapshot at send time.
  - intent: `GBP_USD LONG STOP-ENTRY` units=17000 entry=1.35708 tp=1.35881 sl=1.3563
  - risk metrics: risk=`2080.2 JPY` reward=`4613.7 JPY` rr=`2.22` spread=`1.3pip`
- `range_trader:EUR_JPY:LONG:RANGE_ROTATION` status=`DRY_RUN_BLOCKED`
  - note: Dry-run geometry built from current snapshot; live use still requires fresh snapshot at send time.
  - intent: `EUR_JPY LONG LIMIT` units=14000 entry=183.8 tp=184.081 sl=183.656
  - risk metrics: risk=`2016.0 JPY` reward=`3934.0 JPY` rr=`1.95` spread=`2.4pip`
  - risk BLOCK: SPREAD_TOO_WIDE EUR_JPY spread 2.4pip exceeds 2.5x normal 0.8pip
