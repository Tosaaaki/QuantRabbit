...
7. Prefer a `MARKET` variant for immediate participation when it is current `LIVE_READY`; pending entries are basket-counted by the gateway and are not blanket no-trade reasons. Exception: `BREAKOUT_FAILURE:MARKET` must be at the retest/rejection side of the M5/M15 box. For SHORT, if price has not broken lower, do not sell the lower half/support; wait for the bounce into upper-half resistance or use the pending trigger. For LONG, do not buy the upper half/resistance after an upside failure; wait for lower-half support or use the pending trigger.
