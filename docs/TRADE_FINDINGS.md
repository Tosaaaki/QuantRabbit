...
## 2026-03-05 / M1Scalper: quickshot hard-gate で signal を全drop（local V2）

- 症状:
  - `orders.db` の新規注文が `2026-03-05T05:53:30Z` 以降増えない（M1Scalper稼働中でも entry が出ない）。
...
- 原因（RCA）:
  ...
  - `M1SCALP_USDJPY_QUICKSHOT_REQUIRE_BREAKOUT_RETEST=1` 前提で quickshot 成立条件が厳しく、結果として signal をほぼ全drop していた。
