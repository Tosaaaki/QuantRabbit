...
- **Broker-side SL on new entries + precision filters (A/B/C/D/E, 2026-05-13).** The 2026-05-12T15:33 UTC mass-close incident (-22,980 JPY confirmed loss on 5 protected positions) showed that an honor-system Gate B (`operator_close_authorized` JSON field) is not a structural protection: a panic-close trader can self-set the field and the verifier passes. The structural fix is broker-side SL on every NEW entry, plus precision filters that stop the trader from chasing exhausted moves into the same drawdown pattern.
  - **A — broker-side initial SL + trailing.** When `QR_NEW_ENTRY_INITIAL_SL=1` is exported, `_oanda_order_request` attaches `stopLossOnFill` to every NEW entry order even under SL-free mode. The SL value is the intent's existing `intent.sl` (ATR-derived, market-anchored). `quant_rabbit.strategy.trailing_sl.apply_trailing_sls` and the `trailing-sl-update` CLI then tighten the broker SL when M15/M30/H1 prints a BOS/CHOCH against the position side (new SL = BOS price ± `TRAILING_SL_SPREAD_BUFFER_MULT (= 2.0) × current_spread`). The function **only touches positions that ALREADY have a broker SL**; SL-free positions (every trade open before 2026-05-13) are skipped by construction, so existing-position protection is mechanical, not honor-system. `QR_TRAILING_SL_FROM_TRADE_ID` further restricts trailing to trades opened at/after a given broker id.
...
- **Noise-resistant SL + session-aware spread + token-file Gate B (F/G/H/I/J, 2026-05-13).** Follow-on hardening after A/B/C/D/E:
  ...
  - **G — SL-free bootstrap auto-enables initial-SL on new entries.** `cli._SL_FREE_RUNTIME_DEFAULTS` now includes `"QR_NEW_ENTRY_INITIAL_SL": "1"`, so any live cycle that boots the SL-free runtime gets broker-attached SL on NEW entries by default. Existing positions remain SL-free (their `stop_loss is None` state is preserved by construction).
  - **H — Trailing SL wired into every live cycle.** `automation.AutoTradeCycle._maybe_apply_trailing_sls` calls `strategy.trailing_sl.apply_trailing_sls` after pending-entries pricing. The function only tightens, only touches positions that already have a broker SL, and is silenced by `QR_DISABLE_TRAILING_SL=1` for diagnostics.
...
- **Existing TP is dynamically rebalanced every cycle** (2026-05-13 update; previously "not moved"). The `tp-rebalance` CLI command (invoked from the trader skeleton between intent pricing and gateway execution) recomputes TP per open trader-owned position using `_market_derived_reward_risk(chart_context) × current_ATR`. When the new desired distance differs from the existing TP by ≥ `QR_TP_REBALANCE_HYSTERESIS_PIPS` (default 10), the broker's TP order is replaced. Invariants enforced by `strategy/tp_rebalancer.py`: (a) TP must stay on the correct side of entry (LONG above, SHORT below) so the rebalance cannot lock in a loss, (b) TP must stay ≥ `QR_TP_REBALANCE_MIN_TP_TO_MARKET` pips from current price so it cannot fire on the same tick, (c) TP distance is capped at `QR_TP_REBALANCE_MAX_DISTANCE_ATR × ATR` to bound greed, (d) SL is NEVER touched — SL-free positions remain SL-free. Manual / unknown-owner positions are skipped. Kill switch: `QR_DISABLE_TP_REBALANCE=1`. User directive 2026-05-13:「エントリー時だけではなく、途中で伸び縮みできるようにしてほしい。市況によって。それって当たり前だよね？」. The original "TP is not moved" rule was the conservative default; dynamic rebalancing replaces it because static entry-time TP ignores the live regime and produced the 5/7-5/13 small-TP-big-DD asymmetry (48 TP fills avg +¥497 vs 42 manual closes avg -¥1,209).
...
- **New-entry initial SL (F, 2026-05-13).** When `QR_NEW_ENTRY_INITIAL_SL=1` (default-on under the SL-free bootstrap, see `cli._SL_FREE_RUNTIME_DEFAULTS`), new-entry geometry is floored at `H4_ATR * QR_NEW_ENTRY_SL_H4_ATR_MULT` (default 1.5) and widened by a session-tag multiplier (`QR_NEW_ENTRY_SL_THIN_SESSION_MULT=1.3` for Tokyo/JP holiday, `QR_NEW_ENTRY_SL_OFF_HOURS_MULT=1.4` for off-hours). This makes broker-attached SL noise-resistant — the spec is "wide enough that retail flow can't tag it" — while still bounding tail risk. Existing trader-owned positions are not touched (PositionProtectionGateway path unchanged).
- **Trailing SL auto-application (H, 2026-05-13).** `autotrade-cycle` runs `apply_trailing_sls` after pending-entries pricing on every live cycle (`automation._maybe_apply_trailing_sls`). The function only tightens, never widens, and only applies to positions that already have a broker-side SL; positions with `stop_loss=None` (SL-free mode) are skipped. Disabled with `QR_DISABLE_TRAILING_SL=1` (off-by-default).
