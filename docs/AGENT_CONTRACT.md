...
- **SL-free profit-only break-even/profit-lock stop (2026-05-20).** `QR_TRADER_DISABLE_SL_REPAIR=1` still means missing SL is intentional and must not be "repaired" on underwater positions. Exception: a trader-owned SL-free position that is already profitable may receive `BREAK_EVEN_STOP` at the original entry price once executable profit pips clear the current spread. When profit also exceeds the wider market-derived micro-noise envelope (current M5 ATR and the current spread itself), the stop is placed behind the current executable price by that M5 ATR/spread envelope and clamped so it can never be worse than entry. This is not initial SL repair, not trailing SL, and not permission to place loss-side stops; it is a profit-to-flat/profit-lock escape hatch once MFE is large enough that a reversal should not be allowed to turn the trade red. Manual/tagless positions remain TP-only and must never receive this stop.
