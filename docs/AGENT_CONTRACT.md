...
- **Profit-side partial close is allowed and gated** (2026-05-15). `profit-partial-close --send --confirm-live` may close part of a trader-owned position only when it is already profitable and has crossed a new ATR-derived milestone. The fraction is market-derived via `dynamic_position_policy.partial_close_fraction`: higher-TF with the position keeps more runner; higher-TF against / choppy tape banks more. The command persists `data/profit_partial_close_state.json` after successful sends so the same trade/milestone cannot fire repeatedly. It never realizes a loss from adverse P/L and does not replace CLOSE Gate A/B for full closes. Kill switch: `QR_DISABLE_PROFIT_PARTIAL_CLOSE=1`.
