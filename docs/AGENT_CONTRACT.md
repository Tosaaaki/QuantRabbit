...
- **Bollinger rail continuation handling (2026-05-20; adaptive TP-progress gate added 2026-06-03).** When an already-profitable trader-owned SL-free position has a same-direction BB rail / oscillator setup — e.g. SHORT while M1/M5 price is at the upper Bollinger rail with StochRSI/Williams/MFI overbought, or LONG at the lower rail with oversold evidence — `PositionManager` must not shrink a valid opposite-rail TP just because the micro matrix says HARVEST. If the existing TP already sits at the opposite M5 BB rail, keep that TP and use only the profit-side BE/profit-lock stop. Outside the BB-rail exception, a HARVEST/NARROW contraction of an existing reachable broker TP must also clear the adaptive TP-progress gate (`QR_ADAPTIVE_TP_CONTRACTION_MIN_PROGRESS`, defaulting to the same 60% majority-of-target boundary as profit-lock) before replacing the TP. The only early-contraction exception is a stale-wide TP already beyond `QR_TP_REBALANCE_MAX_DISTANCE_ATR × operating ATR`, which is the same market-derived reachability cap used by `tp-rebalance`. This prevents a +1 pip micro-scalp from replacing a still-reachable +7 pip TP while preserving the stale-target repair that saved trades whose original TP was never realistic. This captures the operator's "BB upper, likely down" read: the rail is continuation evidence for the current SHORT, not a reason to close the short or pull the target prematurely.
