...
- **SL-free profit-only break-even/profit-lock stop (2026-05-20).** `QR_TRADER_DISABLE_SL_REPAIR=1` still means missing SL is intentional and must not be "repaired" on underwater or noise-bound positions. Exception: a trader-owned SL-free position that is already profitable may receive `BREAK_EVEN_STOP` at the original entry price or better only after executable profit pips clear a fresh market-derived micro-noise envelope: current spread plus fresh M5 ATR and quick M1/M5 realized range from recent candles inside the current M5 management window. Stale pair-chart volatility must not be used; if fresh quick volatility is unavailable, the stop is deferred rather than falling back to spread alone. When profit exceeds that noise envelope, the stop is placed behind the current executable price by the same envelope and clamped so it can never be worse than entry. This is not initial SL repair, not trailing SL, and not permission to place loss-side stops; it is a profit-to-flat/profit-lock escape hatch once MFE is large enough that a reversal should not be allowed to turn the trade red. Manual/tagless positions remain TP-only and must never receive this stop.
