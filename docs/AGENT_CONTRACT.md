...
- **Structural loss-cut permission (2026-05-15; clarified 2026-05-28).** The operator clarified that a justified stop-loss is allowed: "妥当な損切りならやっていい". This does not weaken Gate A/B or authorize panic closes. Under `QR_DISABLE_AUTO_CLOSE=1`, PositionManager may keep `ACTION_REVIEW_EXIT` executable only for trader-owned positions that have an `entry_thesis_ledger.jsonl` record written by the live entry gateway and whose adverse P/L is backed by a market-derived structural loss-cut reason: H1/H4 close-confirmed BOS/CHOCH against the side, or a multi-TF structural order-block break. Legacy/no-ledger trader-owned positions can still be loss-cut through the GPT CLOSE receipt path when Gate A thesis invalidation plus Gate B operator authorization both pass. Manual/tagless positions, missing-data repair exits, score-only contradictions, and margin-pressure concerns remain advisory and must not create a market close by themselves.
