...
- **Same-pair hedge margin uses longest-leg increment (`position_intent=HEDGE`, 2026-05-19).** On OANDA v20 hedging accounts, same-instrument opposite exposure is margined by the larger side, not by gross long + short notional. QuantRabbit must therefore size and gate explicit same-pair hedge intents on the broker-required margin increment: a 1k EUR/USD SHORT against an existing 22k EUR/USD LONG has `estimated_margin_jpy=0` until the short side exceeds the current long leg. This is a broker-spec constant, not a market-derived risk relaxation. The relaxation applies only when (i) `snapshot.account.hedging_enabled=true`, (ii) the intent explicitly carries `metadata['position_intent']='HEDGE'`, and (iii) the candidate is same-pair opposite-side exposure that does not exceed the currently uncovered opposite leg; non-hedge pyramids, over-hedges, and cross-pair entries still use normal margin headroom and cannot masquerade as HEDGE. `RiskEngine`, `intent_generator._risk_budgeted_units`, and `LiveOrderGateway` basket checks must all use the same incremental-margin value so a margin-full account can still place a margin-neutral protective hedge without falling to `MARGIN_TOO_THIN_FOR_MIN_LOT` or `MARGIN_UTILIZATION_CAP_EXCEEDED`.
