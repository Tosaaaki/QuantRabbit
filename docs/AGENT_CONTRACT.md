...
- **Bollinger rail continuation handling (2026-05-20).** When an already-profitable trader-owned SL-free position has a same-direction BB rail / oscillator setup — e.g. SHORT while M1/M5 price is at the upper Bollinger rail with StochRSI/Williams/MFI overbought, or LONG at the lower rail with oversold evidence — `PositionManager` must not shrink a valid opposite-rail TP just because the micro matrix says HARVEST. If the existing TP already sits at the opposite M5 BB rail, keep that TP and use only the profit-side BE/profit-lock stop. This captures the operator's "BB upper, likely down" read: the rail is continuation evidence for the current SHORT, not a reason to close the short or pull the target prematurely.
