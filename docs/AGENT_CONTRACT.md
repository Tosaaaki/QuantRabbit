...
- **Profit-side full market take is distinct from loss CLOSE (2026-05-20).** `PositionManager` may emit `TAKE_PROFIT_MARKET` for a trader-owned position only when broker truth says the position is currently profitable and the adaptive TP matrix sees macro reversal against the runner while the trade is in profit. `PositionProtectionGateway` maps this to a full broker close without Gate A/B because it realizes positive P/L and does not cut an underwater thesis. This path is forbidden for manual/tagless or external positions and blocked if the latest broker snapshot is not profitable. Loss-side `REVIEW_EXIT` remains governed by Gate A + Gate B below.
- **CLOSE discipline (two-gate, 2026-05-12, `feedback_no_unilateral_close.md`; same-cycle re-entry clarified 2026-06-04; Gate B split 2026-06-04).** A `CLOSE` receipt must satisfy Gate A plus the applicable Gate B or be rejected by `gpt-trader-decision`. A `TRADE` receipt must not list `close_trade_ids`: loss-cut first, then archive the accepted CLOSE receipt, rerun broker-snapshot / intent generation, and re-enter only if a fresh `LIVE_READY` lane survives a separate verified `TRADE` receipt. Automation may do that in the same outer autotrade cycle after the close has been sent or staged, but never in the same receipt / packet. This prevents "cut and re-enter" from becoming same-packet chase behavior against stale margin, stale quotes, or stale thesis evidence.
  - Fresh sidecar Gate A close evidence is priority work, not a protection-mode escape hatch. While `position_thesis_report.json`, `thesis_evolution_report.json`, `position_management.json`, or `forecast_persistence_report.json` carries current `REVIEW_CLOSE` / `RECOMMEND_CLOSE` / `REVIEW_EXIT` evidence for an open trader-owned position, `gpt-trader-decision` must reject `TRADE`, `WAIT`, `REQUEST_EVIDENCE`, `PROTECT`, and `TIGHTEN_SL` with either `CLOSE_OPERATOR_AUTH_REQUIRED` or `POSITION_CLOSE_REQUIRED`. This makes the close blocker loud instead of accepting a harmless-looking protection receipt that leaves the broken position open.
  ...
  - **Gate B — standing hard loss-cut authorization OR explicit operator authorization.** Hard Gate A paths satisfy the operator's standing instruction 「妥当な損切りならやっていい」 and do **not** need a fresh 5-minute token: M15/H4 close-confirmed BOS/CHOCH against the side, buffered `invalidation_price` hit with technical confirmation, fresh `thesis_evolution` `BROKEN` / `RECOMMEND_CLOSE`, fresh `position_management` `REVIEW_EXIT` whose reasons contain structural `loss-cut:` evidence (close-confirmed structural break or multi-TF structural OB break), or fresh `position_thesis` `REVIEW_CLOSE` that records adverse technical loss / invalidation-hit evidence plus M5/M15/M30/H1 technical confirmation for the same trade. Softer close evidence still requires explicit operator authorization from one of two operator-controlled channels: (i) `QR_OPERATOR_CLOSE_OVERRIDE=1` exported in the operator shell, or (ii) a fresh `data/.operator_close_token` file whose mtime is within `OPERATOR_CLOSE_TOKEN_FRESH_SECONDS` (5 minutes; see `gpt_trader.py`). Softer evidence includes `position_thesis` score-only `REVIEW_CLOSE`, non-structural `position_management` `REVIEW_EXIT`, `forecast_persistence` `RECOMMEND_CLOSE`, score-only reviews, or any close batch where at least one named trade lacks hard Gate A. The receipt's `operator_close_authorized: true` field is **advisory only** and is **not accepted as authorization**.
- The close execution layer must only call broker close for GPT summaries with `status=ACCEPTED`, `allowed=true`, and `action=CLOSE`. Rejected summaries, WAIT / TRADE / CANCEL / PROTECT receipts, and any `TRADE` receipt carrying `close_trade_ids` must not reach the broker close path.
