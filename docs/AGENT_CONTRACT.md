...
- **CLOSE discipline (two-gate, 2026-05-12, `feedback_no_unilateral_close.md`; re-entry split clarified 2026-05-29).** A `CLOSE` receipt must satisfy **both** of the following or be rejected by `gpt-trader-decision`. A `TRADE` receipt must not list `close_trade_ids`: loss-cut first, then rerun broker-snapshot / intent generation and re-enter only if a fresh `LIVE_READY` lane survives the next cycle. This prevents "cut and re-enter" from becoming same-packet chase behavior against stale margin, stale quotes, or stale thesis evidence.
