...
- **CLOSE discipline (two-gate, 2026-05-12, `feedback_no_unilateral_close.md`; same-cycle re-entry clarified 2026-06-04; Gate B split 2026-06-04).** A `CLOSE` receipt must satisfy Gate A plus the applicable Gate B or be rejected by `gpt-trader-decision`. A `TRADE` receipt must not list `close_trade_ids`: loss-cut first, then archive the accepted CLOSE receipt, rerun broker-snapshot / intent generation, and re-enter only if a fresh `LIVE_READY` lane survives a separate verified `TRADE` receipt. Automation may do that in the same outer autotrade cycle after the close has been sent or staged, but never in the same receipt / packet. This prevents "cut and re-enter" from becoming same-packet chase behavior against stale margin, stale quotes, or stale thesis evidence.
  ...
  - **Gate B — standing hard loss-cut authorization OR explicit operator authorization.** Hard Gate A paths satisfy the operator's standing instruction 「妥当な損切りならやっていい」 and do **not** need a fresh 5-minute token: M15/H4 close-confirmed BOS/CHOCH against the side, buffered `invalidation_price` hit with technical confirmation, or fresh `thesis_evolution` `BROKEN` / `RECOMMEND_CLOSE`. Softer close evidence still requires explicit operator authorization from one of two operator-controlled channels: (i) `QR_OPERATOR_CLOSE_OVERRIDE=1` exported in the operator shell, or (ii) a fresh `data/.operator_close_token` file whose mtime is within `OPERATOR_CLOSE_TOKEN_FRESH_SECONDS` (5 minutes; see `gpt_trader.py`). Softer evidence includes `position_thesis` `REVIEW_CLOSE`, `forecast_persistence` `RECOMMEND_CLOSE`, score-only reviews, or any close batch where at least one named trade lacks hard Gate A. The receipt's `operator_close_authorized: true` field is **advisory only** and is **not accepted as authorization**.
