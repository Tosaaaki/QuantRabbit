...
- **Risk caps must be equity-derived, and per-trade ≠ per-day.** The daily-target ledger (`start_balance × daily_risk_pct`) is the **whole-day** budget. The **per-trade** cap is `daily_risk_budget_jpy / target_trades_per_day` — this is what flows into `intent.metadata['max_loss_jpy']`. The split is mandatory: a single losing trade must not be able to exhaust the day's budget, because the campaign needs many attempts to reach the 10% target. `target_trades_per_day` is operator policy (currently `RiskPolicy.target_trades_per_day = 10`, documented per (a)/(b)/(c) below); override via `--target-trades-per-day` on `daily-target-state`. Never derive either cap from a JPY literal stored in code or in a stale state file.
...
- Historical worst loss is a ranking / sizing warning after current risk repair, not a second hard veto. Once the current `LIVE_READY` receipt fits `per_trade_risk_budget_jpy`, old loss memory may reduce size or priority but must not be the only reason to leave a target-open account flat.
