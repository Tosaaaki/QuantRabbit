...
- **SL-free profit-only break-even stop (2026-05-20).** `QR_TRADER_DISABLE_SL_REPAIR=1` still means missing SL is intentional and must not be "repaired" on underwater or noise-bound positions. Exception: a trader-owned SL-free position that is already profitable may receive `BREAK_EVEN_STOP` at the original entry price only after executable profit pips clear the market-derived micro-noise envelope: current M5 ATR and the current spread itself. This is not initial SL repair, not trailing SL, and not permission to place loss-side stops; it is a profit-to-flat escape hatch once MFE is large enough that a reversal should not be allowed to turn the trade red. Manual/tagless positions remain TP-only and must never receive this stop.
- **Bollinger rail continuation handling (2026-05-20).** When an already-profitable trader-owned SL-free position has a same-direction BB rail / oscillator setup — e.g. SHORT while M1/M5 price is at the upper Bollinger rail with StochRSI/Williams/MFI overbought, or LONG at the lower rail with oversold evidence — `PositionManager` must not shrink a valid opposite-rail TP just because the micro matrix says HARVEST. If the existing TP already sits at the opposite M5 BB rail, keep that TP and use only the profit-side BE stop. This captures the operator's "BB upper, likely down" read: the rail is continuation evidence for the current SHORT, not a reason to close the short or pull the target prematurely.
