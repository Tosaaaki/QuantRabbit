...
| `participation_alloc` | strategy-wide conversion に加え、`lane_scoreboard` の setup-scoped `promotion_gate / auto_quarantine` を override として runtime へ渡す。`2 fills + negative realized` の fresh loser lane も fast trim 対象に含む | `config/participation_alloc.json`, `scripts/participation_allocator.py` |
