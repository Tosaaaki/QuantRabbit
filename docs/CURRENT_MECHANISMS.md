...
| `lane_scoreboard` | current setup ごとの `promotion_gate / auto_quarantine` を明示する。winner lane は昇格、fresh/chronic loser lane は隔離の候補として出す | `logs/lane_scoreboard_latest.json`, `scripts/lane_scoreboard.py` |
| `participation_alloc` | strategy-wide conversion に加え、`lane_scoreboard` の setup-scoped `promotion_gate / auto_quarantine` を override として runtime へ渡す。`2 fills + negative realized` の fresh loser lane を fast trim 対象に含み、loser setup override が濃いのに top-level `hold` に残る戦略は mild `trim_units` へ昇格させる | `config/participation_alloc.json`, `scripts/participation_allocator.py` |
...
- `run_local_feedback_cycle`
  の既定は
  `entry_path_aggregator=6h lookback`
  と
  `lane_scoreboard=6h lookback / min_attempts=4 / setup_min_attempts=2 / max_units_cut=0.22 / max_units_boost=0.24 / max_probability_boost=0.10`
  と
...
- `participation_alloc`
  ...
  `boost_participation`
  へ上げる。
  あわせて current loser setup override が
  `quarantine`
  で固まっているのに top-level
  `hold`
  に残る戦略は、
  winner boost share が競り勝っていない場合だけ
  strategy-level mild trim へ昇格する。
  `dynamic_alloc`
...
- `strategies/micro/momentum_burst.py` は current local-v2 で、non-reaccel `MomentumBurst-open_long` の `range_mode=transition` を broad に止めず、`rsi>=66`, `atr_pips<=3.4`, strong `trend_snapshot` long の overbought lane にだけ `pullback` 条件を追加する。current candle の `close_pos<=0.72` と `upper_wick<=max(1.0p, atr*0.28)` を満たさない upper-wick chase は strategy-local に reject し、controlled pullback long や `trend_long` winner lane には広げない。
- `workers/micro_runtime/worker.py` を共有する dedicated micro worker
  （`momentumburst`, `levelreactor`, `trendretest`, `rangebreak`, `momentumstack`, `momentumpulse`, `trendmomentum`, `pullbackema`, `vwapbound`, `vwaprevert`, `compressionrevert`）
  は、
  `market_order()`
  が truthy な dispatch を返したときだけ
  strategy cooldown を更新する。
  `res=None`
  の pre-order reject / no-fill では cooldown を消費せず、
  live log には
  `cooldown_updated=0/1`
  を残す。
- 収益/リスク/ENTRY/EXIT 改善の着手前レビューは `scripts/change_preflight.sh "<strategy_tag or hypothesis_key or close_reason>"` を正とする。wrapper は `collect_local_health.sh` による local health refresh、`tick_cache/factor_cache/health_snapshot` を使った USD/JPY 市況要約、`trade_findings_review.py` による同系改善 review、`generate_repo_history_lane_index.py` による repeat-risk / reopen single-focus 候補をまとめて実行する。`utils.market_hours.is_market_open()` を使い、週末クローズや日次メンテ窓では `tick_stale/data_lag_high` を runtime fault ではなく `market_closed_hold` として出す。
- ユーザへ改善候補を返す直前は `scripts/improvement_preflight.sh "<query>" "<strategy::surface::primary_loss_driver::idea||...>"` を正とする。wrapper は `change_preflight.sh` を内包し、候補ごとに `market_hold / review_existing_pending / escalate_family_not_tighten / allow_new_lane` を返す。`allow_new_lane` 以外は新規 tweak として提案せず、pending 検証または family escalation として扱う。closed 窓では `market_closed:*_to_open` を理由に `market_hold_review_only` を返し、週明け reopen 前の stale cache を新規 runtime tweak の根拠に使わない。
- commit 前 enforcement は `.githooks/pre-commit` の `scripts/preflight_guard.py` が担う。`execution/`, `workers/`, `strategies/`, `analysis/`, 主要 risk config/env の staged 変更では、fresh `logs/change_preflight_latest.json` と staged `docs/TRADE_FINDINGS.md` に加えて、fresh `logs/improvement_gate_latest.json` を必須にする。latest `improvement_preflight` が `blocked=true` か `allow_new_lane` 以外の candidate を含む場合は commit を止め、`recommended_single_focus_lane` / blocked candidate を表示して pending 再検証へ戻す。
- `scripts/trade_findings_lint.py` は strict-since 以降の entry に required fields と `Hypothesis Key` の format を要求し、`scripts/trade_findings_index.py` は latest hypothesis index / unresolved list / dominant loss driver summary に加えて lane family / repeat-risk / reopen single-focus 候補を `logs/trade_findings_index_latest.{json,md}` へ出す。
- `scripts/improvement_gate.py` は `TRADE_FINDINGS` / `change_preflight_latest.json` / repo-history lane payload を横断し、改善候補 spec（`strategy::surface::primary_loss_driver::idea`）ごとに unresolved lane と `market_hold` を照合して `logs/improvement_gate_latest.{json,md}` を出す。same-surface だけでなく same-strategy の open trading lane も veto 対象とし、`Mechanism Fired=0/none` の unresolved が残る候補は `review_existing_pending` か `escalate_family_not_tighten` を返す。agent が「改善点」を口頭で返すだけのときも、この gate を通して anti-loop を運用導線へ落とす。
- `scripts/generate_repo_history_lane_index.py` は `TRADE_FINDINGS` の `Hypothesis Key` と repo history を cross-reference し、`docs/REPO_HISTORY_LANE_INDEX.md` と `logs/repo_history_lane_index_latest.{json,md}` を生成する。lane は `Hypothesis Key`、family は `strategy × Primary Loss Driver` として扱い、history commit count は subject/path の heuristic match を使う。current unresolved trading lane については `repeat_risk` と `recommended_single_focus_lane` も出し、週明けの最初の live 改善を 1 lane に固定する判断材料に使う。
- `scripts/trade_findings_review.py` と `scripts/trade_findings_lint.py` は current local-v2 の anti-loop governance も担う。`2026-03-14 10:00 JST` 以降の entry では `Why Not Same As Last Time / Promotion Gate / Escalation Trigger` を必須にし、禁止対象を `same parameter` ではなく `same hypothesis / same regime / same fingerprint` の焼き直しとして運用する。`Why Not Same As Last Time` は `setup_fingerprint / flow_regime / market regime / evaluation window` の差を書く field とする。
...
- `strategies/micro/momentum_burst.py`
  ...
  summary が出るので、
  pending lane の live 判定を blind tightening なしで進められる。

- `workers/micro_runtime/worker.py`
  の
  `_allowed_strategies()`
  は env 値ごとに allowlist 結果を cache する。
  dedicated runner の
  `allowlist applied`
  は worker 起動後に毎ループ出さず、
  `momentumburst_no_signal`
  のような診断ログを埋もれにくくする。
