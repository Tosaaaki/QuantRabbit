...
- `scripts/generate_repo_history_lane_index.py` は `TRADE_FINDINGS` の `Hypothesis Key` と repo history を cross-reference し、`docs/REPO_HISTORY_LANE_INDEX.md` と `logs/repo_history_lane_index_latest.json` を生成する。lane は `Hypothesis Key`、family は `strategy × Primary Loss Driver` として扱い、history commit count は subject/path の heuristic match を使う。
