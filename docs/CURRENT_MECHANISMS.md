...
- `scripts/improvement_gate.py` は `TRADE_FINDINGS` / `change_preflight_latest.json` を横断し、改善候補 spec（`strategy::surface::primary_loss_driver::idea`）ごとに unresolved lane と `market_hold` を照合して `logs/improvement_gate_latest.{json,md}` を出す。agent が「改善点」を口頭で返すだけのときも、この gate を通して anti-loop を運用導線へ落とす。
