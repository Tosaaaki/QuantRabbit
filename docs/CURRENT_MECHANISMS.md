...
- `scripts/trade_findings_review.py` と `scripts/trade_findings_lint.py` は current local-v2 の anti-loop governance も担う。`2026-03-14 10:00 JST` 以降の entry では `Why Not Same As Last Time / Promotion Gate / Escalation Trigger` を必須にし、same-lane tweak を `pending` のまま積み増さない前提で運用する。
