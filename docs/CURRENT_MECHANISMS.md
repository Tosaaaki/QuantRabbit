...
- 収益/リスク/ENTRY/EXIT 改善の着手前レビューは `python3 scripts/trade_findings_review.py --query "<strategy_tag or hypothesis_key or close_reason>"` を正とし、`docs/TRADE_FINDINGS.md` から同系改善の `Verdict / Primary Loss Driver / Mechanism Fired / Do Not Repeat Unless` を必ず先に読む。
