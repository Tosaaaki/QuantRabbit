...
- Read `data/hierarchical_bot_regime.json`, `data/fast_bot_shadow.json`, `data/fast_bot_scorecard.json`, and `docs/fast_bot_shadow_report.md` before tuning. The fast lane is deterministic and runs every 30 seconds: M1 executes, M5/M15/M30 define the operating state, H1/H4 define structure, and D is the anchor. AI does not approve each shadow signal. Review only when `ai_wake_required=true` from a material state/cost/performance event or when the six-hour tuning interval is due. A pair-level AI `GO`/`CAUTION`/`STOP` may only supervise; it cannot create a side/method, alter TP/SL, bypass stale/cost/Risk/gateway gates, or grant live permission. Fast-bot artifacts remain shadow-only until the exact S5 bid/ask scorecard has at least 100 fills, 10 active days, PF>=1.25, and a strictly positive one-sided 95% expectancy lower bound, followed by a separate content-addressed live-promotion implementation.
