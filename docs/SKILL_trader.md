...
- Read `data/hierarchical_bot_regime.json`, `data/fast_bot_shadow.json`, `data/fast_bot_scorecard.json`, and `docs/fast_bot_shadow_report.md` before tuning. The fast lane is deterministic and runs every 30 seconds: M1 executes, M5/M15/M30 define the operating state, H1/H4 define structure, and D is the anchor. AI does not approve each shadow signal. Review only when `ai_wake_required=true` from a material state/cost/performance event or when the six-hour tuning interval is due. A pair-level AI `GO`/`CAUTION`/`STOP` may only supervise; it cannot create a side/method, alter TP/SL, bypass stale/cost/Risk/gateway gates, or grant live permission. For schema-v2 signals, inspect the scorecard's precommitted passive-entry arms and paired pips deltas against `PASSIVE_NEAR_SIDE`; never choose an arm before 100 resolved paired signals over 10 active days, never reconstruct an arm after seeing its S5 path, and never treat `review_ready` as automatic parameter or live permission. Fast-bot artifacts remain shadow-only until the primary exact S5 bid/ask scorecard has at least 100 fills, 10 active days, PF>=1.25, and a strictly positive one-sided 95% expectancy lower bound, followed by a separate content-addressed live-promotion implementation that retires every overlapping legacy AI-trader new-entry authority. The fast bot and legacy AI trader must never both hold live new-entry authority for the same account/pair/lane.
- Normalize technical tuning identity before reviewing the queue: identity v2 uses one pending `TECHNICAL_STATE_CHANGE` bot-tuning scope per configured G8 pair, while fingerprints, closed-candle watermarks, direction, open-position action hints, quote/spread, and clocks are bounded observations rather than new work-order identities. The queue is bounded at 48; never increase it to hide stalled lifecycle work. Legacy per-candle revision-4 rows must merge without losing observations, and only a review whose `latest_reviewed_observation_id` equals the merged `latest_observation_id` counts as current.
- Treat the guardian's default six candidates as one closed-M1 refresh budget, not a fixed market universe. Preserve all open and hard execution-adjacent pairs, pin the active board/frontier next, and advance an absolute next-configured-pair cursor through the formal 28-pair G8 universe only after a successful refresh. Priority-count changes must not alter cursor stride; the optional cursor override seeds only a missing freshness artifact and must never pin later cycles. `PARTIAL` / `STALE` transport, shape, receipt, or clock failures retain the last-success cursor and retry the exact failed candidate set; newly open/hard pairs may only add to it, and any resulting effective-limit expansion must record its safety reasons. Only a canonical, coverage/provenance-complete, `malformed_count=0`, current-clock quarantine remains pair-local `TECHNICAL_INPUT_STALE` evidence while the cursor advances. Suppress that pair's `TRADE`/`ADD` paths, including retained pending replay, while preserving hold/harvest/reduce/cancel safety actions; do not authorize entry from a stale retained technical row just because its container was refreshed. A lock skip must not advance it, a scope change must not bypass the next-close-plus-grace gate, and a returning pair with unchanged technical or stale-input evidence must not create a false `NEW_EVENT`.
- Treat the guardian chart and freshness files as one content-bound proof. The live wrapper hashes the final atomically scoped chart bytes; the router parses and hashes that same byte snapshot, requires exact M1/M5/M15 sidecar rows, and revalidates every fixed max-age and aware close clock rather than trusting top-level `FRESH`. Production routing also validates the canonical MBA receipt for all three views even when no sidecar was supplied. Missing, empty, forged, scope-mismatched, future, or stale evidence is pair-local `TECHNICAL_INPUT_STALE`, never entry evidence. Before a guardian `TRADE`/`ADD` handoff, require the selected event itself to authorize entry with a concrete side, exact receipt/event/intent pair-side binding, a current persisted per-pair technical row, and RiskEngine exact `ALLOWED`/`allowed=true`; every exception or ambiguous result is no-send. Keep the generic guardian live handoff off unless its separate deployment approval exists.
...
- The GPT (AI trader) choice is bounded but operational: accept the exact deterministic lane and choose `capital_allocation.size_multiple` from `0.5`, `0.75`, or `1.0`, or veto it to `WAIT` / `REQUEST_EVIDENCE` with `NO_TRADE`. Read the packet's direction-specific hit/economic-hit/timeout calibration, exact-vehicle all-exit and TP expectancy, market-close leak, reward/risk, maximum loss, margin, month-scale replay state, and `forecast_replay_scorecard` before sizing. For the scorecard, report selected-pair coverage, proof status/blockers, independent sample count, hit-rate Wilson lower bound, average final pips, MFE/MAE, and realized R; never treat a filtered aggregate, `NOT_COVERED`, diagnostic-only cohort, or small raw point estimate as selected-lane proof. Negative or unknown edge means zero allocation unless the board proves (a) exact pair/side/method/vehicle financing-adjusted **all-exit** net evidence with at least 20 outcomes, positive net/expectancy, and positive Wilson-stressed expectancy, (b) a validated bounded predictive SCOUT, (c) a risk-reducing `HEDGE`, (d) the narrower attached technical-TP repair exception with exact source scope, at least five trades, zero losses, positive expectancy, and positive average win, (e) a separately labelled `EDGE_COLLECTION` M15-recovery route whose board and GPT packet both verify the same source/forecast/lane hashes, exact TP collection contract, Risk marker, exact `STOP-ENTRY` geometry, and 1..999-unit cap, or (f) `TARGET_PATH_LIVE_LEARNING_EDGE_COLLECTION`, bound to the current Codex market read and limited to a passive LIMIT, 1..1000 units, <=0.15 % NAV risk, >=1.5 reward/risk, exact target/attack/preflight proof, and immutable gateway geometry. Predictive SCOUT, `HEDGE`, verified M15 recovery, and verified target-path live learning use only the board's exact `1.0` multiplier; a smaller size requires a freshly regenerated producer receipt. `EDGE_COLLECTION` is bounded evidence acquisition, not proven positive edge, and may never authorize ordinary/full-size allocation. Broad/global positive expectancy and replay scorecards are context only. Never upgrade a non-trade baseline, choose another lane, exceed baseline units, alter order ids/stops/targets/risk caps, or claim live permission.
- `projection_ledger.calibration_evidence` is the bounded canonical `directional_forecast_{up|down|range}` scorecard for the current selected or forced pair/direction. Its hit rate, economic hit rate, timeout, and invalidation-first fields come only from `projection_ledger.compute_hit_rates` after outcome-blind independent-trial selection; raw ledger rows and intent metadata hit-rate claims are not calibration evidence. Recompute it at the packet clock: exclude future emissions before trial selection, and train only after both the full truth window and its valid resolution timestamp are observable. Any malformed JSON, non-object, or unloadable nonblank ledger row makes the whole scorecard no-edge; cite its parse-integrity status/counts instead of using the remaining rows. Cite the exact pair×regime, pair-all-regimes, or all-pair fallback label. Stale packets, insufficient samples, failed Wilson floors, pair-all-regimes rows, and all-pair fallbacks must never be called a pair×regime edge. The semantic digest is revalidated from a current recomputation of the selected scorecard, so an unrelated valid ledger append does not discard a still-identical score or falsely refresh the selected outcome lineage; selected-scope outcome recency is explicitly unproven, while packet age remains governed by the normal market-read artifact freshness gate. This scorecard is read-only: it may veto, dampen, or rank an already `LIVE_READY` lane, and cannot upgrade `WAIT` / `REQUEST_EVIDENCE`, grant gateway permission, or create live permission.
...
- Also inspect confidence-segment accounting and, when present, `by_primary_driver_family_direction`, `by_raw_confidence`, `by_score_margin`, `by_range_competition`, `by_against_driver_family_presence`, `by_session`, `by_technical_regime`, `by_technical_atr_band`, `by_technical_spread_band`, `by_technical_range_location_24h`, and `by_technical_structure_alignment`. A pooled condition whose direction split reverses sign, an unexplained confidence/context row count, or a non-monotonic confidence/score relationship blocks claims of calibrated edge. UTC session and technical-context cohorts are diagnostic and never create a hard trading-hour or live-permission rule. Only an exact content-addressed, mature `QR_FORECAST_FORWARD_HOLDOUT_RESULT_V1` produced from a uniquely registered pre-start lock may support a forward-evidence claim. Verify its lock-derived canonical path, authoritative `head.json`, local/cohort result registries, strict evaluator recomputation, actual-wall-clock forecast receipt chain, forced-production OANDA self-fetch marker/receipt/exact truth bytes, pinned Python/semantic dependencies, and code-enforced proof floors; a copied, stale, deleted, or superseded result file is not proof. Until a genuinely external append-only tip anchor exists, `EXTERNAL_MONOTONIC_ANCHOR_NOT_CONFIGURED` is mandatory, `proof_eligible` must remain false, and forward output is evidence collection only. The maturity time gates only evaluation of that exact lock; it never gates deployment, dry-run activation, current routing, or a separately authorized restart. Keep the legacy V1 cohort immutable and evaluate it when mature, but deploy V2 independently and pre-register a separate wholly post-activation V2 window. Even a future externally anchored result cannot grant live permission by itself.
- For every ordinary fresh entry, inspect the forecast envelope's exact current `regime_family_weighting_v1` receipt together with its bounded raw policy evidence and canonical M5 failed-break proof. Cite its SHA, primary regime, selected family/method, derived situation, aggregate direction, and D/H4/H1/M30/M15/M5/M1 weights. TREND selects TrendScore, RANGE selects MeanRevScore, BREAKOUT_PENDING selects BreakoutScore as readiness only, and TRANSITION/UNKNOWN selects NONE. A pending breakout is never directional from `breakout_score` sign. `BREAKOUT_FAILURE` requires the last explicitly complete M5 candle plus exactly 20 continuous prior complete M5 candles to prove one unambiguous sweep-and-reacceptance direction matching the forecast/order side; a WEAK label, same-direction break, missing proof, or opposite proof cannot select it. For ordinary methods the aggregate must be actionable and aligned; for `BREAKOUT_FAILURE` it must not oppose the canonical proof. Non-selected families are confirmation/counterargument only. Aligned family evidence cannot boost confidence or grant permission; contradiction may only damp/veto. Missing/tampered/internally inconsistent evidence, a `None` selected method, receipt SHA drift, or lane-method mismatch means zero allocation. Legacy and intermediate context remains readable for display/replay but cannot authorize an ordinary fresh entry; only the exact current all-seven-frame shape can.
- The sole fast-tail exception is a `QR_M15_RECOVERY_MICRO_V1` candidate. Never relabel its ordinary `technical_context` as valid: it must remain diagnostic `UNKNOWN`. Use it only when both the lane packet's `m15_recovery.status` and the allocation board's recovery context are `VERIFIED`, their source receipt / forecast binding / lane binding SHA values match, the source scope is exactly M15/M30/H1/H4/D, calibrated and raw direction match the lane, Risk revalidation is true, method is `BREAKOUT_FAILURE`, geometry is exactly `STOP-ENTRY` with attached technical TP, the exact TP mode is `TP_PROOF_COLLECTION_HARVEST` or `TP_PROVEN_HARVEST`, manual-position mutation is false, and units remain 1..999. Missing/mismatched evidence means `NO_TRADE`; this exception never lowers the ordinary seven-frame threshold or grants live permission by itself.
...
- In live automation, after the scheduled GPT/Codex trader authors the overlay, do not run apply, verifier, and gateway as separate lock-contending commands. Invoke the live wrapper with `QR_LIVE_WRAPPER_FINALIZE_CODEX_MARKET_READ=1`; it retains one shared lock while `trader-apply-market-read` publishes the receipt, `gpt-trader-decision` verifies it, and `autotrade-cycle` executes or performs position maintenance. On an accepted verification, the wrapper alone pins the exact response, verifier output, baseline, packet, and overlay SHA-256 plus the current lock-generation token for at most two minutes and one gateway cycle; do not set or copy `QR_AUTOTRADE_VERIFIED_HANDOFF_*` manually. This prevents a concurrent 30-second Guardian publication from falsely turning the result into `STALE_DECISION` while any changed immutable handoff file, another lock generation, verifier rejection, or expired attestation remains fail-closed. Apply failure aborts before any stale response can reach the gateway; verifier rejection still blocks new risk while allowing maintenance.
...
- The `com.quantrabbit.guardian-wake-dispatcher` LaunchAgent wakes GPT-5.5 with read-only `codex exec` only for important entry/ADD or active-exposure events. Non-P0 `NO_ACTION` tuning observations stay with the hourly AI via `QR_GUARDIAN_WAKE_TUNING_MODE=HOURLY`. The 48-slot ordinary tuning queue has 8 additional reserved slots usable only by P0/P1, execution-facing Guardian actions, or events bound to current open exposure, so accumulated research cannot suppress a timely action receipt; the reserve is evidence capacity, never live permission. The installed live policy sets `QR_GUARDIAN_WAKE_GATEWAY_HANDOFF=1`, `QR_GUARDIAN_ACTION_EXECUTE=1`, and `QR_LIVE_ENABLED=1`; accepted wake receipts may therefore call `guardian-action-cycle`, but every broker mutation still requires fresh broker truth, receipt validation, `RiskEngine`, and `LiveOrderGateway`, with no direct GPT/OANDA path.
...
- Bound hourly tuning work to one queue snapshot captured after the opening Guardian-action disposition. Process the exact work-order/observation identities from that snapshot in one manifest whenever possible. Ordinary research observations appended or advanced while the hourly trader is reasoning belong to the next hourly cycle; do not repeatedly refresh support and chase a moving tuning tail before the current MARKET_READ_FIRST decision. P0/P1 or execution-facing events still use the immediate Guardian wake/action path. Deferred ordinary tuning remains an obligation, but it is not market evidence and cannot become a second veto for an authenticated already-`LIVE_READY` scout.
- Before other tuning work, run `PYTHONPATH=src "$QR_PYTHON" tools/guardian_tuning_override_reconcile.py`. Never execute this tool through an ambient/system `python3`; the live runtime requires the configured project interpreter. A staged override without matching revision-4 terminal evidence remains dormant and blocks that lane's live send; reconciliation may confirm only an exact `CONSUMED` + `ACCEPTED_IMPROVEMENT` work-order/experiment/evidence match.
- Then run `PYTHONPATH=src "$QR_PYTHON" tools/guardian_tuning_post_activation_monitor.py`, again with the configured project interpreter rather than an ambient/system `python3`. It fixes the first 20 canonical attributed entries whose ledger row and raw OANDA entry time are both strictly after the immutable activation-ledger anchor, waits for those same 20 to resolve, and revalidates raw broker entry/close truth plus both the sealed ledger prefix and the current full-ledger first-20 truth before committing `KEEP` for a positive normalized metric or fail-closed `QUARANTINE` for a non-positive metric. Unrelated later rows may append, but a late earlier entry or financing/outcome correction invalidates the old seal and requires a new one. Once entry 20 exists, `RiskEngine` blocks that exact lane with `GUARDIAN_TUNING_POST_ACTIVATION_MONITOR_PENDING` even if this worker was skipped. A same-lane successor may follow only a prior `KEEP` whose activation, terminal, monitor, and current-ledger provenance still validates; unmonitored/quarantined/stale state cannot reset the first-20 boundary. Retry reuses an already-valid content-addressed monitor artifact. Deep override validation is cached only within one gateway phase and is cleared for a fresh final pre-POST read; it is never a process-persistent shortcut. Quarantine is not an automatic relaxation to the recorded previous value.
...
- When `capital_allocation_board.forecast_context_scope=FORCED_PREDICTION`, `candidate_count=0`, and `prediction_to_intent_bridge_required=true`, read the content-addressed `CURRENT_PAIR_CHART_AND_BID_ASK` technical context as the current prediction basis instead of calling the technical state unknown. Name the active regime/family/timeframe evidence, state that no matching executable lane exists, and request a fresh pair/direction/method-bound intent in the next producer cycle. This bridge is read-only, grants no live permission, cannot select an arbitrary lane, and must never be described as a current candidate or trade authorization.
- Do not average all indicators together. For ordinary lanes, the verified regime-family receipt is the active technical selector and its D/H4/H1/M30/M15/M5/M1 weights are the only weighting basis for the forecast read. Confirm that the parent raw policy evidence purely reproduces the derived situation/news/edge/weights and, for `BREAKOUT_FAILURE`, that the canonical M5 proof reproduces the selected direction. State which family was active, which evidence was suppressed, and whether its direction aligned or contradicted the lane. If the exact current ordinary context does not prove an executable selected method identical to the lane method, keep `capital_allocation=NO_TRADE`. For the sole verified `QR_M15_RECOVERY_MICRO_V1` exception, state that M1/M5 and all aggregate inputs were suppressed, cite the matching recovery packet/board hashes plus the M15+ source scope and component scores, and apply the separate bounded recovery rule above; never invent per-timeframe weights or pretend that its ordinary regime-family receipt passed.
- An exact regime-family contradiction keeps the ordinary forecast `UNCLEAR`, but it no longer erases the pre-veto detector thesis from learning. When the selected baseline is an authenticated `FORECAST_LEARNING_SCOUT`, verify its content-addressed learning receipt, ranked side, point-in-time technical method/desk, and execution-geometry binding. Do not reapply the ordinary `UNCLEAR` result or the original detector side as a second directional veto; accept or veto the exact bounded passive-LIMIT scout from its current numeric market read and hard execution evidence.
- `trader_support_bot.metrics.send_fresh_entries_allowed=false` and `FRESH_ENTRY_SEND_NOT_ALLOWED` are ordinary support-basket readiness signals, not a second veto for an authenticated, already-`LIVE_READY` `FORECAST_LEARNING_SCOUT`. Do not downgrade that scout only because historical profitability P0s remain open or the same live-runtime lock temporarily ages the Guardian heartbeat; the final wrapper must still re-prove Guardian health and every current RiskEngine/gateway gate. A genuinely inactive Guardian without current-lock deferral, an intent-local Guardian blocker, or any exact current execution blocker remains a hard veto.
...
- Treat that numeric allocation as a prediction hypothesis, not durable order permission. Ordinary schema-v2 allocations execute as MARKET by default and must survive the gateway's fresh quote/NAV/conversion RiskEngine remeasure, directionally favorable drift rule, economic-Wilson EV, quarter-Kelly floor-unit ceiling, and complete OANDA S5 no-touch path from forecast emission through the current quote. The only ordinary non-MARKET exception is a TP-proven `RANGE` `RANGE_ROTATION`/`BREAKOUT_FAILURE` passive LIMIT whose signed basis is `EXACT_VEHICLE_TAKE_PROFIT`, whose exact forecast rails/side/entry/TP/SL and cost-aware risk cap are frozen, and whose current exact TP cohort plus loss-proxy Wilson stress remain positive at every pre/post boundary. It has no `priceBound`, may never become STOP/MARKET, and expires rather than reprices after its rail is crossed or touched. For MARKET, the gateway fixes a side-aware worst-fill `priceBound` into the reservation. For both paths, receipt SHA/basis/cost identity is frozen from one entry read, a target/TP/invalidation/rail touch followed by a retrace is still consumed, and the gateway re-proves NAV/margin/basket/attached-stop truth after reservation. The final post-S5 broker read is one coherent account-and-quote snapshot: transaction truth must remain unchanged, NAV/margin must not worsen, and a passive RANGE LIMIT plus its frozen rails must remain untouched. Failure retains the one-shot reservation and sends no POST. Do not remove or loosen these proofs to increase trade frequency; obtain a fresh forecast instead. Broker-carried entry/TP/SL/disaster prices must already be on the instrument tick grid.
...
- `campaign_exposure_recovery` and deterministic recovery are fresh-entry routes, not WAIT overrides. They require a fresh accepted schema-v2 GPT `TRADE` receipt naming the lane; ADD/HEDGE remain intent roles under that receipt. Stale accepted `WAIT` / `REQUEST_EVIDENCE`, `gpt_allowed=false`, fresh-receipt-required errors, unresolved guardian receipt issues, missing/stale guardian receipt operator review, watchdog `normal_routing_allowed=false`, and missing market-read confirmation block the send.
...
# Capture `authored_at_utc` from the host immediately before the JSON write
# (for example `date -u +%Y-%m-%dT%H:%M:%SZ`). Never infer or mentally calculate
# the current time. A future or expired model-authored clock is a freshness
# defect and must not spend the cycle's one freshness retry unnecessarily.
#
...
QR_LIVE_ENABLED=1 QR_LIVE_WRAPPER_FINALIZE_CODEX_MARKET_READ=1 \
...
#   NAV aggregate tier risk, and the authenticated current technical method plus
#   matching desk identity; `NONE` cannot mint a vehicle and the generator must
#   never rewrite every rank to failure_trader/BREAKOUT_FAILURE. Original
#   current-price forecast geometry remains audit evidence while a separate
#   content-addressed receipt binds the passive LIMIT entry and its exact
#   technical TP/SL. No basket/reprice/chase, and thirty atomic pre-POST
#   reservations/day maximum.
