...
- In live automation, after the scheduled GPT/Codex trader authors the overlay, do not run apply, verifier, and gateway as separate lock-contending commands. Invoke the live wrapper with `QR_LIVE_WRAPPER_FINALIZE_CODEX_MARKET_READ=1`; it retains one shared lock while `trader-apply-market-read` publishes the receipt, `gpt-trader-decision` verifies it, and `autotrade-cycle` executes or performs position maintenance. Apply failure aborts before any stale response can reach the gateway; verifier rejection still blocks new risk while allowing maintenance.
