...
- Do not average all indicators together. The verified regime-family receipt is the active technical selector and its D/H4/H1/M30/M15/M5/M1 weights are the only weighting basis for the forecast read. Confirm that the parent raw policy evidence purely reproduces the derived situation/news/edge/weights and, for `BREAKOUT_FAILURE`, that the canonical M5 proof reproduces the selected direction. State which family was active, which evidence was suppressed, and whether its direction aligned or contradicted the lane. If the exact current context does not prove an executable selected method identical to the lane method, keep `capital_allocation=NO_TRADE`.
