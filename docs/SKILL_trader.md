...
```json
{
  ...
  "evidence_refs": [
    ...
    "chart:<pair>:M1", "chart:<pair>:M5", "chart:<pair>:M15", "chart:<pair>:M30",
    "chart:<pair>:H1", "chart:<pair>:H4", "chart:<pair>:D", "chart:<pair>:structure",
...
- **Picking a single-lane "basket" when `recommended_now_lane_ids` covers multiple distinct pairs.** Per AGENT_CONTRACT §5–§6 campaign exposure occupancy, if `daily_target_state.json` is `PURSUE_TARGET`, the trader is flat, and `ai_attack_advice.recommended_now_lane_ids` (intersected with current tradeable `LIVE_READY` lanes) covers N distinct pairs with N ≥ 2, the selected basket must (a) include one lane per advised pair up to `RiskPolicy.max_portfolio_positions`, OR (b) cite a named deterministic exposure / risk / strategy / spread / event / broker-truth gate per skipped pair in `risk_notes`. Selecting one pair when four are advised and none are blocked is leaving edge on the table — every advised pair is already a validated `LIVE_READY` lane, the gateway still performs basket / portfolio / margin validation, and `per_trade_risk_budget_jpy` already bounds each shot. When multiple `MARKET` and pending lanes exist for the same pair-direction, prefer the `MARKET` variant for immediate participation; pending entries on the same pair count as basket occupancy at the gateway but should not replace `MARKET` if one is currently `LIVE_READY`.
