...
- A guardian wake receipt is executable only when its `event_id`, `pair`, compatible side/direction, and any included `dedupe_key` match the selected event. `RECEIPT_EVENT_MISMATCH` means the dispatcher removed `data/guardian_action_receipt.json`; resolve the queued event from `docs/guardian_action_review.md` before ordinary entries.
