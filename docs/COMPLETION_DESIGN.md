...
- Known live-risk failure mode fixed: every fresh-entry broker POST, including ADD/HEDGE intent roles, must carry the fresh accepted schema-v2 GPT `TRADE` receipt and its exact capital allocation. `stage-live-order` may stage without it but cannot send; guardian or direct gateway receipts alone cannot authorize units.
