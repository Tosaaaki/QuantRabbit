...
2. Otherwise emit a disabled optional snapshot. The trader treats option-skew
   data as absent rather than guessed, and does not spend cycle time or matrix
   budget on a feed that is not wired.
...
def build_option_skew_snapshot(*, pairs: Sequence[str], tenors: Sequence[str] = ("1W", "1M", "3M")) -> OptionSkewSnapshot:
    """Try the configured provider; otherwise return a disabled snapshot.
    ...
    if provider_key in _PROVIDERS:
        ...
        issues.append(f"OPTION_SKEW_PROVIDER_UNREGISTERED: {provider_key}")
