...
class BasicScalpStrategy:
    ...
    def evaluate(
        ...
        *,
        ...
        revert_sl_atr_k: float = 2.0,
        revert_sl_min_pips: float = 8.0,
        ...
        vwap_delta = float(fac_m1.get("tick_vwap_delta") or 0.0)

        # ATR連動のモメンタム閾値に補正（高ボラ時の偽反転を抑制）
        try:
            dyn_flip = max(float(momentum_flip_pips), max(0.0, atr_pips) * 0.12)
            dyn_confirm = max(float(momentum_confirm_pips), max(0.0, atr_pips) * 0.08)
        except Exception:
            dyn_flip = float(momentum_flip_pips)
            dyn_confirm = float(momentum_confirm_pips)
        ...
        if allow_mean_revert and price > ma10 and rsi > 56 and not trend_short:
            if not (
                momentum_5 <= -dyn_flip
                and momentum_10 <= -dyn_confirm
            ):
                return None, "momentum_short_missing"
            # 十分な上方乖離（VWAP）を確認してからショートの逆張り
            if vwap_delta < 0.15:
                return None, "vwap_ext_weak_short"
            # 逆張りは広めのSLに引き上げ
            sl_pips = max(sl_pips, atr_pips * max(0.0, float(revert_sl_atr_k)), max(6.0, float(revert_sl_min_pips)))
            tp_pips = max(tp_pips, sl_pips * max(1.25, tp_multiplier))
        ...
        if allow_mean_revert and price < ma10 and rsi < 44 and not trend_long:
            ...
            # 十分な下方乖離（VWAP）を確認してからロングの逆張り
            if vwap_delta > -0.15:
                return None, "vwap_ext_weak_long"
            # 逆張りは広めのSLに引き上げ
            sl_pips = max(sl_pips, atr_pips * max(0.0, float(revert_sl_atr_k)), max(6.0, float(revert_sl_min_pips)))
            tp_pips = max(tp_pips, sl_pips * max(1.25, tp_multiplier))
