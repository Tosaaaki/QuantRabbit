...
### 8.4 Open Trades Guard（2026-03-06 追記）

- `M1SCALP_MAX_OPEN_TRADES=1` は strategy tag 単位の open trade 数で強制する。
- 実装は `workers/scalp_m1scalper/worker.py` の `PositionManager` ベース guard で行う。
- `position_manager` 不達時は `M1SCALP_FAIL_CLOSED_ON_POSITIONS_ERROR=1` を既定とし、fail-open で積み上げない。
- 監視ポイント:
  - `quant-m1scalper.log` に `open_trades_block` が出ること自体は正常。
  - `openTradeCount` が短時間に増え続ける場合は guard 漏れか service 経路断を疑う。

### 8.5 Derivative Workers Open Trades Guard（2026-03-06 追記）

- `TrendBreakout` と `pullback_continuation` も M1 family 派生 worker として同じ `M1SCALP_MAX_OPEN_TRADES` 契約に従う。
- 実装は各 worker 内で `PositionManager.get_open_positions(include_unknown=False)` を参照し、`entry_thesis.env_prefix=M1SCALP` を含む同一 family 単位で block する。
- `M1SCALP_FAIL_CLOSED_ON_POSITIONS_ERROR=1` を既定とし、`position_manager` stale/busy/unavailable 時は fail-open しない。
- `M1SCALP_ENTRY_GUARD_BYPASS=0` を local override に戻し、`bb_entry_reject` を無視して送る経路は止める。
- 監視ポイント:
  - `quant-m1scalper.log` / `quant-scalp-trend-breakout.log` / `quant-scalp-pullback-continuation.log` に `open_trades_block` が出ること。
  - `source_signal_tag=M1Scalper-*` の派生 worker が別 tag で同方向に積み上がっていないこと。
