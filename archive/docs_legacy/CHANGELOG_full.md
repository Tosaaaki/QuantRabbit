...
## 2026-03-26
- 2026-03-26T17:15Z: **v8 — traderを正のシステムとして昇格。リポジトリ全面整理** — (1) 無効scheduled tasks 7個削除 (2) scripts/直下162個→scripts/archive/ (3) workers/, addons/, analysis/, tests/→archive/ (4) logs/旧ファイル→logs/archive_legacy/ (5) remote_logs 21GB削除(core DB 96MBに圧縮してarchive/に保存) (6) systemd/, infra/terraform/, .gcloud/, ops/, legacy/, local/→archive/ (7) 旧プロンプト12個+旧ドキュメント→archive/docs_legacy/ (8) 旧config(pattern_book, brain_*, tuning_*)→archive/config_legacy/ (9) トップレベルのレガシーファイル・ディレクトリ30個+→archive/ (10) staleワークツリー30個+削除、codex/ブランチ96個+削除 (11) CLAUDE.md v8刷新、メモリ更新。最終構成: CLAUDE.md, collab_trade/, scripts/trader_tools/, indicators/, logs/, config/, docs/, archive/
- 2026-03-26T21:30Z: **trade_performance.py v4 — パフォーマンス導線復旧** — (1) 致命的断絶の修正: v6ログ形式(`CLOSE PAIR DIR UNITSu @PRICE PL=±NNN円`)に対応。旧v3は`FAST: CLOSED PAIR DIR ±N.Npip`しかパースできず、v6移行後の69件が全件ドロップしていた (2) P/L単位をpip→JPYに変更。実際のログに合致 (3) 日別/ペア別/セッション別/方向別/ヘッジ別の集計追加 (4) `--days N`/`--date YYYY-MM-DD`フィルタ追加 (5) SKILL.md: Bash②初手とSESSION_ENDにperformance集計を組込。traderが毎セッション自分の戦績を見てから判断する導線
- 2026-03-26T13:00Z: **v7 — マージン安全ルール + 判断品質改善** — (1) 2分短命セッション維持（10分に伸ばしたがセッション死亡で引き継ぎ不可→戻した） (2) マージン安全ルール追加: marginAvailable<marginUsed×0.3で新規禁止、×0.15で強制半利確 (3) 1ペアadd-on上限5本。クローズアウト後30分待ちルール (4) Sonnet化。背景: 03:30Zにマージンクローズアウト発生。GBP_JPY 9本+AUD_JPY 12本+EUR_JPY 2本=23ポジション。問題はセッション長ではなく判断の質。マージン残2,935円でEUR_JPY追加が致命傷

## 2026-03-25
- 2026-03-25T17:00Z: **両建て（ヘッジ）回転戦術をtraderに組込** — (1) OANDA v20ヘッジ口座のマージン計算確認: 反対ポジションは追加マージン0（大きい方の片側にフルマージン） (2) SKILL.md: 「両建て回転」セクション追加。毎サイクルでM5過熱時のヘッジSHORT判断を行う導線 (3) strategy_memory.md: 両建て戦術セクション追加。マージン計算ルール・MTF階層分離・回転パターン・注意点 (4) メモリ更新: feedback_hedging.mdにマージン詳細追記
