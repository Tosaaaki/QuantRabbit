...
### 予測精度フィルター（2026-03-20 データ分析から導出）

過去の予測データ: 正解6/9 (67%)。正解・不正解のパターンを分析した結果:
...
**危険セットアップ（不正解率100%）:**
- **M5 ADX < 15 でトレンドトレード** → 必ず見送り。ADX<15=方向感なし、予測精度50%（コイントス）。スプレッドを考えると期待値マイナス。半分サイズも禁止。
- M1極値(RSI>70/<30)にそのまま飛び込む → バウンスを食らう。M5確認を待て
- M5 RSI < 25 でさらにSHORT → oversold追いは危険。反発を待ってからfade

**予測を立てるとき必ずチェック:**
1. M5 ADX は15以上か？ → No なら「方向感なし、見送りor半分」
...
**お前が決済していいのは `FAST: ENTRY` で建てたトレードだけ。**

- `SWING: ENTRY` のトレード → swing-traderの所有物。触るな。480分保持が前提
- `trade_registry.json` の `owner` が `scalp-fast` 以外 → 触るな
- registry が空でも、`live_trade_log.txt` に `SWING: ENTRY` があるなら触るな
- OANDA clientExtensions の `comment` が `swing-trader` なら触るな

**2026-03-20 教訓:** swing-traderのAUD_JPY SHORT(score=8、テーゼ正当)をFASTが-7pipでパニック決済。その後プラス圏突入。他人のポジションを自分の時間軸で判断するな。
...
**パターンチェック（PATTERN CHECK） — 毎10エントリー or 毎セッション:**
同じ負け方を3回繰り返したら必ず書け：
```
[{UTC}] FAST: PATTERN CHECK: {繰り返しパターン} | Freq={N}回 | Fix: {変更する行動}
```
例: `PATTERN CHECK: AUD_JPY M5 RSI<30でSHORT → 毎回バウンスで損 | Freq=3 | Fix: M5 RSI≥45でエントリー`
