...
def _map_exit_env_prefix() -> None:
    ...
    os.environ.setdefault(
        ...
        os.getenv(f"{_EXIT_ENV_PREFIX}_TAGS", "LevelReject"),
    )
    os.environ.setdefault(
    ...
    os.environ.setdefault(
        ...
        os.getenv(
            f"{_EXIT_ENV_PREFIX}_PROFILE_ENABLED",
            os.getenv("SCALP_LEVEL_REJECT_EXIT_PROFILE_ENABLED", "1"),
        ),
    )
    os.environ.setdefault(
...
_STRATEGY_PROTECTION_ENABLED = (
    os.getenv("SCALP_LEVEL_REJECT_EXIT_PROFILE_ENABLED", "1").strip().lower()
    not in _FALSEY
)
...
def _level_reject_live_protection_adjustments(
    *,
    ...
    if (
        ...
        and abs(ma_gap_pips) > max(1.0, atr_pips * 1.2)
    ):
        stress += 0.04
    if weak_probe:
        stress += 0.08
    ...
    return {
        ...
        "lock_ratio_mult": round(
            _clamp_float(1.0 + stress * 0.85 - support * 0.55, 0.76, 1.28), 3
        ),
        "min_lock_mult": round(
        ...
        "buffer_mult": round(
            _clamp_float(1.0 - stress * 0.55 + support * 0.80, 0.62, 1.28), 3
        ),
        "min_gap_mult": round(
            _clamp_float(
...
class RangeFaderExitWorker:
    def __init__(self) -> None:
        ...
        self.exit_policy_start_ts = load_rollout_start_ts(
            "SCALP_LEVEL_REJECT_EXIT_POLICY_START_TS"
        )
