...
def load_strategy_profile(
    ...
    *,
    ...
    file_path = Path(
        path or os.getenv("PARTICIPATION_ALLOC_PATH", "config/participation_alloc.json")
    )
    ...
    for key in _candidate_keys(strategy):
        ...
        profile = {
            ...
            "probability_multiplier": _safe_float(
                item.get("probability_multiplier"), 1.0
            ),
            ...
            "attempts": _safe_int(
                item.get("attempts"), _safe_int(item.get("preflights"), 0)
            ),
            "preflights": _safe_int(
                item.get("preflights"), _safe_int(item.get("attempts"), 0)
            ),
            "fills": _safe_int(item.get("fills"), _safe_int(item.get("filled"), 0)),
            "filled": _safe_int(item.get("filled"), _safe_int(item.get("fills"), 0)),
            "filled_rate": _safe_float(
                item.get("filled_rate"), _safe_float(item.get("fill_rate"), 0.0)
            ),
            "fill_rate": _safe_float(
                item.get("fill_rate"), _safe_float(item.get("filled_rate"), 0.0)
            ),
            "attempt_share": _safe_float(
                item.get("attempt_share"), _safe_float(item.get("current_share"), 0.0)
            ),
            "current_share": _safe_float(
                item.get("current_share"), _safe_float(item.get("attempt_share"), 0.0)
            ),
            "fill_share": _safe_float(
                item.get("fill_share"), _safe_float(item.get("target_share"), 0.0)
            ),
            "target_share": _safe_float(
                item.get("target_share"), _safe_float(item.get("fill_share"), 0.0)
            ),
