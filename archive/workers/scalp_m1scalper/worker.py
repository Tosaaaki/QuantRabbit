...
def _apply_signal_setup_context(
    entry_thesis: dict[str, object], signal: dict | None
) -> dict[str, object]:
    ...
    if _bb_float(payload.get("entry_probability")) is not None:
        entry_thesis["signal_entry_probability"] = round(
            float(payload["entry_probability"]), 3
        )
    ...
    while True:
        ...
        if _regime_snapshot is not None:
            ...
            _r_allowed, _r_reason = regime_should_enter(
                _regime_snapshot, _r_strategy_type, signal_side
            )
            if not _r_allowed:
                now_mono = time.monotonic()
        ...
        setup_label = usdjpy_setup_mode or str(
            quickshot_plan.get("mode") or signal_setup.get("strategy_mode") or "none"
        )
