...
- **Claude Code / Codex による裁量トレードを許可する**。Claude（Scheduled Tasks含む）が自ら市況を判断し、OANDA APIで直接注文・決済することを正規運用とする。


## 1. ミッション / 役割
> 狙い: USD/JPY で資産の10%増を狙う Claude裁量スキャルピング。
> Claudeが自分の頭で市況を読み、OANDA APIで直接注文する裁量トレーダー。
> ボット/自動売買ではない。Scheduled Tasks（scalp-trader / market-radar / macro-intel / secretary）が連携して裁量判断を行う。
...
- **ユーザへ改善案を返す前には必ず** `scripts/improvement_preflight.sh "<query>" "<strategy::surface::primary_loss_driver::idea||...>"` を実行する。`change_preflight.sh` だけで済ませず、候補ごとに `market_hold / review_existing_pending / escalate_family_not_tighten / allow_new_lane` を確認してから返答する。
...
- `scripts/improvement_preflight.sh` の候補 spec は `strategy::surface::primary_loss_driver::idea` を `||` で連結したものを使う。`surface` を具体化できない候補は新規改善案として出さない。
...
- `scripts/improvement_preflight.sh` が `review_existing_pending` か `market_hold_review_only` を返した候補は、新しい改善案として口頭でも出さない。返答は「既存 pending の再検証対象」か「family escalation 候補」に止める。
...
- 2026-03-11 追記: current 30 分 lane を shared artifact へ早めに上げるため、`scripts/participation_allocator.py` の setup override は `setup_min_attempts=4` を正とし、`max_units_cut / max_units_boost / max_probability_boost` を setup payload へ保持する。`workers/common/participation_alloc.py` はこれを runtime profile へそのまま渡し、winner setup の unit/probability boost を probability-only に潰さない。
- 2026-03-11 追記: `WickReversalBlend` 系の `DroughtRevert` / `PrecisionLowVol` / `VwapRevertS` は、current live loser short（bullish continuation 下の range fade short）に対して dedicated env を増やさず、worker 内 `flow_guard` を正とする。`workers/scalp_wick_reversal_blend/worker.py` の short fade は `ema_slope_10/20`, `macd_hist`, `DI gap`, `vwap_gap stretch`, `price-vs-mean`, `range_score`, `tick_reversal strength` から `continuation_pressure / reversion_support / setup_quality` を動的算出し、strong headwind short を strategy-local に reject する。`PrecisionLowVol` の `vgap_bias_ok` は marginal headwind では boost せず、`DroughtRevert` は `projection_decision` を必須化する。shared gate / time block / order-manager の後付け抑止は追加しない。
- 2026-03-11 追記: shared participation の current 運用は strategy-wide blanket trim を正としない。`scripts/entry_path_aggregator.py` は `orders.request_json.entry_thesis` から setup identity を抽出し、strategy ごとに `setups` を集計する。`scripts/participation_allocator.py` は `setup_fingerprint / flow_regime / microstructure_bucket` ごとの `setup_overrides` を生成し、`workers/common/participation_alloc.py` は live `entry_thesis` の current setup と一致する override だけを選ぶ。`execution/strategy_entry.py` は current `entry_thesis` を loader へ渡し、shared participation でも別 setup への一律 trim/boost を行わない。
...
- 2026-03-11 追記: `strategy_feedback` の current 運用は strategy-wide blanket trim を正としない。`analysis/strategy_feedback_worker.py` は recent trades の `entry_thesis` から `setup_fingerprint` / `flow_regime` / `microstructure_bucket` を集計し、strategy ごとに `setup_overrides` を出力する。explicit な setup field が無い古い trade でも `technical_context` / `spread_pips` / `range_score` / `units` から setup identity を再構成して current setup cluster へ寄せる。`analysis/strategy_feedback.py` は live `entry_thesis` の current setup に一致する override だけを適用し、fresh artifact でも別 setup へ一律に流用しない。shared feedback は「戦略全体」ではなく「今の型」にだけ効かせる。
- 2026-03-11 追記: `M1Scalper` の current 運用は `quickshot` / `reversion` / `breakout_retest` / `vshape` を env 固定 gate のまま扱わない。`strategies/scalping/m1_scalper.py` は breakout/vshape signal ごとに `flow_regime` / `continuation_pressure` / `microstructure_bucket` / `setup_quality` / `setup_fingerprint` / `entry_probability` / `setup_size_mult` を live factor から算出して signal に載せる。`workers/scalp_m1scalper/worker.py` はこれを `entry_thesis` へ保存し、units scale にも反映する。shared layer は `M1Scalper-M1` を strategy-wide に見るのではなく current setup 単位で学習・調整することを正とする。
- 2026-03-11 追記: `dynamic_alloc` / `auto_canary` も strategy-wide blanket trim を正としない。`scripts/dynamic_alloc_worker.py` は recent trades の `entry_thesis` から setup identity を再構成して `config/dynamic_alloc.json` に `setup_overrides` を出力し、`workers/common/dynamic_alloc.py` と `execution/strategy_entry.py` は live `entry_thesis` に一致する override だけを適用する。`scripts/loser_cluster_worker.py` は cluster ごとに `setup_context` を保持し、`scripts/auto_canary_improver.py` は `config/auto_canary_overrides.json` へ setup-scoped canary override を出力する。`analysis/auto_canary.py` は live setup match の override だけを使う。さらに `workers/scalp_m1scalper/exit_worker.py` は `entry_thesis.m1_setup` の `setup_quality / flow_regime / continuation_pressure / setup_fingerprint` から trade-local の `profit_take / lock_trigger / hold / soft-exit` threshold を再計算し、shared post-hoc gate を増やさない。
- 2026-03-11 追記: `dynamic_alloc` の current 運用は `setup_min_trades=4` を正とし、current 4-trade setup を strategy-level `min_trades` と切り離して trim/hold 判定する。さらに `sum_realized_jpy<=-8` かつ `weighted_win_rate<=0.25` / `jpy_pf<=0.25` の single-trade severe loser setup は、`setup_min_trades` 未満でも setup override を emit して current lane を薄くする。winner 側の shared push は `participation_alloc` が担当し、`dynamic_alloc` では global cap を広域に緩めない。
- 2026-03-11 追記: `scalp_ping_5s_d_live` の current 運用は `SCALP_PING_5S_D_TP_ENABLED=1` を正とし、`workers/scalp_ping_5s/worker.py` は `horizon_composite_side != neutral` で horizon に逆らい、かつ `m1_trend_gate == m1_opposite` の D variant entry を strategy-local に reject する。shared gate / time block ではなく D worker 内 guard で current loser lane を落とし、small-win capture は broker TP を併用する。監査根拠は `docs/TRADE_FINDINGS.md` / `docs/RISK_AND_EXECUTION.md` を正とする。
- 2026-03-11 追記: local-v2 の current 運用は、shared env の blanket `STRATEGY_CONTROL_ENTRY_* = 0` を常用しない。loser lane の改善は `flow_guard` / lane-aware exit / setup-scoped shared trim（`trim_units` / bounded negative `probability_offset` / active `boost_participation`）を正とし、shared `RISK_PERF_MIN_MULT` などの広域 multiplier は safer baseline を維持する。`execution/order_manager.py` の `ORDER_MIN_RR_*` は preserve-intent 運用でも risk/execution guard として適用し、intent preserve を理由に bypass しない。
- 2026-03-11 追記: shared participation の current fast-profit 運用では、`scripts/run_local_feedback_cycle.py` の `entry_path_aggregator` / `participation_allocator` を `120s` 周期で回し、`participation_allocator` は `--lookback-hours 6 --min-attempts 12 --setup-min-attempts 2 --max-units-cut 0.22 --max-units-boost 0.24 --max-probability-boost 0.10` を正とする。`scripts/participation_allocator.py` は `2 attempts / 2 fills / positive realized_jpy` の fresh winner lane を early `boost_participation` へ上げ、current loser setup は `2-attempt` でも trim override を emit する。`execution/strategy_entry.py` は runtime cap `STRATEGY_PARTICIPATION_ALLOC_MULT_MAX=1.24` / `STRATEGY_PARTICIPATION_ALLOC_PROB_BOOST_MAX=0.12` を上限に、artifact で指定された lane だけを押し引きする。監査根拠は `docs/TRADE_FINDINGS.md` / `docs/RISK_AND_EXECUTION.md` を正とする。
- 2026-03-12 追記: local-v2 の `strategy_feedback` current 運用は、`participation_alloc` の boosted low-sample lane を directional key のまま health / feedback coverage に使わない。`analysis/strategy_feedback_worker.py` と `scripts/publish_health_snapshot.py` は discovered strategy の canonical key へ remap して `feedback_probe` と coverage を揃える。あわせて `STRATEGY_FEEDBACK_LOOP_SEC=120` を現行運用値とし、restart 後の薄い payload が長く残らないようにする。監査根拠は `docs/TRADE_FINDINGS.md` / `docs/RISK_AND_EXECUTION.md` を正とする。
- 2026-03-12 追記: `scalp_extrema_reversal_live` の current 運用は short 全停止を正としないが、`range_fade / range_compression` の浅い short probe を worker local で落とす。`workers/scalp_extrema_reversal/worker.py` は `M5` の bearish support が無い状態で `range_mode=RANGE` かつ `ma_gap_pips>0`、`dist_high / short_bounce / tick_strength` が shallow な short を `short_countertrend_block` / `short_shallow_probe_block` で reject する。bearish `M5` support が揃う short は維持し、shared gate / time block / strategy-wide blanket trim は追加しない。監査根拠は `docs/TRADE_FINDINGS.md` / `docs/RISK_AND_EXECUTION.md` を正とする。
- 2026-03-12 追記: `PrecisionLowVol` / `DroughtRevert` の current 運用は、scalp pocket の global `ORDER_MIN_RR=1.50` をそのまま食わせない。`execution/order_manager.py` は `ORDER_MIN_RR_STRATEGY_*` を優先して読み、current live loser だった `PrecisionLowVol` / `DroughtRevert` には `1.10` を適用する。あわせて `workers/scalp_wick_reversal_blend/worker.py` は両 strategy の `sl_pips` を current 1.8-2.2 pip ATR 帯向けに広げ、`units` は sl 連動で自動縮小させる。shared blanket loosening ではなく strategy-local / strategy-scoped RR 緩和を正とする。監査根拠は `docs/TRADE_FINDINGS.md` / `docs/RISK_AND_EXECUTION.md` を正とする。
- 2026-03-12 追記: `scalp_extrema_reversal_live` の current 運用は short 全停止を正とせず、recent outcome を使った setup-pressure guard を worker local に持つ。`workers/scalp_extrema_reversal/worker.py` は recent trades の `entry_thesis.extrema.side=short` かつ `range_reason=volatility_compression` を集計し、`sl_rate / fast_sl_rate / net_jpy` が悪化している間だけ `dist_high<=0.90`, `short_bounce<=0.50`, `tick_strength<=0.50` の shallow short を reject する。stronger reversal short は維持し、shared gate / time block / strategy-wide blanket trim は追加しない。監査根拠は `docs/TRADE_FINDINGS.md` / `docs/RISK_AND_EXECUTION.md` を正とする。
- 2026-03-12 追記: market order の current 運用では、retry / slippage 後の actual `executed_price` に対して broker protection を再アンカーする。`execution/order_manager.py` は fill 後の `on_fill_protection` で submit 時の `sl_price / tp_price` をそのまま再送せず、thesis `sl_pips / tp_pips` を actual fill 基準へ引き直した protection を優先する。small-target scalp が retry basis と executed price のズレで actual RR を失うことを避け、preflight の risk gate 自体は変えない。監査根拠は `docs/TRADE_FINDINGS.md` / `docs/RISK_AND_EXECUTION.md` を正とする。
- 2026-03-12 追記: `PrecisionLowVol` の current 運用は short 全停止を正とせず、recent stop-loss burst を使った setup-pressure guard を worker local に持つ。`workers/scalp_wick_reversal_blend/worker.py` は `range_reason=volatility_compression` の recent `PrecisionLowVol` short close を集計し、`2x STOP_LOSS_ORDER` と `1x fast SL (<=35s)` が `180s` 以内に並ぶ burst 中は weak re-entry を reject する。`touch_ratio / rev_strength / setup_quality / reversion_support / projection.score` が強い reclaim short は維持し、shared gate / time block / strategy-wide blanket trim は追加しない。監査根拠は `docs/TRADE_FINDINGS.md` / `docs/RISK_AND_EXECUTION.md` を正とする。
- 2026-03-12 追記: `PrecisionLowVol` の current `volatility_compression` short は、setup-pressure 発火前でも `flow_guard` 付き weak overbought lane を worker local で落とす。`workers/scalp_wick_reversal_blend/worker.py` は `rsi>=60`, `projection.score<=0`, `setup_quality<0.46` の short を reject し、positive projection か higher-quality の reclaim short だけを残す。shared gate / time block / strategy-wide blanket trim は追加しない。監査根拠は `docs/TRADE_FINDINGS.md` / `docs/RISK_AND_EXECUTION.md` を正とする。
- 2026-03-12 追記: `WickReversalBlend` の current 運用は fixed long stop を正とせず、`市況 + recent outcome` で weak countertrend long を worker local に絞る。`workers/scalp_wick_reversal_blend/policy.py` は `range_reason / rsi / projection.score / macd_hist_pips / di_gap` から weak long lane を落とし、`workers/scalp_wick_reversal_blend/worker.py` は recent `STOP_LOSS_ORDER` の setup-pressure が active な間だけ `bbw / range_score / wick_quality / projection_score` の shallow long を追加で reject する。momentum が整った reclaim long は残す。あわせて `execution/order_manager.py` は fill 直後の submit-time `SL/TP` を cache して、actual fill への realign が不要なときは child order を cancel/recreate しない。shared gate / time block / blanket stop は追加しない。
- 2026-03-12 追記: `DroughtRevert` の current 運用は long 全停止を正とせず、recent outcome を使った dynamic setup-pressure guard を worker local に持つ。`workers/scalp_wick_reversal_blend/worker.py` は recent `DroughtRevert` long close の `sl_rate / fast_sl_rate / net_jpy / age` を集計し、active pressure 中だけ `projection.score<=0.08` で `setup_quality / reversion_support` が弱いか `continuation_pressure` が高い reclaim long を reject する。`touch_ratio / rev_strength / setup_quality / reversion_support / projection` が強い reclaim long は維持し、shared gate / time block / blanket stop は追加しない。
- 2026-03-12 追記: `workers/scalp_wick_reversal_blend` 系の current 運用は、worker-local `flow_guard` を `flow_regime=range_fade|continuation_headwind` の binary に潰して shared setup context を上書きしない。worker は coarse headwind label を `flow_headwind_regime` として保持し、`flow_regime / microstructure_bucket / setup_fingerprint` は `execution/strategy_entry.py` の richer live setup context を優先して current market phase を残す。市況ラベルの flattening による setup learning 劣化を避ける。
- 2026-03-12 追記: `execution/strategy_entry.py` の current 運用では、`market_order` / `limit_order` と reject path の `entry_path_attribution` が `live_setup_context` stage を必ず持つことを回帰テストで固定する。worker-local の coarse headwind label は `flow_headwind_regime` に留め、dispatch path 上で richer live setup context が再び stage ごと欠落・flatten しても test failure で止まる状態を正とする。
- 2026-03-12 追記: `workers/scalp_wick_reversal_blend` 系の `PrecisionLowVol` / `DroughtRevert` / `WickReversalBlend` は、current live で worker-local の `setup_pressure` / `flow_guard` / `RR` 修正を優先する。soft block (`pf<1` や `failfast_soft`) で entry starvation を起こしている間は perf guard を dedicated env 側で明示的に切る。wrapper 経由の `PrecisionLowVol` / `DroughtRevert` は `SCALP_PRECISION_LOWVOL_PERF_GUARD_ENABLED=0` / `SCALP_PRECISION_DROUGHT_REVERT_PERF_GUARD_ENABLED=0` の source-prefix key を正とし、wrapper を使わない `WickReversalBlend` は `SCALP_PRECISION_PERF_GUARD_ENABLED=0` を使う。`VwapRevertS` のような `hard:failfast` lane は reopen せず、hard loser は別扱いとする。
- 2026-03-12 追記: `scalp_extrema_reversal_live` の current entry starvation では、worker/local/shared の dynamic trim は残しつつ、`quant-order-manager` の preserve-intent 二次縮小だけを strategy-scoped に止める。`ops/env/quant-order-manager.env` の現行運用値は `ORDER_MANAGER_PRESERVE_INTENT_REJECT_UNDER_STRATEGY_SCALP_EXTREMA_REVERSAL(_LIVE)=0.35` と `ORDER_MANAGER_PRESERVE_INTENT_MIN/MAX_SCALE_STRATEGY_SCALP_EXTREMA_REVERSAL(_LIVE)=1.00` を正とし、`analysis_feedback` / `participation_alloc` / `auto_canary` / `dynamic_alloc` 後の `entry_units_intent` を order-manager が `entry_probability_below_min_units` へ再度落とさない。`entry_probability<0.35` の truly weak setup だけは共通 reject を維持する。
- 2026-03-12 追記: current fast-profit 運用では、shared participation は zero-profit lane を `boost_participation` しない。`scripts/participation_allocator.py` は strategy / setup ともに `filled_rate` や `share_gap` が強くても `realized_jpy > 0.0` を満たさない lane を boost せず、`dynamic_alloc` の trim と衝突させない。winner cadence 回復は positive realized が確認できた lane だけへ寄せる。
- 2026-03-12 追記: `scalp_extrema_reversal_live` の current 運用は entry 数の維持だけでなく、`mid-RSI` の shallow loser probe を worker-local で落とす。`workers/scalp_extrema_reversal/worker.py` は non-supportive `range_compression / volatility_compression` で、short は `RSI<=60`, `dist_high<=0.90`, `short_bounce<=0.85`, `tick_strength<=0.25`, `range_score>=0.50`、long は `RSI>=40`, `dist_low<=0.85`, `long_bounce<=0.25`, `tick_strength<=0.25`, `ADX>=15`, `range_score>=0.55` の probe を reject する。high-RSI short winner lane は維持し、shared gate / time block / blanket stop は追加しない。
- 2026-03-12 追記: shared setup identity の current 運用では、common 形式の `setup_fingerprint` (`Strategy|side|flow_regime|microstructure_bucket|...`) を `flow_regime / microstructure_bucket` 復元の正とする。`workers/common/setup_context.py` の `derive_live_setup_context` / `extract_setup_identity` は pre-fix trade に stale な top-level `flow_regime=range_fade` が残っていても、fingerprint が `range_compression` など richer setup を含む場合はそちらを優先し、`dynamic_alloc` / `participation_alloc` / `strategy_feedback` / `auto_canary` が current 6-24h trade を miscluster しない。`RangeFader` のような custom fingerprint は parser 対象外とし、explicit context を維持する。
