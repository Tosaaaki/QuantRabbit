...
class _ManualPressureSnapshot:
    def __init__(
        self, margin_available: float, free_margin_ratio: float, health_buffer: float
    ):
        self.margin_available = margin_available
...
def test_preflight_allows_margin_reduction_on_hedge(monkeypatch):
    ...
    monkeypatch.setattr(
        "utils.oanda_account.get_position_summary", lambda timeout=3.0: (0.0, 80_000.0)
    )
...
def test_preflight_scales_down_with_budget(monkeypatch):
    ...
    monkeypatch.setattr(
        "utils.oanda_account.get_position_summary", lambda timeout=3.0: (50_000.0, 0.0)
    )
...
def test_preflight_uses_recent_transient_stale_snapshot(monkeypatch):
    ...
    monkeypatch.setattr(
        "utils.oanda_account.get_account_snapshot_state", lambda **_kwargs: state
    )
    monkeypatch.setattr(
        "utils.oanda_account.get_position_summary", lambda timeout=3.0: (50_000.0, 0.0)
    )
...
def test_manual_margin_pressure_details_blocks_when_manual_exposure_and_margin_tight(
    monkeypatch,
):
    ...
    monkeypatch.setattr(
        order_manager, "_ORDER_MANUAL_MARGIN_GUARD_MIN_FREE_RATIO", 0.12
    )
    monkeypatch.setattr(
        order_manager, "_ORDER_MANUAL_MARGIN_GUARD_MIN_HEALTH_BUFFER", 0.18
    )
    monkeypatch.setattr(
        order_manager, "_ORDER_MANUAL_MARGIN_GUARD_MIN_AVAILABLE_JPY", 8000.0
    )
...
def test_projected_usage_with_netting_allows_reduction(monkeypatch):
    ...
    monkeypatch.setattr(
        "utils.oanda_account.get_position_summary", lambda: (0.0, 80_000.0)
    )
...
def test_entry_quality_gate_strategy_penalty_blocks_low_quality_tag(
    monkeypatch,
) -> None:
    monkeypatch.setattr(
        order_manager,
...
def test_entry_quality_microstructure_gate_blocks_on_low_density(monkeypatch):
    ...
    monkeypatch.setenv(
        "ORDER_ENTRY_QUALITY_MICROSTRUCTURE_MIN_TICK_DENSITY_SCALP", "1.0"
    )
...
def test_quote_is_usable_blocks_crossed_and_wide_spread(monkeypatch) -> None:
    ...
    assert (
        order_manager._quote_is_usable(
            {"bid": 150.0, "ask": 150.01, "spread_pips": 1.0}
        )
        is True
    )
    assert (
        order_manager._quote_is_usable(
            {"bid": 150.01, "ask": 150.00, "spread_pips": -1.0}
        )
        is False
    )
    assert (
        order_manager._quote_is_usable(
            {"bid": 150.0, "ask": 150.05, "spread_pips": 5.0}
        )
        is False
    )
...
def test_policy_generation_marker_applies_only_to_new_entries(monkeypatch) -> None:
    monkeypatch.setattr(
        order_manager, "_ENTRY_POLICY_GENERATION", "2026-02-11-losscap-v1"
    )
...
def test_apply_min_rr_floor_can_expand_tp_with_preserve_intent_safe_mode(
    monkeypatch,
) -> None:
    monkeypatch.setattr(order_manager, "_MIN_RR_ENABLED", True)
    monkeypatch.setattr(order_manager, "_MIN_RR_BY_POCKET", {"scalp": 1.5})
...
def test_order_manager_perf_guard_bypass_uses_strategy_specific_keys(
    monkeypatch,
) -> None:
    ...
    monkeypatch.setenv(
        "ORDER_MANAGER_PERF_GUARD_BYPASS_STRATEGY_WICKREVERSALBLEND", "1"
    )
...
def test_protection_fallback_gap_price_prefers_strategy_then_pocket(
    monkeypatch,
) -> None:
    ...
    assert (
        _protection_fallback_gap_price("scalp_fast", strategy_tag="scalp_ping_5s_live")
        == 0.015
    )
